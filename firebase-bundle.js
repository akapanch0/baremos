...
`, 10],
  ...
  static toUpperCase(codepoint) {
    if (codepoint < ASCII_SIZE)
      return ASCII_TO_UPPER[codepoint];
  ...
  static toLowerCase(codepoint) {
    if (codepoint < ASCII_SIZE)
      return ASCII_TO_LOWER[codepoint];
...
var Unicode = class Unicode2 {
  ...
  static isPrint(r) {
    if (r <= Unicode2.MAX_LATIN1)
      return r >= 32 && r < Unicode2.MAX_ASCII || r >= 161 && r !== 173;
  ...
  static equalsIgnoreCase(r1, r2) {
    ...
    for (let r = Unicode2.simpleFold(r1);r !== r1; r = Unicode2.simpleFold(r))
      if (r === r2)
        return true;
    return false;
  }
...
for (let i = 0;i < FAST_PATH_TABLE_SIZE; i++)
  WORD_RUNE_TABLE[i] = 97 <= i && i <= 122 || 65 <= i && i <= 90 || 48 <= i && i <= 57 || i === 95 ? 1 : 0;
...
var Utils = class Utils2 {
  ...
  static escapeRune(rune) {
    ...
    if (Unicode.isPrint(rune)) {
      if (Utils2.METACHARACTERS.indexOf(String.fromCodePoint(rune)) >= 0)
        out += "\\";
...
`):
  ...
  static emptyOpContext(r1, r2) {
    let op = 0;
    if (r1 < 0)
      op |= Utils2.EMPTY_BEGIN_TEXT | Utils2.EMPTY_BEGIN_LINE;
    if (r1 === 10)
      op |= Utils2.EMPTY_BEGIN_LINE;
    if (r2 < 0)
      op |= Utils2.EMPTY_END_TEXT | Utils2.EMPTY_END_LINE;
    if (r2 === 10)
      op |= Utils2.EMPTY_END_LINE;
    if (Utils2.isWordRune(r1) !== Utils2.isWordRune(r2))
      op |= Utils2.EMPTY_WORD_BOUNDARY;
    else
      op |= Utils2.EMPTY_NO_WORD_BOUNDARY;
    return op;
  }
  static quoteMeta(str) {
    return str.split("").map((s) => {
      if (Utils2.METACHARACTERS.indexOf(s) >= 0)
        return `\\${s}`;
      return s;
    }).join("");
  }
  ...
  static stringToUtf8ByteArray(str) {
    if (globalThis.TextEncoder) {
      if (!cachedNativeEncoder)
        cachedNativeEncoder = new TextEncoder;
  ...
  static utf8ByteArrayToString(bytes) {
    if (globalThis.TextDecoder) {
      if (!cachedNativeDecoder)
        cachedNativeDecoder = new TextDecoder("utf-8");
  ...
  indexOf(source, target, fromIndex = 0) {
    ...
    while (i <= limit) {
      if (hasNativeIndexOf) {
        ...
        while (i <= limit && source[i] !== firstByte)
          i++;
        if (i > limit)
          return -1;
      }
...
var MachineUTF16Input = class extends MachineInputBase {
  ...
  step(pos) {
    ...
    if (c2 >= Unicode.MIN_LOW_SURROGATE && c2 <= Unicode.MAX_LOW_SURROGATE)
      return (c1 - Unicode.MIN_HIGH_SURROGATE) * 1024 + (c2 - Unicode.MIN_LOW_SURROGATE) + Unicode.MIN_SUPPLEMENTARY_CODE_POINT << 3 | 2;
    return c1 << 3 | 1;
  }
  index(re2, pos) {
    pos += this.start;
...
var Matcher = class Matcher2 {
  ...
  getNamedGroups() {
    ...
    for (const name4 of Object.keys(this.namedGroups))
      result[name4] = this.group(name4);
    return result;
  }
  groupCount() {
      ...
      if (cCode === Codepoint.CODES.get("$")) {
        ...
        if (Codepoint.CODES.get("0") <= nextCode && nextCode <= Codepoint.CODES.get("9")) {
          ...
          for (;j < m; j++) {
            ...
            if (digit < Codepoint.CODES.get("0") || digit > Codepoint.CODES.get("9") || n * 10 + digit - Codepoint.CODES.get("0") > this.patternGroupCount)
              break;
  ...
  replace(replacement, all = true, javaMode = false) {
    ...
    if (isFunc) {
      if (this.groupCount() >= Matcher2.MAX_REPLACER_ARGS)
  ...
  matchSet(input, pos, anchor) {
    ...
    while (true) {
      ...
      for (let j = 0;j < runq.size; j++) {
        ...
        switch (i.op) {
          case Inst.MATCH:
            if (anchor === RE2Flags.ANCHOR_BOTH && currentPos !== input.endPos())
              break;
            matches.add(i.arg);
            break;
          case Inst.RUNE:
...
var Backtracker = class Backtracker2 {
  ...
  static maxBitStateLen(prog) {
    if (!Backtracker2.shouldBacktrack(prog))
      return 0;
...
var mergeRuneSets = (leftRunes, rightRunes, leftPC, rightPC) => {
  ...
  const extend = (isLeft) => {
    ...
    if (ix > 0 && newArray[low] <= merged[ix])
      return false;
...
var onePassCopy = (prog) => {
  ...
  for (let pc = 0;pc < p.inst.length; pc++) {
    ...
    if (inst.op !== Inst.ALT && inst.op !== Inst.ALT_MATCH)
      continue;
    let pAOther = "out";
    let pAAlt = "arg";
    ...
    if (instOther.op === Inst.ALT || instOther.op === Inst.ALT_MATCH)
      continue;
    let pBAlt = "out";
    let pBOther = "arg";
    let patch = false;
    ...
    if (patch)
      instAlt[pBAlt] = inst[pAOther];
    if (inst[pAOther] === instAlt[pBAlt])
      inst[pAAlt] = instAlt[pBOther];
  }
  return p;
};
var makeOnePass = (p) => {
  ...
  const check = (pc) => {
    ...
    switch (inst.op) {
      ...
      case Inst.RUNE: {
        m[pc] = false;
        if (inst.next && inst.next.length > 0)
          break;
  ...
  while (!instQueue.empty()) {
    visitQueue.clear();
    if (!check(instQueue.next()))
      return null;
  }
  for (let i = 0;i < p.inst.length; i++)
    if (onePassRunes[i])
      p.inst[i].runes = onePassRunes[i];
  return p;
};
...
var OnePass = class OnePass2 {
  ...
  static next(inst, r) {
    ...
    if (nextIdx >= 0)
      return inst.next[nextIdx];
    if (inst.op === Inst.ALT_MATCH)
      return inst.out;
    return 0;
  }
  static execute(re2, input, pos, anchor, ncap) {
    ...
    while (true) {
      ...
      switch (inst.op) {
        ...
        case Inst.RUNE:
          if (!inst.matchRune(rune))
            return null;
          break;
        case Inst.RUNE1:
          if (rune !== inst.runes[0])
            return null;
          break;
        case Inst.RUNE_ANY:
          break;
...
var Regexp = class Regexp2 {
  ...
  appendTo() {
    ...
    switch (this.op) {
      ...
      case Regexp2.Op.REPEAT: {
        ...
        if (sub.op > Regexp2.Op.CAPTURE || sub.op === Regexp2.Op.LITERAL && sub.runes.length > 1)
          out += `(?:${sub.appendTo()})`;
        else
          out += sub.appendTo();
        switch (this.op) {
      ...
      case Regexp2.Op.CONCAT:
        for (let sub of this.subs)
          if (sub.op === Regexp2.Op.ALTERNATE)
            out += `(?:${sub.appendTo()})`;
          else
            out += sub.appendTo();
        break;
      ...
      case Regexp2.Op.CAPTURE:
        if (this.name === null || this.name.length === 0)
          ...
          out += `(?P<${this.name}>`;
        if (this.subs[0].op !== Regexp2.Op.EMPTY_MATCH)
          out += this.subs[0].appendTo();
        out += ")";
        break;
  ...
  equals(that) {
    if (!(that !== null && that instanceof Regexp2))
      return false;
    if (this.op !== that.op)
      return false;
    switch (this.op) {
    ...
    switch (re.op) {
      ...
      case Regexp.Op.CONCAT: {
        ...
        for (const sub of re.subs)
          pf.subs.push(PrefilterTree2.fromRegexp(sub));
        return pf;
      }
...
`) + 1 && re.runes[3] === Unicode.MAX_RUNE)
      ...
      case Regexp.Op.REPEAT: {
        ...
        if (re.max === -1) {
          if (re.min === 0)
            return Simplify2.simplify1(Regexp.Op.STAR, re.flags, sub, null);
          if (re.min === 1)
            return Simplify2.simplify1(Regexp.Op.PLUS, re.flags, sub, null);
...
var CharClass = class CharClass2 {
  ...
  static qsortIntPair(array, left, right) {
    ...
    let j = right;
    while (i <= j) {
      while (i < right && CharClass2.cmp(array, i, pivotFrom, pivotTo) < 0)
        i += 2;
      while (j > left && CharClass2.cmp(array, j, pivotFrom, pivotTo) > 0)
        j -= 2;
      if (i <= j) {
        if (i !== j) {
          let temp = array[i];
    ...
    if (left < j)
      CharClass2.qsortIntPair(array, left, j);
    if (i < right)
      CharClass2.qsortIntPair(array, i, right);
  }
  ...
  appendGroup(g, foldCase) {
    let cls = g.cls;
    if (foldCase)
      cls = new CharClass2().appendFoldedClass(cls).cleanClass().toArray();
...
var Parser = class Parser2 {
  ...
  static parseRepeat(t) {
    ...
    if (!t.lookingAt(","))
      ...
      if (t.lookingAt("}"))
        max = -1;
      else if ((max = Parser2.parseInt(t)) === -1)
        return -1;
    }
    if (!t.more() || !t.lookingAt("}"))
      return -1;
    ...
    if (min < 0 || min > 1000 || max === -2 || max > 1000 || max >= 0 && min > max)
      throw new RE2JSSyntaxException(Parser2.ERR_INVALID_REPEAT_SIZE, t.from(start));
...
`)))
  ...
  static parseEscape(t) {
    ...
    t.skip(1);
    if (!t.more())
      throw new RE2JSSyntaxException(Parser2.ERR_TRAILING_BACKSLASH);
    let c = t.pop();
    bigswitch:
      switch (c) {
...
`);
  ...
  static parseClassChar(t, wholeClassPos) {
    if (!t.more())
      throw new RE2JSSyntaxException(Parser2.ERR_MISSING_BRACKET, t.from(wholeClassPos));
    if (t.lookingAt("\\"))
      return Parser2.parseEscape(t);
    return t.pop();
  }
  ...
  static hasCapture(re) {
    if (re === null)
      return false;
    if (re.op === Regexp.Op.CAPTURE)
      return true;
    if (re.subs) {
      for (let sub of re.subs)
        if (Parser2.hasCapture(sub))
          return true;
    }
    return false;
  }
  ...
  checkLimits(re) {
    if (this.numRunes > Parser2.MAX_RUNES)
      throw new RE2JSSyntaxException(Parser2.ERR_LARGE);
    this.checkSize(re);
    this.checkHeight(re);
  }
  checkSize(re) {
    if (this.size === null) {
      ...
      if (re.op === Regexp.Op.REPEAT) {
        ...
        if (n <= 0)
          n = 1;
        if (n > Math.floor(Parser2.MAX_SIZE / this.repeats))
          this.repeats = Parser2.MAX_SIZE;
        else
          this.repeats *= n;
      }
      if (this.numRegexp < Math.floor(Parser2.MAX_SIZE / this.repeats))
        return;
      this.size = /* @__PURE__ */ new Map;
      for (let reEx of this.stack)
        this.checkSize(reEx);
    }
    if (this.calcSize(re, true) > Parser2.MAX_SIZE)
      throw new RE2JSSyntaxException(Parser2.ERR_LARGE);
  }
  ...
  checkHeight(re) {
    if (this.numRegexp < Parser2.MAX_HEIGHT)
      return;
    if (this.height === null) {
    ...
    if (this.calcHeight(re, true) > Parser2.MAX_HEIGHT)
      throw new RE2JSSyntaxException(Parser2.ERR_NESTING_DEPTH);
  }
  ...
  repeat(op, min, max, beforePos, t, lastRepeatPos) {
    ...
    if ((flags & RE2Flags.PERL_X) !== 0) {
      ...
      if (lastRepeatPos !== -1)
        throw new RE2JSSyntaxException(Parser2.ERR_INVALID_REPEAT_OP, t.from(lastRepeatPos));
    }
    const n = this.stack.length;
    if (n === 0)
      throw new RE2JSSyntaxException(Parser2.ERR_MISSING_REPEAT_ARGUMENT, t.from(beforePos));
    ...
    if (op === Regexp.Op.REPEAT && (min >= 2 || max >= 2) && !this.repeatIsValid(re, 1000))
      throw new RE2JSSyntaxException(Parser2.ERR_INVALID_REPEAT_SIZE, t.from(beforePos));
  }
  repeatIsValid(re, n) {
    if (re.op === Regexp.Op.REPEAT) {
      let m = re.max;
...
`) + 1 && re.runes[3] === Unicode.MAX_RUNE) {
  ...
  factor(array) {
    ...
    for (let i = 0;i <= lensub; i++) {
      ...
      if (i < lensub) {
        ...
        if (first !== null && first.equals(ifirst) && (Parser2.isCharClass(first) || first.op === Regexp.Op.REPEAT && first.min === first.max && Parser2.isCharClass(first.subs[0])))
          continue;
      }
      if (i === start) {} else if (i === start + 1)
        array[lenout++] = array[s + start];
    ...
    lenout = 0;
    for (let i = 0;i <= lensub; i++) {
      if (i < lensub && Parser2.isCharClass(array[s + i]))
        continue;
      if (i === start) {} else if (i === start + 1)
        array[lenout++] = array[s + start];
      else {
  ...
  parseInternal() {
    if ((this.flags & RE2Flags.LITERAL) !== 0)
      return Parser2.literalRegexp(this.wholeRegexp, this.flags);
    let lastRepeatPos = -1;
    let min = -1;
    let max = -1;
    ...
    if (this.stack.length !== 1)
      throw new RE2JSSyntaxException(Parser2.ERR_MISSING_PAREN, this.wholeRegexp);
  ...
  parsePerlFlags(t) {
    ...
    if (s.startsWith("(?P<") || s.startsWith("(?<")) {
      ...
      const end = s.indexOf(">");
      if (end < 0)
        throw new RE2JSSyntaxException(Parser2.ERR_INVALID_NAMED_CAPTURE, s);
      const name4 = s.substring(begin, end);
      t.skipString(name4);
      t.skip(begin + 1);
      if (!Parser2.isValidCaptureName(name4))
        throw new RE2JSSyntaxException(Parser2.ERR_INVALID_NAMED_CAPTURE, s.substring(0, end + 1));
      const re = this.op(Regexp.Op.LEFT_PAREN);
      re.cap = ++this.numCap;
      if (this.namedGroups[name4])
        throw new RE2JSSyntaxException(Parser2.ERR_DUPLICATE_NAMED_CAPTURE, name4);
  ...
  parseRightParen() {
    ...
    if (this.stack.length < 2)
      throw new RE2JSSyntaxException(Parser2.ERR_UNEXPECTED_PAREN, this.wholeRegexp);
    const re1 = this.pop();
    const re2 = this.pop();
    ...
    if (re2.lb !== 0) {
      if (Parser2.hasCapture(re1))
        throw new RE2JSSyntaxException(Parser2.ERR_INVALID_CAPTURE_IN_LOOKBEHIND, this.wholeRegexp);
      if (re2.lb > 0)
  ...
  parseNamedClass(t, cc) {
    ...
    if (g === null)
      throw new RE2JSSyntaxException(Parser2.ERR_INVALID_CHAR_RANGE, name4);
    cc.appendGroup(g, (this.flags & RE2Flags.FOLD_CASE) !== 0);
    return true;
  }
  parseUnicodeClass(t, cc) {
    ...
    if (pair === null)
      throw new RE2JSSyntaxException(Parser2.ERR_INVALID_CHAR_RANGE, t.from(startPos));
    if (pair.sign < 0)
      sign = 0 - sign;
    const tab = pair.tab;
    ...
    while (!t.more() || t.peek() !== Codepoint.CODES.get("]") || first) {
      ...
      if (t.more() && t.lookingAt("-")) {
        ...
        if (t.more() && t.lookingAt("]"))
          ...
          if (hi < lo)
            throw new RE2JSSyntaxException(Parser2.ERR_INVALID_CHAR_RANGE, t.from(beforePos));
        }
      }
      if ((this.flags & RE2Flags.FOLD_CASE) === 0)
...
var RE2Set = class RE2Set2 {
  ...
  match(input) {
    ...
    if (this.anchor === RE2Set2.ANCHOR_START)
      internalAnchor = RE2Flags.ANCHOR_START;
    else if (this.anchor === RE2Set2.ANCHOR_BOTH)
          ...
          switch (ch) {
              ...
              if (i + 2 < size) {
                if (data[i + 2] === "{") {
                  ...
                  while (j < size) {
                    ...
                    if (!TranslateRegExpString2.isHexadecimal(hexChar))
                      break;
                    hasHex = true;
                    j++;
                  }
                  ...
                  let isHex4 = true;
                  for (let j = 0;j < 4; j++)
                    if (!TranslateRegExpString2.isHexadecimal(data[i + 2 + j])) {
                      isHex4 = false;
                      break;
                    }
                  if (isHex4) {
            ...
            case "x": {
              ...
              if (i + 2 < size && data[i + 2] === "{") {
                ...
                if (closed && hasHex)
                  isValidHex = true;
              } else if (i + 3 < size && TranslateRegExpString2.isHexadecimal(data[i + 2]) && TranslateRegExpString2.isHexadecimal(data[i + 3]))
                isValidHex = true;
              if (isValidHex) {
                result += "\\x";
                i += 2;
...
var RE2JS = class RE2JS2 {
  ...
  static compile(regex, flags = 0) {
    let fregex = regex;
    if ((flags & RE2JS2.CASE_INSENSITIVE) !== 0)
      fregex = `(?i)${fregex}`;
    if ((flags & RE2JS2.DOTALL) !== 0)
      fregex = `(?s)${fregex}`;
    if ((flags & RE2JS2.MULTILINE) !== 0)
      fregex = `(?m)${fregex}`;
    if ((flags & ~(RE2JS2.MULTILINE | RE2JS2.DOTALL | RE2JS2.CASE_INSENSITIVE | RE2JS2.DISABLE_UNICODE_GROUPS | RE2JS2.LONGEST_MATCH | RE2JS2.LOOKBEHINDS)) !== 0)
    ...
    let re2Flags = RE2Flags.PERL;
    if ((flags & RE2JS2.DISABLE_UNICODE_GROUPS) !== 0)
      re2Flags &= ~RE2Flags.UNICODE_GROUPS;
    if ((flags & RE2JS2.LOOKBEHINDS) !== 0)
      re2Flags |= RE2Flags.LOOKBEHIND;
  ...
  exec(input) {
    ...
    for (let i = 1;i <= m.groupCount(); i++) {
      const val = m.group(i);
      result.push(val === null ? undefined : val);
    }
...
// node_modules/@firebase/firestore/dist/common-CLMydGSF.esm.js
var Se = "12.19.0";
...
class e extends FirebaseError {
  constructor(e2, t) {
    super(e2, t), this.code = e2, this.message = t, this.toString = () => `${this.name}: [code=${this.code}]: ${this.message}`;
  }
}
var Ce = "__name__";
...
class BasePath {
  constructor(e2, t, n) {
    t === undefined ? t = 0 : t > e2.length && l(637, {
      offset: t,
      range: e2.length
    }), n === undefined ? n = e2.length - t : n > e2.length - t && l(1746, {
      length: n,
      range: e2.length - t
    }), this.segments = e2, this.offset = t, this.len = n;
  }
  get length() {
    return this.len;
  }
  isEqual(e2) {
    return BasePath.comparator(this, e2) === 0;
  }
  child(e2) {
    const t = this.segments.slice(this.offset, this.limit());
    return e2 instanceof BasePath ? e2.forEach((e3) => {
      t.push(e3);
    }) : t.push(e2), this.construct(t);
  }
  limit() {
    return this.offset + this.length;
  }
  popFirst(e2) {
    return e2 = e2 === undefined ? 1 : e2, this.construct(this.segments, this.offset + e2, this.length - e2);
  }
  popLast() {
  ...
  isPrefixOf(e2) {
    ...
    for (let t = 0;t < this.length; t++)
      if (this.get(t) !== e2.get(t))
        return false;
    return true;
  }
  isImmediateParentOf(e2) {
    if (this.length + 1 !== e2.length)
      return false;
    for (let t = 0;t < this.length; t++)
      if (this.get(t) !== e2.get(t))
        return false;
  ...
  static comparator(e2, t) {
    const n = Math.min(e2.length, t.length);
    for (let r = 0;r < n; r++) {
      const n2 = BasePath.compareSegments(e2.get(r), t.get(r));
      if (n2 !== 0)
        return n2;
    }
    return __PRIVATE_primitiveComparator(e2.length, t.length);
  }
  static compareSegments(e2, t) {
    const n = BasePath.isNumericId(e2), r = BasePath.isNumericId(t);
    return n && !r ? -1 : !n && r ? 1 : n && r ? BasePath.extractNumericId(e2).compare(BasePath.extractNumericId(t)) : __PRIVATE_compareUtf8Strings(e2, t);
  }
...
class ResourcePath extends BasePath {
  construct(e2, t, n) {
    return new ResourcePath(e2, t, n);
  }
  canonicalString() {
  ...
  static fromString(...t) {
    ...
    for (const r of t) {
      if (r.indexOf("//") >= 0)
        throw new e(ta.INVALID_ARGUMENT, `Invalid segment (${r}). Paths must not contain // in them.`);
      n.push(...r.split("/").filter((e2) => e2.length > 0));
    }
...
var Oe = class FieldPath extends BasePath {
  construct(e2, t, n) {
    return new FieldPath(e2, t, n);
  }
  static isValidIdentifier(e2) {
    return Fe.test(e2);
  }
  canonicalString() {
    return this.toArray().map((e2) => (e2 = e2.replace(/\\/g, "\\\\").replace(/`/g, "\\`"), FieldPath.isValidIdentifier(e2) || (e2 = "`" + e2 + "`"), e2)).join(".");
  }
  toString() {
    return this.canonicalString();
  }
  isKeyField() {
    return this.length === 1 && this.get(0) === Ce;
  }
  static keyField() {
  ...
  static fromServerFormat(t) {
    ...
    const __PRIVATE_addCurrentSegment = () => {
      if (r.length === 0)
        throw new e(ta.INVALID_ARGUMENT, `Invalid field path (${t}). Paths must not be empty, begin with '.', end with '.', or contain '..'`);
      n.push(r), r = "";
    };
    let s = false;
    for (;i < t.length; ) {
      ...
      if (n2 === "\\") {
        if (i + 1 === t.length)
          throw new e(ta.INVALID_ARGUMENT, "Path has trailing escape character: " + t);
        const n3 = t[i + 1];
        if (n3 !== "\\" && n3 !== "." && n3 !== "`")
          throw new e(ta.INVALID_ARGUMENT, "Path has invalid escape sequence: " + t);
        r += n3, i += 2;
      } else
        n2 === "`" ? (s = !s, i++) : n2 !== "." || s ? (r += n2, i++) : (__PRIVATE_addCurrentSegment(), i++);
    }
    if (__PRIVATE_addCurrentSegment(), s)
      throw new e(ta.INVALID_ARGUMENT, "Unterminated ` in path: " + t);
...
class FieldMask {
  constructor(e2) {
    this.fields = e2, e2.sort(Oe.comparator);
  }
  static empty() {
  ...
  isEqual(e2) {
    return __PRIVATE_arrayEquals(this.fields, e2.fields, (e3, t) => e3.isEqual(t));
  }
}
function __PRIVATE_objectSize(e2) {
  let t = 0;
  for (const n in e2)
    Object.prototype.hasOwnProperty.call(e2, n) && t++;
  return t;
}
function forEach(e2, t) {
  for (const n in e2)
    Object.prototype.hasOwnProperty.call(e2, n) && t(n, e2[n]);
}
function __PRIVATE_mapToArray(e2, t) {
  const n = [];
  for (const r in e2)
    Object.prototype.hasOwnProperty.call(e2, r) && n.push(t(e2[r], r, e2));
  return n;
}
function isEmpty2(e2) {
  for (const t in e2)
...
class DocumentKey {
  ...
  isEqual(e2) {
    return e2 !== null && ResourcePath.comparator(this.path, e2.path) === 0;
  }
  toString() {
    return this.path.toString();
  }
  static comparator(e2, t) {
    return ResourcePath.comparator(e2.path, t.path);
  }
...
function __PRIVATE_validateNonEmptyArgument(t, n, r) {
  if (!r)
    throw new e(ta.INVALID_ARGUMENT, `Function ${t}() cannot be called with an empty ${n}.`);
}
function __PRIVATE_validateIsNotUsedTogether(t, n, r, i) {
  if (n === true && i === true)
    throw new e(ta.INVALID_ARGUMENT, `${t} and ${r} cannot be used together.`);
}
function __PRIVATE_validateDocumentPath(t) {
  if (!DocumentKey.isDocumentKey(t))
    throw new e(ta.INVALID_ARGUMENT, `Invalid document reference. Document references must have an even number of segments, but ${t} has ${t.length}.`);
}
function __PRIVATE_validateCollectionPath(t) {
  if (DocumentKey.isDocumentKey(t))
    throw new e(ta.INVALID_ARGUMENT, `Invalid collection reference. Collection references must have an odd number of segments, but ${t} has ${t.length}.`);
}
function p(e2) {
...
function __PRIVATE_valueDescription(e2) {
  ...
  if (e2 === null)
    return "null";
  if (typeof e2 == "string")
    return e2.length > 20 && (e2 = `${e2.substring(0, 20)}...`), JSON.stringify(e2);
  ...
  if (typeof e2 == "object") {
    if (e2 instanceof Array)
      return "an array";
    {
      const t = function __PRIVATE_tryGetCustomObjectType(e3) {
        if (e3.constructor)
  ...
  return typeof e2 == "function" ? "a function" : l(12329, {
    type: typeof e2
  });
}
function ra(t, n) {
  if ("_delegate" in t && (t = t._delegate), !(t instanceof n)) {
    if (n.name === t.constructor.name)
      throw new e(ta.INVALID_ARGUMENT, "Type does not match the expected instance. Did you pass a reference from a different Firestore SDK?");
    {
...
function __PRIVATE_validateJSON(t, n) {
  if (!p(t))
    throw new e(ta.INVALID_ARGUMENT, "JSON must be an object");
  let r;
  for (const e2 in n)
    if (n[e2]) {
      const i = n[e2].typeString, s = "value" in n[e2] ? {
        value: n[e2].value
      } : undefined;
      if (!(e2 in t)) {
        r = `JSON missing required field: '${e2}'`;
        break;
      }
      const _ = t[e2];
      if (i && typeof _ !== i) {
        r = `JSON field '${e2}' must be a ${i}.`;
        break;
      }
      if (s !== undefined && _ !== s.value) {
        r = `Expected '${e2}' field to equal '${s.value}'`;
        break;
      }
    }
  if (r)
    throw new e(ta.INVALID_ARGUMENT, r);
  return true;
}
var Me = -62135596800;
var Ne = 1e6;
...
class Timestamp {
  ...
  static fromMillis(e2) {
    const t = Math.floor(e2 / 1000), n = Math.floor((e2 - 1000 * t) * Ne);
    return new Timestamp(t, n);
  }
  static fromInstant(t) {
    if (!t || typeof t.t != "bigint")
      throw new e(ta.INVALID_ARGUMENT, "Invalid Temporal.Instant object provided.");
    return Timestamp._fromEpochNanoseconds(t.t);
  }
  static _fromEpochNanoseconds(e2) {
    let t, n;
    if (e2 >= 0n)
      t = Number(e2 / 1000000000n), n = Number(e2 % 1000000000n);
    else {
      const r = e2 % 1000000000n;
      r === 0n ? (t = Number(e2 / 1000000000n), n = 0) : (t = Number(e2 / 1000000000n - 1n), n = Number(r + 1000000000n));
    }
    return new Timestamp(t, n);
  }
  constructor(t, n) {
    if (this.seconds = t, this.nanoseconds = n, n < 0)
      throw new e(ta.INVALID_ARGUMENT, "Timestamp nanoseconds out of range: " + n);
    if (n >= 1e9)
      throw new e(ta.INVALID_ARGUMENT, "Timestamp nanoseconds out of range: " + n);
    if (t < Me)
      throw new e(ta.INVALID_ARGUMENT, "Timestamp seconds out of range: " + t);
    if (t >= 253402300800)
      throw new e(ta.INVALID_ARGUMENT, "Timestamp seconds out of range: " + t);
  }
  toDate() {
  ...
  toMillis() {
    return 1000 * this.seconds + this.nanoseconds / Ne;
  }
  toInstant() {
    if (typeof Temporal == "undefined" || !Temporal.Instant)
      throw new e(ta.FAILED_PRECONDITION, "The Temporal object is not available in the current environment.");
  ...
  static fromJSON(e2) {
    if (__PRIVATE_validateJSON(e2, Timestamp._jsonSchema))
      return new Timestamp(e2.seconds, e2.nanoseconds);
  }
  valueOf() {
    const e2 = this.seconds - Me;
...
class ByteString {
  ...
  static fromBase64String(e2) {
    const t = function __PRIVATE_decodeBase64(e3) {
      try {
        return atob(e3);
      } catch (e4) {
        throw typeof DOMException != "undefined" && e4 instanceof DOMException ? new __PRIVATE_Base64DecodeError("Invalid base64 string: " + e4) : e4;
      }
    }(e2);
    return new ByteString(t);
  }
  static fromUint8Array(e2) {
    const t = function __PRIVATE_binaryStringFromUint8Array(e3) {
      let t2 = "";
      for (let n = 0;n < e3.length; ++n)
        t2 += String.fromCharCode(e3[n]);
      return t2;
    }(e2);
    return new ByteString(t);
  }
...
function __PRIVATE_normalizeTimestamp(e2) {
  if (__PRIVATE_hardAssert(!!e2, 39018), typeof e2 == "string") {
    let t = 0;
    const n = Le.exec(e2);
    if (__PRIVATE_hardAssert(!!n, 46558, {
      ...
      let e3 = n[1];
      e3 = (e3 + "000000000").substr(0, 9), t = Number(e3);
    }
    const r = new Date(e2);
    return {
...
function __PRIVATE_normalizeNumber(e2) {
  return typeof e2 == "number" ? e2 : typeof e2 == "string" ? Number(e2) : 0;
}
function __PRIVATE_normalizeByteString(e2) {
  return typeof e2 == "string" ? ByteString.fromBase64String(e2) : ByteString.fromUint8Array(e2);
}
...
function __PRIVATE_isServerTimestamp(e2) {
  const t = (e2?.mapValue?.fields || {})[Ue]?.stringValue;
  return t === Be;
}
function __PRIVATE_getPreviousValue(e2) {
  const t = e2.mapValue.fields[ke];
  return __PRIVATE_isServerTimestamp(t) ? __PRIVATE_getPreviousValue(t) : t;
}
function __PRIVATE_getLocalWriteTime(e2) {
  const t = __PRIVATE_normalizeTimestamp(e2.mapValue.fields[qe].timestampValue);
  return new Timestamp(t.seconds, t.nanos);
}

class DatabaseInfo {
  constructor(e2, t, n, r, i, s, _, o, a, u, c, l2, E) {
    this.databaseId = e2, this.appId = t, this.persistenceKey = n, this.host = r, this.ssl = i, this.forceLongPolling = s, this.autoDetectLongPolling = _, this.longPollingOptions = o, this.useFetchStreams = a, this.isUsingEmulator = u, this.apiKey = c, this._customHeaders = l2, this.grpcFlowControlWindow = E;
  }
}
var $e = "(default)";
...
class DatabaseId {
  constructor(e2, t) {
    this.projectId = e2, this.database = t || $e;
  }
  static empty() {
...
function __PRIVATE_databaseIdFromApp(t, n) {
  if (!Object.prototype.hasOwnProperty.apply(t.options, ["projectId"]))
    throw new e(ta.INVALID_ARGUMENT, '"projectId" not provided in firebase.initializeApp.');
...
function isSafeInteger(e2) {
  return typeof e2 == "number" && Number.isInteger(e2) && !__PRIVATE_isNegativeZero(e2) && e2 <= Number.MAX_SAFE_INTEGER && e2 >= Number.MIN_SAFE_INTEGER;
}
function __PRIVATE_isString$1(e2) {
...
function __PRIVATE_valueEquals$1(e2, t, n) {
  ...
  switch (r) {
    ...
    case 9007199254740991:
      return true;
    case 1:
      return e2.booleanValue === t.booleanValue;
    case 4:
      return __PRIVATE_getLocalWriteTime(e2).isEqual(__PRIVATE_getLocalWriteTime(t));
    case 3:
      return function __PRIVATE_timestampEquals(e3, t2) {
        if (typeof e3.timestampValue == "string" && typeof t2.timestampValue == "string" && e3.timestampValue.length === t2.timestampValue.length)
          return e3.timestampValue === t2.timestampValue;
        const n2 = __PRIVATE_normalizeTimestamp(e3.timestampValue), r2 = __PRIVATE_normalizeTimestamp(t2.timestampValue);
        return n2.seconds === r2.seconds && n2.nanos === r2.nanos;
      }(e2, t);
    case 5:
      return e2.stringValue === t.stringValue;
    case 6:
      return function __PRIVATE_blobEquals(e3, t2) {
        return __PRIVATE_normalizeByteString(e3.bytesValue).isEqual(__PRIVATE_normalizeByteString(t2.bytesValue));
      }(e2, t);
    case 7:
      return e2.referenceValue === t.referenceValue;
    case 8:
      return function __PRIVATE_geoPointEquals(e3, t2) {
        return __PRIVATE_normalizeNumber(e3.geoPointValue.latitude) === __PRIVATE_normalizeNumber(t2.geoPointValue.latitude) && __PRIVATE_normalizeNumber(e3.geoPointValue.longitude) === __PRIVATE_normalizeNumber(t2.geoPointValue.longitude);
      }(e2, t);
    case 2:
      return function __PRIVATE_numberEquals(e3, t2, n2) {
        if ("integerValue" in e3 && "integerValue" in t2)
          return __PRIVATE_normalizeNumber(e3.integerValue) === __PRIVATE_normalizeNumber(t2.integerValue);
        let r2, i;
        if ("doubleValue" in e3 && "doubleValue" in t2)
          r2 = __PRIVATE_normalizeNumber(e3.doubleValue), i = __PRIVATE_normalizeNumber(t2.doubleValue);
        else {
          if (!n2?.i)
            return false;
          r2 = __PRIVATE_normalizeNumber(e3.integerValue ?? e3.doubleValue), i = __PRIVATE_normalizeNumber(t2.integerValue ?? t2.doubleValue);
        }
        if (r2 === i)
          return !!n2?.o || __PRIVATE_isNegativeZero(r2) === __PRIVATE_isNegativeZero(i);
        return !!(n2 === undefined || n2.u) && (isNaN(r2) && isNaN(i));
      }(e2, t, n);
    case 9:
      return __PRIVATE_arrayEquals(e2.arrayValue.values || [], t.arrayValue.values || [], (e3, t2) => __PRIVATE_valueEquals$1(e3, t2, n));
    case 10:
    case 11:
      return function __PRIVATE_objectEquals(e3, t2, n2) {
        const r2 = e3.mapValue.fields || {}, i = t2.mapValue.fields || {};
        if (__PRIVATE_objectSize(r2) !== __PRIVATE_objectSize(i))
          return false;
        for (const e4 in r2)
          if (r2.hasOwnProperty(e4) && (i[e4] === undefined || !__PRIVATE_valueEquals$1(r2[e4], i[e4], n2)))
            return false;
        return true;
      }(e2, t, n);
    default:
...
function __PRIVATE_arrayValueContains(e2, t) {
  return (e2.values || []).find((e3) => __PRIVATE_valueEquals$1(e3, t)) !== undefined;
}
function __PRIVATE_valueCompare(e2, t) {
  if (e2 === t)
    return 0;
  const n = __PRIVATE_typeOrder(e2), r = __PRIVATE_typeOrder(t);
  if (n !== r)
  ...
  switch (n) {
    ...
    case 9007199254740991:
      return 0;
    case 1:
      return __PRIVATE_primitiveComparator(e2.booleanValue, t.booleanValue);
    case 2:
      return function __PRIVATE_compareNumbers(e3, t2) {
        const n2 = __PRIVATE_normalizeNumber(e3.integerValue || e3.doubleValue), r2 = __PRIVATE_normalizeNumber(t2.integerValue || t2.doubleValue);
        return n2 < r2 ? -1 : n2 > r2 ? 1 : n2 === r2 ? 0 : isNaN(n2) ? isNaN(r2) ? 0 : -1 : 1;
      }(e2, t);
    case 3:
      return __PRIVATE_compareTimestamps(e2.timestampValue, t.timestampValue);
    case 4:
      return __PRIVATE_compareTimestamps(__PRIVATE_getLocalWriteTime(e2), __PRIVATE_getLocalWriteTime(t));
    case 5:
      return __PRIVATE_compareUtf8Strings(e2.stringValue, t.stringValue);
    case 6:
      return function __PRIVATE_compareBlobs(e3, t2) {
        const n2 = __PRIVATE_normalizeByteString(e3), r2 = __PRIVATE_normalizeByteString(t2);
        return n2.compareTo(r2);
      }(e2.bytesValue, t.bytesValue);
    case 7:
      return function __PRIVATE_compareReferences(e3, t2) {
        const n2 = e3.split("/"), r2 = t2.split("/");
        for (let e4 = 0;e4 < n2.length && e4 < r2.length; e4++) {
          const t3 = __PRIVATE_primitiveComparator(n2[e4], r2[e4]);
          if (t3 !== 0)
            return t3;
        }
        return __PRIVATE_primitiveComparator(n2.length, r2.length);
      }(e2.referenceValue, t.referenceValue);
    case 8:
      return function __PRIVATE_compareGeoPoints(e3, t2) {
        const n2 = __PRIVATE_primitiveComparator(__PRIVATE_normalizeNumber(e3.latitude), __PRIVATE_normalizeNumber(t2.latitude));
        if (n2 !== 0)
          return n2;
        return __PRIVATE_primitiveComparator(__PRIVATE_normalizeNumber(e3.longitude), __PRIVATE_normalizeNumber(t2.longitude));
      }(e2.geoPointValue, t.geoPointValue);
    case 9:
      return __PRIVATE_compareArrays(e2.arrayValue, t.arrayValue);
    case 10:
      return function __PRIVATE_compareVectors(e3, t2) {
        const n2 = e3.fields || {}, r2 = t2.fields || {}, i = n2[je]?.arrayValue, s = r2[je]?.arrayValue, _ = __PRIVATE_primitiveComparator(i?.values?.length || 0, s?.values?.length || 0);
        if (_ !== 0)
          return _;
        return __PRIVATE_compareArrays(i, s);
      }(e2.mapValue, t.mapValue);
    case 11:
      return function __PRIVATE_compareMaps(e3, t2) {
        if (e3 === Ge.mapValue && t2 === Ge.mapValue)
          return 0;
        if (e3 === Ge.mapValue)
          return 1;
        if (t2 === Ge.mapValue)
          return -1;
        const n2 = e3.fields || {}, r2 = Object.keys(n2), i = t2.fields || {}, s = Object.keys(i);
        r2.sort(), s.sort();
        for (let e4 = 0;e4 < r2.length && e4 < s.length; ++e4) {
          const t3 = __PRIVATE_compareUtf8Strings(r2[e4], s[e4]);
          if (t3 !== 0)
            return t3;
          const _ = __PRIVATE_valueCompare(n2[r2[e4]], i[s[e4]]);
          if (_ !== 0)
            return _;
        }
...
function __PRIVATE_compareTimestamps(e2, t) {
  if (typeof e2 == "string" && typeof t == "string" && e2.length === t.length)
    return __PRIVATE_primitiveComparator(e2, t);
  const n = __PRIVATE_normalizeTimestamp(e2), r = __PRIVATE_normalizeTimestamp(t), i = __PRIVATE_primitiveComparator(n.seconds, r.seconds);
  return i !== 0 ? i : __PRIVATE_primitiveComparator(n.nanos, r.nanos);
}
function __PRIVATE_compareArrays(e2, t) {
  const n = e2.values || [], r = t.values || [];
  for (let e3 = 0;e3 < n.length && e3 < r.length; ++e3) {
    const t2 = __PRIVATE_valueCompare(n[e3], r[e3]);
...
function __PRIVATE_canonifyValue(e2) {
  return "nullValue" in e2 ? "null" : ("booleanValue" in e2) ? "" + e2.booleanValue : ("integerValue" in e2) ? "" + e2.integerValue : ("doubleValue" in e2) ? "" + e2.doubleValue : ("timestampValue" in e2) ? function __PRIVATE_canonifyTimestamp(e3) {
    const t = __PRIVATE_normalizeTimestamp(e3);
    ...
    let t = "[", n = true;
    for (const r of e3.values || [])
      n ? n = false : t += ",", t += __PRIVATE_canonifyValue(r);
    return t + "]";
  }(e2.arrayValue) : ("mapValue" in e2) ? function __PRIVATE_canonifyMap(e3) {
    const t = Object.keys(e3.fields || {}).sort();
    let n = "{", r = true;
    for (const i of t)
      r ? r = false : n += ",", n += `${i}:${__PRIVATE_canonifyValue(e3.fields[i])}`;
    return n + "}";
  }(e2.mapValue) : l(61005, {
    value: e2
  });
...
function __PRIVATE_estimateByteSize(e2) {
  switch (__PRIVATE_typeOrder(e2)) {
    ...
    case 8:
      return 16;
    case 4:
      const t = __PRIVATE_getPreviousValue(e2);
    ...
    case 9:
      return function __PRIVATE_estimateArrayByteSize(e3) {
        return (e3.values || []).reduce((e4, t2) => e4 + __PRIVATE_estimateByteSize(t2), 0);
      }(e2.arrayValue);
    case 10:
    case 11:
      return function __PRIVATE_estimateMapByteSize(e3) {
        let t2 = 0;
        return forEach(e3.fields, (e4, n) => {
          t2 += e4.length + __PRIVATE_estimateByteSize(n);
        }), t2;
      }(e2.mapValue);
    default:
      throw l(13486, {
...
function __PRIVATE_isDouble(e2) {
  return !!e2 && "doubleValue" in e2;
}
function R(e2) {
  return isInteger(e2) || __PRIVATE_isDouble(e2);
}
function isArray(e2) {
...
function __PRIVATE_isVectorValue(e2) {
  const t = (e2?.mapValue?.fields || {})[Qe]?.stringValue;
  return t === ze;
}
function __PRIVATE_getVectorValue(e2) {
  return (e2?.mapValue?.fields || {})[je]?.arrayValue;
}
function __PRIVATE_deepClone(e2) {
  ...
  if (e2.timestampValue && typeof e2.timestampValue == "object")
    return {
      timestampValue: {
        ...e2.timestampValue
      }
  ...
  if (e2.mapValue) {
    ...
    return forEach(e2.mapValue.fields, (e3, n) => t.mapValue.fields[e3] = __PRIVATE_deepClone(n)), t;
  }
  if (e2.arrayValue) {
    const t = {
      arrayValue: {
    ...
    for (let n = 0;n < (e2.arrayValue.values || []).length; ++n)
      t.arrayValue.values[n] = __PRIVATE_deepClone(e2.arrayValue.values[n]);
    return t;
  }
  return {
    ...e2
...
function __PRIVATE_isMaxValue(e2) {
  return (((e2.mapValue || {}).fields || {}).__type__ || {}).stringValue === We;
}
var Ze = {
  mapValue: {
    fields: {
...
class ObjectValue {
  ...
  field(e2) {
    ...
    {
      let t = this.value;
      for (let n = 0;n < e2.length - 1; ++n)
        if (t = (t.mapValue.fields || {})[e2.get(n)], !__PRIVATE_isMapValue(t))
          return null;
      return t = (t.mapValue.fields || {})[e2.lastSegment()], t || null;
    }
  }
  set(e2, t) {
    this.getFieldsMap(e2.popLast())[e2.lastSegment()] = __PRIVATE_deepClone(t);
  }
  setAll(e2) {
    let t = Oe.emptyPath(), n = {}, r = [];
    e2.forEach((e3, i2) => {
      if (!t.isImmediateParentOf(i2)) {
        const e4 = this.getFieldsMap(t);
        this.applyChanges(e4, n, r), n = {}, r = [], t = i2.popLast();
      }
      e3 ? n[i2.lastSegment()] = __PRIVATE_deepClone(e3) : r.push(i2.lastSegment());
    });
    const i = this.getFieldsMap(t);
    this.applyChanges(i, n, r);
  }
  delete(e2) {
    const t = this.field(e2.popLast());
    __PRIVATE_isMapValue(t) && t.mapValue.fields && delete t.mapValue.fields[e2.lastSegment()];
  }
  isEqual(e2) {
    return __PRIVATE_valueEquals$1(this.value, e2.value);
  }
  getFieldsMap(e2) {
    let t = this.value;
    ...
    for (let n = 0;n < e2.length; ++n) {
      let r = t.mapValue.fields[e2.get(n)];
      __PRIVATE_isMapValue(r) && r.mapValue.fields || (r = {
        mapValue: {
          fields: {}
        }
      }, t.mapValue.fields[e2.get(n)] = r), t = r;
    }
    return t.mapValue.fields;
  }
  applyChanges(e2, t, n) {
    forEach(t, (t2, n2) => e2[t2] = n2);
    for (const t2 of n)
      delete e2[t2];
  }
  clone() {
...
function __PRIVATE_extractFieldMask(e2) {
  ...
  return forEach(e2.fields, (e3, n) => {
    ...
    if (__PRIVATE_isMapValue(n)) {
      const e4 = __PRIVATE_extractFieldMask(n.mapValue).fields;
      if (e4.length === 0)
        t.push(r);
      else
        for (const n2 of e4)
...
function __PRIVATE_toDouble(e2, t) {
  if (e2.useProto3Json) {
    if (isNaN(t))
      return {
        doubleValue: "NaN"
...
function toNumber(e2, t, n) {
  return isSafeInteger(t) ? __PRIVATE_toInteger(t) : __PRIVATE_toDouble(e2, t);
}
...
function __PRIVATE_applyTransformOperationToLocalView(e2, t, n) {
  return e2 instanceof __PRIVATE_ServerTimestampTransform ? function serverTimestamp$1(e3, t2) {
    const n2 = {
      fields: {
        [Ue]: {
          stringValue: Be
    ...
    return t2 && __PRIVATE_isServerTimestamp(t2) && (t2 = __PRIVATE_getPreviousValue(t2)), t2 && (n2.fields[ke] = t2), {
      mapValue: n2
    };
  }(n, t) : e2 instanceof __PRIVATE_ArrayUnionTransformOperation ? __PRIVATE_applyArrayUnionTransformOperation(e2, t) : e2 instanceof __PRIVATE_ArrayRemoveTransformOperation ? __PRIVATE_applyArrayRemoveTransformOperation(e2, t) : e2 instanceof __PRIVATE_NumericIncrementTransformOperation ? function __PRIVATE_applyNumericIncrementTransformOperationToLocalView(e3, t2) {
    const n2 = __PRIVATE_computeTransformOperationBaseValue(e3, t2), r = asNumber(n2) + asNumber(e3.h);
    return isInteger(n2) && isInteger(e3.h) ? __PRIVATE_toInteger(r) : __PRIVATE_toDouble(e3.serializer, r);
  }(e2, t) : e2 instanceof __PRIVATE_NumericMinimumTransformOperation ? function __PRIVATE_applyNumericMinimumTransformOperationToLocalView(e3, t2) {
    return __PRIVATE_applyNumericTransformOperationToLocalView(e3, t2, Math.min);
  }(e2, t) : e2 instanceof __PRIVATE_NumericMaximumTransformOperation ? function __PRIVATE_applyNumericMaximumTransformOperationToLocalView(e3, t2) {
    return __PRIVATE_applyNumericTransformOperationToLocalView(e3, t2, Math.max);
  }(e2, t) : undefined;
}
function __PRIVATE_applyTransformOperationToRemoteDocument(e2, t, n) {
  return e2 instanceof __PRIVATE_ArrayUnionTransformOperation ? __PRIVATE_applyArrayUnionTransformOperation(e2, t) : e2 instanceof __PRIVATE_ArrayRemoveTransformOperation ? __PRIVATE_applyArrayRemoveTransformOperation(e2, t) : n;
}
function __PRIVATE_computeTransformOperationBaseValue(e2, t) {
  return e2 instanceof __PRIVATE_NumericIncrementTransformOperation ? R(t) ? t : {
    integerValue: 0
  } : null;
}
...
function __PRIVATE_applyArrayUnionTransformOperation(e2, t) {
  const n = __PRIVATE_coercedFieldValuesArray(t);
  for (const t2 of e2.elements)
    n.some((e3) => __PRIVATE_valueEquals$1(e3, t2)) || n.push(t2);
  return {
    arrayValue: {
      values: n
    }
...
function __PRIVATE_applyArrayRemoveTransformOperation(e2, t) {
  let n = __PRIVATE_coercedFieldValuesArray(t);
  for (const t2 of e2.elements)
    n = n.filter((e3) => !__PRIVATE_valueEquals$1(e3, t2));
  return {
    arrayValue: {
      values: n
    }
...
class __PRIVATE_NumericTransformOperation extends TransformOperation {
  constructor(e2, t) {
    super(), this.serializer = e2, this.h = t;
  }
}
...
function __PRIVATE_applyNumericTransformOperationToLocalView(e2, t, n) {
  if (!R(t))
    return e2.h;
  const r = n(asNumber(t), asNumber(e2.h));
  return isInteger(t) && isInteger(e2.h) ? __PRIVATE_toInteger(r) : __PRIVATE_toDouble(e2.serializer, r);
}
...
function __PRIVATE_fieldTransformEquals(e2, t) {
  return e2.field.isEqual(t.field) && function __PRIVATE_transformOperationEquals(e3, t2) {
    return e3 instanceof __PRIVATE_ArrayUnionTransformOperation && t2 instanceof __PRIVATE_ArrayUnionTransformOperation || e3 instanceof __PRIVATE_ArrayRemoveTransformOperation && t2 instanceof __PRIVATE_ArrayRemoveTransformOperation ? __PRIVATE_arrayEquals(e3.elements, t2.elements, __PRIVATE_valueEquals$1) : e3 instanceof __PRIVATE_NumericIncrementTransformOperation && t2 instanceof __PRIVATE_NumericIncrementTransformOperation || e3 instanceof __PRIVATE_NumericMinimumTransformOperation && t2 instanceof __PRIVATE_NumericMinimumTransformOperation || e3 instanceof __PRIVATE_NumericMaximumTransformOperation && t2 instanceof __PRIVATE_NumericMaximumTransformOperation ? __PRIVATE_valueEquals$1(e3.h, t2.h) : e3 instanceof __PRIVATE_ServerTimestampTransform && t2 instanceof __PRIVATE_ServerTimestampTransform;
  }(e2.transform, t.transform);
}

class MutationResult {
  constructor(e2, t) {
    this.version = e2, this.transformResults = t;
  }
}

class Precondition {
  constructor(e2, t) {
    this.updateTime = e2, this.exists = t;
  }
  static none() {
    return new Precondition;
  }
  static exists(e2) {
    return new Precondition(undefined, e2);
  }
...
function __PRIVATE_preconditionIsValidForDocument(e2, t) {
  return e2.updateTime !== undefined ? t.isFoundDocument() && t.version.isEqual(e2.updateTime) : e2.exists === undefined || e2.exists === t.isFoundDocument();
}

class Mutation {
}
function __PRIVATE_calculateOverlayMutation(e2, t) {
  if (!e2.hasLocalMutations || t && t.fields.length === 0)
    return null;
  if (t === null)
    return e2.isNoDocument() ? new __PRIVATE_DeleteMutation(e2.key, Precondition.none()) : new __PRIVATE_SetMutation(e2.key, e2.data, Precondition.none());
  {
    const n = e2.data, r = ObjectValue.empty();
    let i = new SortedSet(Oe.comparator);
    for (let e3 of t.fields)
      if (!i.has(e3)) {
        let t2 = n.field(e3);
        t2 === null && e3.length > 1 && (e3 = e3.popLast(), t2 = n.field(e3)), t2 === null ? r.delete(e3) : r.set(e3, t2), i = i.add(e3);
      }
    return new __PRIVATE_PatchMutation(e2.key, r, new FieldMask(i.toArray()), Precondition.none());
  }
}
function __PRIVATE_mutationApplyToRemoteDocument(e2, t, n) {
  e2 instanceof __PRIVATE_SetMutation ? function __PRIVATE_setMutationApplyToRemoteDocument(e3, t2, n2) {
    const r = e3.value.clone(), i = __PRIVATE_serverTransformResults(e3.fieldTransforms, t2, n2.transformResults);
    r.setAll(i), t2.convertToFoundDocument(n2.version, r).setHasCommittedMutations();
  }(e2, t, n) : e2 instanceof __PRIVATE_PatchMutation ? function __PRIVATE_patchMutationApplyToRemoteDocument(e3, t2, n2) {
    if (!__PRIVATE_preconditionIsValidForDocument(e3.precondition, t2))
      return void t2.convertToUnknownDocument(n2.version);
    const r = __PRIVATE_serverTransformResults(e3.fieldTransforms, t2, n2.transformResults), i = t2.data;
    i.setAll(__PRIVATE_getPatch(e3)), i.setAll(r), t2.convertToFoundDocument(n2.version, i).setHasCommittedMutations();
  }(e2, t, n) : function __PRIVATE_deleteMutationApplyToRemoteDocument(e3, t2, n2) {
    t2.convertToNoDocument(n2.version).setHasCommittedMutations();
  }(0, t, n);
}
function __PRIVATE_mutationApplyToLocalView(e2, t, n, r) {
  return e2 instanceof __PRIVATE_SetMutation ? function __PRIVATE_setMutationApplyToLocalView(e3, t2, n2, r2) {
    if (!__PRIVATE_preconditionIsValidForDocument(e3.precondition, t2))
      return n2;
    const i = e3.value.clone(), s = __PRIVATE_localTransformResults(e3.fieldTransforms, r2, t2);
    return i.setAll(s), t2.convertToFoundDocument(t2.version, i).setHasLocalMutations(), null;
  }(e2, t, n, r) : e2 instanceof __PRIVATE_PatchMutation ? function __PRIVATE_patchMutationApplyToLocalView(e3, t2, n2, r2) {
    if (!__PRIVATE_preconditionIsValidForDocument(e3.precondition, t2))
      return n2;
    const i = __PRIVATE_localTransformResults(e3.fieldTransforms, r2, t2), s = t2.data;
    if (s.setAll(__PRIVATE_getPatch(e3)), s.setAll(i), t2.convertToFoundDocument(t2.version, s).setHasLocalMutations(), n2 === null)
      return null;
    return n2.unionWith(e3.fieldMask.fields).unionWith(e3.fieldTransforms.map((e4) => e4.field));
  }(e2, t, n, r) : function __PRIVATE_deleteMutationApplyToLocalView(e3, t2, n2) {
    if (__PRIVATE_preconditionIsValidForDocument(e3.precondition, t2))
      return t2.convertToNoDocument(t2.version).setHasLocalMutations(), null;
    return n2;
  }(e2, t, n);
}
function __PRIVATE_mutationExtractBaseValue(e2, t) {
  let n = null;
  for (const r of e2.fieldTransforms) {
    const e3 = t.data.field(r.field), i = __PRIVATE_computeTransformOperationBaseValue(r.transform, e3 || null);
    i != null && (n === null && (n = ObjectValue.empty()), n.set(r.field, i));
  }
  return n || null;
}
function __PRIVATE_mutationEquals(e2, t) {
  return e2.type === t.type && (!!e2.key.isEqual(t.key) && (!!e2.precondition.isEqual(t.precondition) && (!!function __PRIVATE_fieldTransformsAreEqual(e3, t2) {
    return e3 === undefined && t2 === undefined || !(!e3 || !t2) && __PRIVATE_arrayEquals(e3, t2, (e4, t3) => __PRIVATE_fieldTransformEquals(e4, t3));
  }(e2.fieldTransforms, t.fieldTransforms) && (e2.type === 0 ? e2.value.isEqual(t.value) : e2.type !== 1 || e2.data.isEqual(t.data) && e2.fieldMask.isEqual(t.fieldMask)))));
}

class __PRIVATE_SetMutation extends Mutation {
  constructor(e2, t, n, r = []) {
    super(), this.key = e2, this.value = t, this.precondition = n, this.fieldTransforms = r, this.type = 0;
  }
  getFieldMask() {
    return null;
  }
...
class __PRIVATE_PatchMutation extends Mutation {
  constructor(e2, t, n, r, i = []) {
    super(), this.key = e2, this.data = t, this.fieldMask = n, this.precondition = r, this.fieldTransforms = i, this.type = 1;
  }
  getFieldMask() {
...
function __PRIVATE_getPatch(e2) {
  const t = new Map;
  return e2.fieldMask.fields.forEach((n) => {
    if (!n.isEmpty()) {
      const r = e2.data.field(n);
      t.set(n, r);
    }
  }), t;
}
function __PRIVATE_serverTransformResults(e2, t, n) {
  const r = new Map;
  __PRIVATE_hardAssert(e2.length === n.length, 32656, {
    T: n.length,
    P: e2.length
  });
  for (let i = 0;i < n.length; i++) {
    const s = e2[i], _ = s.transform, o = t.data.field(s.field);
    r.set(s.field, __PRIVATE_applyTransformOperationToRemoteDocument(_, o, n[i]));
  }
  return r;
}
function __PRIVATE_localTransformResults(e2, t, n) {
  const r = new Map;
  for (const i of e2) {
    const e3 = i.transform, s = n.data.field(i.field);
    r.set(i.field, __PRIVATE_applyTransformOperationToLocalView(e3, s, t));
  }
  return r;
}
...
class __PRIVATE_DeleteMutation extends Mutation {
  constructor(e2, t) {
    super(), this.key = e2, this.precondition = t, this.type = 2, this.fieldTransforms = [];
  }
  getFieldMask() {
    return null;
  }
...
class __PRIVATE_VerifyMutation extends Mutation {
  constructor(e2, t) {
    super(), this.key = e2, this.precondition = t, this.type = 3, this.fieldTransforms = [];
  }
  getFieldMask() {
    return null;
  }
...
class Bound {
  constructor(e2, t) {
    this.position = e2, this.inclusive = t;
  }
}
function __PRIVATE_boundCompareToDocument(e2, t, n) {
  let r = 0;
  for (let i = 0;i < e2.position.length; i++) {
    const s = t[i], _ = e2.position[i];
    if (s.field.isKeyField())
      r = DocumentKey.comparator(DocumentKey.fromName(_.referenceValue), n.key);
    else {
...
function __PRIVATE_boundEquals(e2, t) {
  if (e2 === null)
    return t === null;
  if (t === null)
    return false;
  if (e2.inclusive !== t.inclusive || e2.position.length !== t.position.length)
    return false;
  for (let n = 0;n < e2.position.length; n++) {
    if (!__PRIVATE_valueEquals$1(e2.position[n], t.position[n]))
      return false;
  }
  return true;
}
...
class FieldFilter extends Filter {
  constructor(e2, t, n) {
    super(), this.field = e2, this.op = t, this.value = n;
  }
  static create(e2, t, n) {
    return e2.isKeyField() ? t === "in" || t === "not-in" ? this.createKeyFieldInFilter(e2, t, n) : new __PRIVATE_KeyFieldFilter(e2, t, n) : t === "array-contains" ? new __PRIVATE_ArrayContainsFilter(e2, n) : t === "in" ? new __PRIVATE_InFilter(e2, n) : t === "not-in" ? new __PRIVATE_NotInFilter(e2, n) : t === "array-contains-any" ? new __PRIVATE_ArrayContainsAnyFilter(e2, n) : new FieldFilter(e2, t, n);
  }
  static createKeyFieldInFilter(e2, t, n) {
    return t === "in" ? new __PRIVATE_KeyFieldInFilter(e2, n) : new __PRIVATE_KeyFieldNotInFilter(e2, n);
  }
  matches(e2) {
    const t = e2.data.field(this.field);
...
class CompositeFilter extends Filter {
  constructor(e2, t) {
    super(), this.filters = e2, this.op = t, this.I = null;
  }
  static create(e2, t) {
    return new CompositeFilter(e2, t);
  }
  matches(e2) {
    return __PRIVATE_compositeFilterIsConjunction(this) ? this.filters.find((t) => !t.matches(e2)) === undefined : this.filters.find((t) => t.matches(e2)) !== undefined;
  }
  getFlattenedFilters() {
    return this.I !== null || (this.I = this.filters.reduce((e2, t) => e2.concat(t.getFlattenedFilters()), [])), this.I;
  }
  getFilters() {
...
function __PRIVATE_canonifyFilter(e2) {
  ...
  {
    const t = e2.filters.map((e3) => __PRIVATE_canonifyFilter(e3)).join(",");
    return `${e2.op}(${t})`;
  }
}
function __PRIVATE_filterEquals(e2, t) {
  return e2 instanceof FieldFilter ? function __PRIVATE_fieldFilterEquals(e3, t2) {
    return t2 instanceof FieldFilter && e3.op === t2.op && e3.field.isEqual(t2.field) && __PRIVATE_valueEquals$1(e3.value, t2.value);
  }(e2, t) : e2 instanceof CompositeFilter ? function __PRIVATE_compositeFilterEquals(e3, t2) {
    if (t2 instanceof CompositeFilter && e3.op === t2.op && e3.filters.length === t2.filters.length) {
      return e3.filters.reduce((e4, n, r) => e4 && __PRIVATE_filterEquals(n, t2.filters[r]), true);
    }
    return false;
  }(e2, t) : void l(19439);
}
...
class __PRIVATE_KeyFieldFilter extends FieldFilter {
  constructor(e2, t, n) {
    super(e2, t, n), this.key = DocumentKey.fromName(n.referenceValue);
  }
  matches(e2) {
    const t = DocumentKey.comparator(e2.key, this.key);
...
class __PRIVATE_KeyFieldInFilter extends FieldFilter {
  constructor(e2, t) {
    super(e2, "in", t), this.keys = __PRIVATE_extractDocumentKeysFromArrayValue("in", t);
  }
  matches(e2) {
    return this.keys.some((t) => t.isEqual(e2.key));
  }
}

class __PRIVATE_KeyFieldNotInFilter extends FieldFilter {
  constructor(e2, t) {
    super(e2, "not-in", t), this.keys = __PRIVATE_extractDocumentKeysFromArrayValue("not-in", t);
  }
  matches(e2) {
    return !this.keys.some((t) => t.isEqual(e2.key));
  }
}
function __PRIVATE_extractDocumentKeysFromArrayValue(e2, t) {
  return (t.arrayValue?.values || []).map((e3) => DocumentKey.fromName(e3.referenceValue));
}

class __PRIVATE_ArrayContainsFilter extends FieldFilter {
  constructor(e2, t) {
    super(e2, "array-contains", t);
  }
  matches(e2) {
    const t = e2.data.field(this.field);
...
class __PRIVATE_ArrayContainsAnyFilter extends FieldFilter {
  constructor(e2, t) {
    super(e2, "array-contains-any", t);
  }
  matches(e2) {
    const t = e2.data.field(this.field);
    return !(!isArray(t) || !t.arrayValue.values) && t.arrayValue.values.some((e3) => __PRIVATE_arrayValueContains(this.value.arrayValue, e3));
  }
}

class OrderBy {
  constructor(e2, t = "asc") {
    this.field = e2, this.dir = t;
  }
}
function __PRIVATE_orderByEquals(e2, t) {
  return e2.dir === t.dir && e2.field.isEqual(t.field);
}
...
class MutableDocument {
  constructor(e2, t, n, r, i, s, _) {
    this.key = e2, this.documentType = t, this.version = n, this.readTime = r, this.createTime = i, this.data = s, this.documentState = _;
  }
  static newInvalidDocument(e2) {
    return new MutableDocument(e2, 0, SnapshotVersion.min(), SnapshotVersion.min(), SnapshotVersion.min(), ObjectValue.empty(), 0);
  }
  static newFoundDocument(e2, t, n, r) {
    return new MutableDocument(e2, 1, t, SnapshotVersion.min(), n, r, 0);
  }
  static newNoDocument(e2, t) {
    return new MutableDocument(e2, 2, t, SnapshotVersion.min(), SnapshotVersion.min(), ObjectValue.empty(), 0);
  }
  static newUnknownDocument(e2, t) {
    return new MutableDocument(e2, 3, t, SnapshotVersion.min(), SnapshotVersion.min(), ObjectValue.empty(), 2);
  }
  convertToFoundDocument(e2, t) {
    return !this.createTime.isEqual(SnapshotVersion.min()) || this.documentType !== 2 && this.documentType !== 0 || (this.createTime = e2), this.version = e2, this.documentType = 1, this.data = t, this.documentState = 0, this;
  }
...
class FieldIndex {
  constructor(e2, t, n, r) {
    this.indexId = e2, this.collectionGroup = t, this.fields = n, this.indexState = r;
  }
}
FieldIndex.UNKNOWN_ID = -1;
function __PRIVATE_newIndexOffsetSuccessorFromReadTime(e2, t) {
  const n = e2.toTimestamp().seconds, r = e2.toTimestamp().nanoseconds + 1, i = SnapshotVersion.fromTimestamp(r === 1e9 ? new Timestamp(n + 1, 0) : new Timestamp(n, r));
  return new IndexOffset(i, DocumentKey.empty(), t);
}
function __PRIVATE_newIndexOffsetFromDocument(e2) {
  return new IndexOffset(e2.readTime, e2.key, Xe);
}

class IndexOffset {
  constructor(e2, t, n) {
    this.readTime = e2, this.documentKey = t, this.largestBatchId = n;
  }
  static min() {
...
function __PRIVATE_indexOffsetComparator(e2, t) {
  let n = e2.readTime.compareTo(t.readTime);
  return n !== 0 ? n : (n = DocumentKey.comparator(e2.documentKey, t.documentKey), n !== 0 ? n : __PRIVATE_primitiveComparator(e2.largestBatchId, t.largestBatchId));
}

class __PRIVATE_TargetImpl {
  constructor(e2, t = null, n = [], r = [], i = null, s = null, _ = null) {
    this.path = e2, this.collectionGroup = t, this.orderBy = n, this.filters = r, this.limit = i, this.startAt = s, this.endAt = _, this.R = null;
  }
}
function __PRIVATE_newTarget(e2, t = null, n = [], r = [], i = null, s = null, _ = null) {
  return new __PRIVATE_TargetImpl(e2, t, n, r, i, s, _);
}
function __PRIVATE_canonifyTarget(e2) {
  const t = __PRIVATE_debugCast(e2);
  if (t.R === null) {
    let e3 = t.path.canonicalString();
    t.collectionGroup !== null && (e3 += "|cg:" + t.collectionGroup), e3 += "|f:", e3 += t.filters.map((e4) => __PRIVATE_canonifyFilter(e4)).join(","), e3 += "|ob:", e3 += t.orderBy.map((e4) => function __PRIVATE_canonifyOrderBy(e5) {
      return e5.field.canonicalString() + e5.dir;
    }(e4)).join(","), __PRIVATE_isNullOrUndefined(t.limit) || (e3 += "|l:", e3 += t.limit), t.startAt && (e3 += "|lb:", e3 += t.startAt.inclusive ? "b:" : "a:", e3 += t.startAt.position.map((e4) => canonicalId(e4)).join(",")), t.endAt && (e3 += "|ub:", e3 += t.endAt.inclusive ? "a:" : "b:", e3 += t.endAt.position.map((e4) => canonicalId(e4)).join(",")), t.R = e3;
  }
  return t.R;
}
function __PRIVATE_targetEquals(e2, t) {
  if (e2.limit !== t.limit)
    return false;
  if (e2.orderBy.length !== t.orderBy.length)
    return false;
  for (let n = 0;n < e2.orderBy.length; n++)
    if (!__PRIVATE_orderByEquals(e2.orderBy[n], t.orderBy[n]))
      return false;
  if (e2.filters.length !== t.filters.length)
    return false;
  for (let n = 0;n < e2.filters.length; n++)
    if (!__PRIVATE_filterEquals(e2.filters[n], t.filters[n]))
      return false;
  return e2.collectionGroup === t.collectionGroup && (!!e2.path.isEqual(t.path) && (!!__PRIVATE_boundEquals(e2.startAt, t.startAt) && __PRIVATE_boundEquals(e2.endAt, t.endAt)));
}
function __PRIVATE_targetIsPipelineTarget(e2) {
  return !!e2.isCorePipeline;
}
function __PRIVATE_targetIsDocumentTarget(e2) {
  return !!e2.path && DocumentKey.isDocumentKey(e2.path) && e2.collectionGroup === null && e2.filters.length === 0;
}
class __PRIVATE_QueryImpl {
  constructor(e2, t = null, n = [], r = [], i = null, s = "F", _ = null, o = null) {
    this.path = e2, this.collectionGroup = t, this.explicitOrderBy = n, this.filters = r, this.limit = i, this.limitType = s, this.startAt = _, this.endAt = o, this.A = null, this.V = null, this.m = null, this.startAt, this.endAt;
  }
}
function __PRIVATE_newQuery(e2, t, n, r, i, s, _, o) {
  return new __PRIVATE_QueryImpl(e2, t, n, r, i, s, _, o);
}
function __PRIVATE_newQueryForPath(e2) {
  return new __PRIVATE_QueryImpl(e2);
}
function __PRIVATE_queryMatchesAllDocuments(e2) {
  return e2.filters.length === 0 && e2.limit === null && e2.startAt == null && e2.endAt == null && (e2.explicitOrderBy.length === 0 || e2.explicitOrderBy.length === 1 && e2.explicitOrderBy[0].field.isKeyField());
}
function __PRIVATE_isDocumentQuery$1(e2) {
...
function __PRIVATE_isCollectionGroupQuery(e2) {
  return e2.collectionGroup !== null;
}
function __PRIVATE_queryNormalizedOrderBy(e2) {
  ...
  if (t.A === null) {
    ...
    for (const n2 of t.explicitOrderBy)
      t.A.push(n2), e3.add(n2.field.canonicalString());
    const n = t.explicitOrderBy.length > 0 ? t.explicitOrderBy[t.explicitOrderBy.length - 1].dir : "asc", r = function __PRIVATE_getInequalityFilterFields(e4) {
      let t2 = new SortedSet(Oe.comparator);
      return e4.filters.forEach((e5) => {
        e5.getFlattenedFilters().forEach((e6) => {
          e6.isInequality() && (t2 = t2.add(e6.field));
        });
      }), t2;
    }(t);
    r.forEach((r2) => {
      e3.has(r2.canonicalString()) || r2.isKeyField() || t.A.push(new OrderBy(r2, n));
    }), e3.has(Oe.keyField().canonicalString()) || t.A.push(new OrderBy(Oe.keyField(), n));
  }
  return t.A;
}
function __PRIVATE_queryToTarget(e2) {
  const t = __PRIVATE_debugCast(e2);
  return t.V || (t.V = __PRIVATE__queryToTarget(t, __PRIVATE_queryNormalizedOrderBy(e2))), t.V;
}
function __PRIVATE__queryToTarget(e2, t) {
  if (e2.limitType === "F")
    return __PRIVATE_newTarget(e2.path, e2.collectionGroup, t, e2.filters, e2.limit, e2.startAt, e2.endAt);
  {
    t = t.map((e3) => {
      const t2 = e3.dir === "desc" ? "asc" : "desc";
      return new OrderBy(e3.field, t2);
    });
    const n = e2.endAt ? new Bound(e2.endAt.position, e2.endAt.inclusive) : null, r = e2.startAt ? new Bound(e2.startAt.position, e2.startAt.inclusive) : null;
    return __PRIVATE_newTarget(e2.path, e2.collectionGroup, t, e2.filters, e2.limit, n, r);
  }
}
function __PRIVATE_queryWithLimit(e2, t, n) {
  return new __PRIVATE_QueryImpl(e2.path, e2.collectionGroup, e2.explicitOrderBy.slice(), e2.filters.slice(), t, n, e2.startAt, e2.endAt);
}
function __PRIVATE_queryEquals(e2, t) {
  return __PRIVATE_targetEquals(__PRIVATE_queryToTarget(e2), __PRIVATE_queryToTarget(t)) && e2.limitType === t.limitType;
}
function __PRIVATE_stringifyQuery(e2) {
  return `Query(target=${function __PRIVATE_stringifyTarget(e3) {
    let t = e3.path.canonicalString();
    return e3.collectionGroup !== null && (t += " collectionGroup=" + e3.collectionGroup), e3.filters.length > 0 && (t += `, filters: [${e3.filters.map((e4) => __PRIVATE_stringifyFilter(e4)).join(", ")}]`), __PRIVATE_isNullOrUndefined(e3.limit) || (t += ", limit: " + e3.limit), e3.orderBy.length > 0 && (t += `, orderBy: [${e3.orderBy.map((e4) => function __PRIVATE_stringifyOrderBy(e5) {
      return `${e5.field.canonicalString()} (${e5.dir})`;
    }(e4)).join(", ")}]`), e3.startAt && (t += ", startAt: ", t += e3.startAt.inclusive ? "b:" : "a:", t += e3.startAt.position.map((e4) => canonicalId(e4)).join(",")), e3.endAt && (t += ", endAt: ", t += e3.endAt.inclusive ? "a:" : "b:", t += e3.endAt.position.map((e4) => canonicalId(e4)).join(",")), `Target(${t})`;
  }(__PRIVATE_queryToTarget(e2))}; limitType=${e2.limitType})`;
}
function __PRIVATE_queryMatches(e2, t) {
  return t.isFoundDocument() && function __PRIVATE_queryMatchesPathAndCollectionGroup(e3, t2) {
    const n = t2.key.path;
    return e3.collectionGroup !== null ? t2.key.hasCollectionId(e3.collectionGroup) && e3.path.isPrefixOf(n) : DocumentKey.isDocumentKey(e3.path) ? e3.path.isEqual(n) : e3.path.isImmediateParentOf(n);
  }(e2, t) && function __PRIVATE_queryMatchesOrderBy(e3, t2) {
    for (const n of __PRIVATE_queryNormalizedOrderBy(e3))
      if (!n.field.isKeyField() && t2.data.field(n.field) === null)
        return false;
    return true;
  }(e2, t) && function __PRIVATE_queryMatchesFilters(e3, t2) {
    for (const n of e3.filters)
      if (!n.matches(t2))
        return false;
    return true;
  }(e2, t) && function __PRIVATE_queryMatchesBounds(e3, t2) {
    if (e3.startAt && !function __PRIVATE_boundSortsBeforeDocument(e4, t3, n) {
      const r = __PRIVATE_boundCompareToDocument(e4, t3, n);
      return e4.inclusive ? r <= 0 : r < 0;
    }(e3.startAt, __PRIVATE_queryNormalizedOrderBy(e3), t2))
      return false;
    if (e3.endAt && !function __PRIVATE_boundSortsAfterDocument(e4, t3, n) {
      const r = __PRIVATE_boundCompareToDocument(e4, t3, n);
      return e4.inclusive ? r >= 0 : r > 0;
    }(e3.endAt, __PRIVATE_queryNormalizedOrderBy(e3), t2))
      return false;
    return true;
  }(e2, t);
}
function __PRIVATE_newQueryComparator(e2) {
  return (t, n) => {
    let r = false;
    for (const i of __PRIVATE_queryNormalizedOrderBy(e2)) {
      const e3 = __PRIVATE_compareDocs(i, t, n);
      if (e3 !== 0)
        return e3;
...
function __PRIVATE_compareDocs(e2, t, n) {
  const r = e2.field.isKeyField() ? DocumentKey.comparator(t.key, n.key) : function __PRIVATE_compareDocumentsByField(e3, t2, n2) {
    const r2 = t2.data.field(e3), i = n2.data.field(e3);
    return r2 !== null && i !== null ? __PRIVATE_valueCompare(r2, i) : l(42886);
  }(e2.field, t, n);
  switch (e2.dir) {
    case "asc":
      return r;
...
class ExistenceFilter {
  constructor(e2, t) {
    this.count = e2, this.unchangedNames = t;
  }
}
var et;
var tt;
...
function __PRIVATE_mapCodeFromRpcCode(e2) {
  if (e2 === undefined)
    return __PRIVATE_logError("GRPC error has no .code"), ta.UNKNOWN;
  switch (e2) {
    case et.OK:
      return ta.OK;
    case et.CANCELLED:
    ...
    case et.DEADLINE_EXCEEDED:
      return ta.DEADLINE_EXCEEDED;
    case et.RESOURCE_EXHAUSTED:
      return ta.RESOURCE_EXHAUSTED;
    case et.INTERNAL:
      return ta.INTERNAL;
    case et.UNAVAILABLE:
      return ta.UNAVAILABLE;
    case et.UNAUTHENTICATED:
      return ta.UNAUTHENTICATED;
    case et.INVALID_ARGUMENT:
      return ta.INVALID_ARGUMENT;
    case et.NOT_FOUND:
      return ta.NOT_FOUND;
    case et.ALREADY_EXISTS:
      return ta.ALREADY_EXISTS;
    case et.PERMISSION_DENIED:
      return ta.PERMISSION_DENIED;
    case et.FAILED_PRECONDITION:
      return ta.FAILED_PRECONDITION;
    case et.ABORTED:
      return ta.ABORTED;
    case et.OUT_OF_RANGE:
      return ta.OUT_OF_RANGE;
    case et.UNIMPLEMENTED:
      return ta.UNIMPLEMENTED;
    case et.DATA_LOSS:
      return ta.DATA_LOSS;
    default:
      return l(39323, {
...
class ObjectMap {
  constructor(e2, t) {
    this.mapKeyFn = e2, this.equalsFn = t, this.inner = {}, this.innerSize = 0;
  }
  get(e2) {
    const t = this.mapKeyFn(e2), n = this.inner[t];
    if (n !== undefined) {
      for (const [t2, r] of n)
        if (this.equalsFn(t2, e2))
          return r;
    }
  }
  has(e2) {
    return this.get(e2) !== undefined;
  }
  set(e2, t) {
    const n = this.mapKeyFn(e2), r = this.inner[n];
    if (r === undefined)
      return this.inner[n] = [[e2, t]], void this.innerSize++;
    for (let n2 = 0;n2 < r.length; n2++)
      if (this.equalsFn(r[n2][0], e2))
        return void (r[n2] = [e2, t]);
    r.push([e2, t]), this.innerSize++;
  }
  delete(e2) {
    const t = this.mapKeyFn(e2), n = this.inner[t];
    if (n === undefined)
...
function __PRIVATE_convertOverlayedDocumentMapToDocumentMap(e2) {
  let t = rt;
  return e2.forEach((e3, n) => t = t.insert(e3, n.overlayedDocument)), t;
}
...
function __PRIVATE_newDocumentKeyMap() {
  return new ObjectMap((e2) => e2.toString(), (e2, t) => e2.isEqual(t));
}
...
function __PRIVATE_getMd5HashValue(e2) {
  const t = __PRIVATE_newTextEncoder().encode(e2), n = new Md5;
  return n.update(t), new Uint8Array(n.digest());
}
function __PRIVATE_get64BitUints(e2) {
  const t = new DataView(e2.buffer), n = t.getUint32(0, true), r = t.getUint32(4, true), i = t.getUint32(8, true), s = t.getUint32(12, true);
  return [new Integer([n, r], 0), new Integer([i, s], 0)];
}

class BloomFilter {
  constructor(e2, t, n) {
    if (this.bitmap = e2, this.padding = t, this.hashCount = n, t < 0 || t >= 8)
      throw new __PRIVATE_BloomFilterError(`Invalid padding: ${t}`);
    if (n < 0)
      throw new __PRIVATE_BloomFilterError(`Invalid hash count: ${n}`);
    if (e2.length > 0 && this.hashCount === 0)
      throw new __PRIVATE_BloomFilterError(`Invalid hash count: ${n}`);
    if (e2.length === 0 && t !== 0)
      throw new __PRIVATE_BloomFilterError(`Invalid padding when bitmap length is 0: ${t}`);
    this.p = 8 * e2.length - t, this.S = Integer.fromNumber(this.p);
  }
  v(e2, t, n) {
    let r = e2.add(t.multiply(Integer.fromNumber(n)));
    return r.compare(at) === 1 && (r = new Integer([r.getBits(0), r.getBits(1)], 0)), r.modulo(this.S).toNumber();
  }
  D(e2) {
  ...
  mightContain(e2) {
    if (this.p === 0)
      return false;
    const t = __PRIVATE_getMd5HashValue(e2), [n, r] = __PRIVATE_get64BitUints(t);
    for (let e3 = 0;e3 < this.hashCount; e3++) {
      const t2 = this.v(n, r, e3);
      if (!this.D(t2))
        return false;
    }
    return true;
  ...
  static create(e2, t, n) {
    const r = e2 % 8 == 0 ? 0 : 8 - e2 % 8, i = new Uint8Array(Math.ceil(e2 / 8)), s = new BloomFilter(i, r, t);
    return n.forEach((e3) => s.insert(e3)), s;
  }
  insert(e2) {
    if (this.p === 0)
      return;
  ...
  C(e2) {
    const t = Math.floor(e2 / 8), n = e2 % 8;
    this.bitmap[t] |= 1 << n;
  }
}
...
class RemoteEvent {
  constructor(e2, t, n, r, i, s) {
    this.snapshotVersion = e2, this.targetChanges = t, this.targetMismatches = n, this.documentUpdates = r, this.augmentedDocumentUpdates = i, this.resolvedLimboDocuments = s;
  }
  static createSynthesizedRemoteEventForCurrentChange(e2, t, n) {
    const r = new Map;
    return r.set(e2, TargetChange.createSynthesizedTargetChangeForCurrentChange(e2, t, n)), new RemoteEvent(SnapshotVersion.min(), r, new SortedMap(__PRIVATE_primitiveComparator), __PRIVATE_mutableDocumentMap(), __PRIVATE_mutableDocumentMap(), __PRIVATE_documentKeySet());
  }
}

class TargetChange {
  constructor(e2, t, n, r, i) {
    this.resumeToken = e2, this.current = t, this.addedDocuments = n, this.modifiedDocuments = r, this.removedDocuments = i;
  }
  static createSynthesizedTargetChangeForCurrentChange(e2, t, n) {
...
class __PRIVATE_DocumentWatchChange {
  constructor(e2, t, n, r) {
    this.F = e2, this.removedTargetIds = t, this.key = n, this.O = r;
  }
}

class __PRIVATE_ExistenceFilterChange {
  constructor(e2, t) {
    this.targetId = e2, this.M = t;
  }
}

class __PRIVATE_WatchTargetChange {
  constructor(e2, t, n = ByteString.EMPTY_BYTE_STRING, r = null) {
    this.state = e2, this.targetIds = t, this.resumeToken = n, this.cause = r;
  }
}

class __PRIVATE_TargetState {
  constructor(e2) {
    this.targetId = e2, this.N = 0, this.L = __PRIVATE_snapshotChangesMap(), this.B = ByteString.EMPTY_BYTE_STRING, this.U = false, this.k = true;
  }
  get current() {
    return this.U;
  }
  ...
  K(e2) {
    e2.approximateByteSize() > 0 && (this.k = true, this.B = e2);
  }
  W() {
    let e2 = __PRIVATE_documentKeySet(), t = __PRIVATE_documentKeySet(), n = __PRIVATE_documentKeySet();
    ...
    }), new TargetChange(this.B, this.U, e2, t, n);
  }
  G() {
    this.k = false, this.L = __PRIVATE_snapshotChangesMap();
  }
  j(e2, t) {
    this.k = true, this.L = this.L.insert(e2, t);
  }
  H(e2) {
    this.k = true, this.L = this.L.remove(e2);
  }
  J() {
    this.N += 1;
  }
...
class __PRIVATE_WatchChangeAggregator {
  constructor(e2) {
    this.X = e2, this.ee = new Map, this.te = __PRIVATE_mutableDocumentMap(), this.ne = __PRIVATE_documentTargetMap(), this.re = __PRIVATE_mutableDocumentMap(), this.ie = __PRIVATE_documentTargetMap(), this.se = new SortedMap(__PRIVATE_primitiveComparator);
  }
  _e(e2) {
    for (const t of e2.F)
      e2.O && e2.O.isFoundDocument() ? this.oe(t, e2.O) : this.ae(t, e2.key, e2.O);
    for (const t of e2.removedTargetIds)
      this.ae(t, e2.key, e2.O);
  }
  ue(e2) {
    this.forEachTarget(e2, (t) => {
      const n = this.ee.get(t);
      if (n)
        switch (e2.state) {
          case 0:
            this.ce(t) && n.K(e2.resumeToken);
            break;
          case 1:
            n.Y(), n.q || n.G(), n.K(e2.resumeToken);
            break;
          case 2:
            n.Y(), n.q || this.removeTarget(t);
            break;
          case 3:
            this.ce(t) && (n.Z(), n.K(e2.resumeToken));
            break;
          case 4:
            this.ce(t) && (this.le(t), n.K(e2.resumeToken));
            break;
          default:
            l(56790, {
              state: e2.state
        ...
        __PRIVATE_logDebug(ut, `handleTargetChange received targetChange for untracked target ID (${t}) with state (${e2.state})`);
    });
  }
  forEachTarget(e2, t) {
    e2.targetIds.length > 0 ? e2.targetIds.forEach(t) : this.ee.forEach((e3, n) => {
      this.ce(n) && t(n);
    });
  }
  Ee(e2) {
    return __PRIVATE_targetIsPipelineTarget(e2) ? e2.getPipelineSourceType() === "documents" && e2.getPipelineDocuments()?.length === 1 : __PRIVATE_targetIsDocumentTarget(e2);
  }
  he(e2) {
    const t = e2.targetId, n = e2.M.count, r = this.Te(t);
    if (r) {
      const i = r.target;
      if (this.Ee(i))
        if (n === 0) {
          const e3 = new DocumentKey(__PRIVATE_targetIsPipelineTarget(i) ? ResourcePath.fromString(i.getPipelineDocuments()[0]) : i.path);
          this.ae(t, e3, MutableDocument.newNoDocument(e3, SnapshotVersion.min()));
        } else
          __PRIVATE_hardAssert(n === 1, 20013, "Single document existence filter with count: " + n);
      else {
        const r2 = this.Pe(t);
        if (r2 !== n) {
          const n2 = this.Ie(e2), i2 = n2 ? this.Re(n2, e2, r2) : 1;
          if (i2 !== 0) {
            this.le(t);
            const e3 = i2 === 2 ? "TargetPurposeExistenceFilterMismatchBloom" : "TargetPurposeExistenceFilterMismatch";
            this.se = this.se.insert(t, e3);
          }
          ot?.Ae(function __PRIVATE_createExistenceFilterMismatchInfoForTestingHooks(e3, t2, n3, r3, i3) {
            const s = {
              localCacheCount: e3,
            ...
            _ && (s.bloomFilter = {
              ...
              mightContain: (e4) => r3?.mightContain(e4) ?? false
            });
            return s;
          }(r2, e2.M, this.X.Ve(), n2, i2));
        }
      }
    }
  }
  Ie(e2) {
    const t = e2.M.unchangedNames;
    if (!t || !t.bits)
      return null;
    ...
    try {
      ...
      if (e3 instanceof __PRIVATE_Base64DecodeError)
        return __PRIVATE_logWarn("Decoding the base64 bloom filter in existence filter failed (" + e3.message + "); ignoring the bloom filter and falling back to full re-query."), null;
      throw e3;
    }
    try {
  ...
  Re(e2, t, n) {
    return t.M.count === n - this.de(e2, t.targetId) ? 0 : 2;
  }
  de(e2, t) {
    const n = this.X.getRemoteKeysForTarget(t);
    let r = 0;
    return n.forEach((n2) => {
      const i = this.X.Ve(), s = `projects/${i.projectId}/databases/${i.database}/documents/${n2.path.canonicalString()}`;
      e2.mightContain(s) || (this.ae(t, n2, null), r++);
    }), r;
  }
  fe(e2) {
    const t = new Map;
    this.ee.forEach((n2, r2) => {
      const i = this.Te(r2);
      if (i) {
        if (n2.current && this.Ee(i.target)) {
          const t2 = __PRIVATE_targetIsPipelineTarget(i.target) ? ResourcePath.fromString(i.target.getPipelineDocuments()[0]) : i.target.path, n3 = new DocumentKey(t2);
          this.me(n3).has(r2) || this.pe(r2, n3) || this.ae(r2, n3, MutableDocument.newNoDocument(n3, e2));
        }
        n2.$ && (t.set(r2, n2.W()), n2.G());
      }
    });
    let n = __PRIVATE_documentKeySet();
    this.ie.forEach((e3, t2) => {
      let r2 = true;
      t2.forEachWhile((e4) => {
        const t3 = this.Te(e4);
        return !t3 || t3.purpose === "TargetPurposeLimboResolution" || (r2 = false, false);
      }), r2 && (n = n.add(e3));
    }), this.te.forEach((t2, n2) => n2.setReadTime(e2)), this.re.forEach((t2, n2) => n2.setReadTime(e2));
    const r = new RemoteEvent(e2, t, this.se, this.te, this.re, n);
  ...
  oe(e2, t) {
    const n = this.ee.get(e2);
    if (!n || !this.ce(e2))
      return void __PRIVATE_logDebug(ut, `addDocumentToTarget received document for unknown inactive target (${e2})`);
    const r = this.pe(e2, t.key) ? 2 : 0;
    n.j(t.key, r), __PRIVATE_targetIsPipelineTarget(this.Te(e2).target) && this.Te(e2).target.getPipelineFlavor() !== "exact" ? this.re = this.re.insert(t.key, t) : this.te = this.te.insert(t.key, t), this.ne = this.ne.insert(t.key, this.me(t.key).add(e2)), this.ie = this.ie.insert(t.key, this.ge(t.key).add(e2));
  }
  ae(e2, t, n) {
    const r = this.ee.get(e2);
    r && this.ce(e2) ? (this.pe(e2, t) ? r.j(t, 1) : r.H(t), this.ie = this.ie.insert(t, this.ge(t).delete(e2)), this.ie = this.ie.insert(t, this.ge(t).add(e2)), n && (__PRIVATE_targetIsPipelineTarget(this.Te(e2).target) && this.Te(e2).target.getPipelineFlavor() !== "exact" ? this.re = this.re.insert(t, n) : this.te = this.te.insert(t, n))) : __PRIVATE_logDebug(ut, `removeDocumentFromTarget received document for unknown or inactive target (${e2})`);
  }
  removeTarget(e2) {
    this.ee.delete(e2);
  }
  Pe(e2) {
    const t = this.ee.get(e2);
    if (!t)
      return 0;
    const n = t.W();
    return this.X.getRemoteKeysForTarget(e2).size + n.addedDocuments.size - n.removedDocuments.size;
  }
  J(e2) {
    let t = this.ee.get(e2);
    t || (__PRIVATE_logDebug(ut, `recordPendingTargetRequest set up tracking for target ID ${e2}`), t = new __PRIVATE_TargetState(e2), this.ee.set(e2, t)), t.J();
  }
  ge(e2) {
    let t = this.ie.get(e2);
    return t || (t = new SortedSet(__PRIVATE_primitiveComparator), this.ie = this.ie.insert(e2, t)), t;
  }
  me(e2) {
    let t = this.ne.get(e2);
    return t || (t = new SortedSet(__PRIVATE_primitiveComparator), this.ne = this.ne.insert(e2, t)), t;
  }
  ce(e2) {
    const t = this.Te(e2) !== null;
    return t || __PRIVATE_logDebug(ut, "Detected inactive target", e2), t;
  }
  Te(e2) {
    const t = this.ee.get(e2);
    return t === undefined || t.q ? null : this.X.ye(e2);
  }
  le(e2) {
    this.ee.set(e2, new __PRIVATE_TargetState(e2));
    this.X.getRemoteKeysForTarget(e2).forEach((t) => {
      this.ae(e2, t, null);
    });
  }
  pe(e2, t) {
    return this.X.getRemoteKeysForTarget(e2).has(t);
  }
}
...
class JsonProtoSerializer {
  constructor(e2, t) {
    this.databaseId = e2, this.useProto3Json = t;
  }
}
function __PRIVATE_toInt32Proto(e2, t) {
  return e2.useProto3Json || __PRIVATE_isNullOrUndefined(t) ? t : {
    value: t
  };
}
function toTimestamp(e2, t) {
  if (e2.useProto3Json) {
...
function __PRIVATE_toBytes(e2, t) {
  return e2.useProto3Json ? t.toBase64() : t.toUint8Array();
}
function __PRIVATE_toVersion(e2, t) {
  return toTimestamp(e2, t.toTimestamp());
}
function __PRIVATE_fromVersion(e2) {
  return __PRIVATE_hardAssert(!!e2, 49232), SnapshotVersion.fromTimestamp(fromTimestamp(e2));
}
function __PRIVATE_toResourceName(e2, t) {
  return __PRIVATE_toResourcePath(e2, t).canonicalString();
}
function __PRIVATE_toResourcePath(e2, t) {
  const n = function __PRIVATE_fullyQualifiedPrefixPath(e3) {
...
function __PRIVATE_fromResourceName(e2) {
  const t = ResourcePath.fromString(e2);
  return __PRIVATE_hardAssert(__PRIVATE_isValidResourceName(t), 10190, {
    key: t.toString()
  }), t;
}
function __PRIVATE_toName(e2, t) {
  return __PRIVATE_toResourceName(e2.databaseId, t.path);
}
function fromName(t, n) {
  const r = __PRIVATE_fromResourceName(n);
  if (r.get(1) !== t.databaseId.projectId)
    throw new e(ta.INVALID_ARGUMENT, "Tried to deserialize key from different project: " + r.get(1) + " vs " + t.databaseId.projectId);
  if (r.get(3) !== t.databaseId.database)
    throw new e(ta.INVALID_ARGUMENT, "Tried to deserialize key from different database: " + r.get(3) + " vs " + t.databaseId.database);
  return new DocumentKey(__PRIVATE_extractLocalPathFromResourceName(r));
}
function __PRIVATE_toQueryPath(e2, t) {
  return __PRIVATE_toResourceName(e2.databaseId, t);
}
function __PRIVATE_fromQueryPath(e2) {
  const t = __PRIVATE_fromResourceName(e2);
...
function __PRIVATE_extractLocalPathFromResourceName(e2) {
  return __PRIVATE_hardAssert(e2.length > 4 && e2.get(4) === "documents", 29091, {
    key: e2.toString()
  }), e2.popFirst(5);
}
function __PRIVATE_toMutationDocument(e2, t, n) {
  return {
    name: __PRIVATE_toName(e2, t),
...
function __PRIVATE_fromWatchChange(t, n) {
  ...
  if ("targetChange" in n) {
    n.targetChange;
    const i = function __PRIVATE_fromWatchTargetChangeState(e2) {
      return e2 === "NO_CHANGE" ? 0 : e2 === "ADD" ? 1 : e2 === "REMOVE" ? 2 : e2 === "CURRENT" ? 3 : e2 === "RESET" ? 4 : l(39313, {
        state: e2
      });
    }(n.targetChange.targetChangeType || "NO_CHANGE"), s = n.targetChange.targetIds || [], _ = function __PRIVATE_fromBytes(e2, t2) {
    ...
    }(t, n.targetChange.resumeToken), o = n.targetChange.cause, a = o && function __PRIVATE_fromRpcStatus(t2) {
      const n2 = t2.code === undefined ? ta.UNKNOWN : __PRIVATE_mapCodeFromRpcCode(t2.code);
      return new e(n2, t2.message || "");
    }(o);
    r = new __PRIVATE_WatchTargetChange(i, s, _, a || null);
  } else if ("documentChange" in n) {
    n.documentChange;
    const e2 = n.documentChange;
    e2.document, e2.document.name, e2.document.updateTime;
    const i = fromName(t, e2.document.name), s = __PRIVATE_fromVersion(e2.document.updateTime), _ = e2.document.createTime ? __PRIVATE_fromVersion(e2.document.createTime) : SnapshotVersion.min(), o = new ObjectValue({
      mapValue: {
        fields: e2.document.fields
      }
    }), a = MutableDocument.newFoundDocument(i, s, _, o), u = e2.targetIds || [], c = e2.removedTargetIds || [];
    r = new __PRIVATE_DocumentWatchChange(u, c, a.key, a);
  } else if ("documentDelete" in n) {
    n.documentDelete;
    const e2 = n.documentDelete;
    e2.document;
    const i = fromName(t, e2.document), s = e2.readTime ? __PRIVATE_fromVersion(e2.readTime) : SnapshotVersion.min(), _ = MutableDocument.newNoDocument(i, s), o = e2.removedTargetIds || [];
    r = new __PRIVATE_DocumentWatchChange([], o, _.key, _);
  } else if ("documentRemove" in n) {
    n.documentRemove;
    const e2 = n.documentRemove;
    e2.document;
    const i = fromName(t, e2.document), s = e2.removedTargetIds || [];
    r = new __PRIVATE_DocumentWatchChange([], s, i, null);
  } else {
    if (!("filter" in n))
      return l(11601, {
        we: n
      });
    {
      ...
      const e2 = n.filter;
      e2.targetId;
      const { count: t2 = 0, unchangedNames: i } = e2, s = new ExistenceFilter(t2, i), _ = e2.targetId;
      r = new __PRIVATE_ExistenceFilterChange(_, s);
    }
  }
  return r;
}
function toMutation(e2, t) {
  let n;
  if (t instanceof __PRIVATE_SetMutation)
    n = {
      update: __PRIVATE_toMutationDocument(e2, t.key, t.value)
    };
  else if (t instanceof __PRIVATE_DeleteMutation)
    n = {
      delete: __PRIVATE_toName(e2, t.key)
    };
  else if (t instanceof __PRIVATE_PatchMutation)
    n = {
      update: __PRIVATE_toMutationDocument(e2, t.key, t.data),
    ...
    n = {
      verify: __PRIVATE_toName(e2, t.key)
    };
  }
  return t.fieldTransforms.length > 0 && (n.updateTransforms = t.fieldTransforms.map((e3) => function __PRIVATE_toFieldTransform(e4, t2) {
  ...
  }(0, e3))), t.precondition.isNone || (n.currentDocument = function __PRIVATE_toPrecondition(e3, t2) {
    return t2.updateTime !== undefined ? {
      updateTime: __PRIVATE_toVersion(e3, t2.updateTime)
    } : t2.exists !== undefined ? {
      exists: t2.exists
    } : l(27497);
  }(e2, t.precondition)), n;
}
function __PRIVATE_fromWriteResults(e2, t) {
  return e2 && e2.length > 0 ? (__PRIVATE_hardAssert(t !== undefined, 14353), e2.map((e3) => function __PRIVATE_fromWriteResult(e4, t2) {
    let n = e4.updateTime ? __PRIVATE_fromVersion(e4.updateTime) : __PRIVATE_fromVersion(t2);
    return n.isEqual(SnapshotVersion.min()) && (n = __PRIVATE_fromVersion(t2)), new MutationResult(n, e4.transformResults || []);
  }(e3, t))) : [];
}
function __PRIVATE_toDocumentsTarget(e2, t) {
  return {
    documents: [__PRIVATE_toQueryPath(e2, t.path)]
  };
}
function __PRIVATE_toQueryTarget(e2, t) {
  const n = {
    structuredQuery: {}
  }, r = t.path;
  let i;
  ...
  }]), n.parent = __PRIVATE_toQueryPath(e2, i);
  const s = function __PRIVATE_toFilters(e3) {
    if (e3.length === 0)
      return;
    return __PRIVATE_toFilter(CompositeFilter.create(e3, "and"));
  }(t.filters);
  s && (n.structuredQuery.where = s);
  const _ = function __PRIVATE_toOrder(e3) {
    if (e3.length === 0)
      return;
    return e3.map((e4) => function __PRIVATE_toPropertyOrder(e5) {
      return {
  ...
  const o = __PRIVATE_toInt32Proto(e2, t.limit);
  return o !== null && (n.structuredQuery.limit = o), t.startAt && (n.structuredQuery.startAt = function __PRIVATE_toStartAtCursor(e3) {
    return {
      before: e3.inclusive,
      values: e3.position
    };
  }(t.startAt)), t.endAt && (n.structuredQuery.endAt = function __PRIVATE_toEndAtCursor(e3) {
    return {
      before: !e3.inclusive,
      values: e3.position
    };
...
function __PRIVATE_convertQueryTargetToQuery(e2) {
  let t = __PRIVATE_fromQueryPath(e2.parent);
  const n = e2.structuredQuery, r = n.from ? n.from.length : 0;
  let i = null;
  if (r > 0) {
    __PRIVATE_hardAssert(r === 1, 65062);
    const e3 = n.from[0];
    e3.allDescendants ? i = e3.collectionId : t = t.child(e3.collectionId);
  }
  let s = [];
  n.where && (s = function __PRIVATE_fromFilters(e3) {
    const t2 = __PRIVATE_fromFilter(e3);
    if (t2 instanceof CompositeFilter && __PRIVATE_compositeFilterIsFlatConjunction(t2))
      return t2.getFilters();
    return [t2];
  }(n.where));
  let _ = [];
  n.orderBy && (_ = function __PRIVATE_fromOrder(e3) {
    return e3.map((e4) => function __PRIVATE_fromPropertyOrder(e5) {
      return new OrderBy(__PRIVATE_fromFieldPathReference(e5.field), function __PRIVATE_fromDirection(e6) {
        switch (e6) {
          case "ASCENDING":
            return "asc";
          case "DESCENDING":
  ...
  let o = null;
  n.limit && (o = function __PRIVATE_fromInt32Proto(e3) {
    let t2;
    return t2 = typeof e3 == "object" ? e3.value : e3, __PRIVATE_isNullOrUndefined(t2) ? null : t2;
  }(n.limit));
  let a = null;
  n.startAt && (a = function __PRIVATE_fromStartAtCursor(e3) {
    const t2 = !!e3.before, n2 = e3.values || [];
    return new Bound(n2, t2);
  }(n.startAt));
  let u = null;
  return n.endAt && (u = function __PRIVATE_fromEndAtCursor(e3) {
    const t2 = !e3.before, n2 = e3.values || [];
    return new Bound(n2, t2);
  }(n.endAt)), __PRIVATE_newQuery(t, i, _, s, o, "F", a, u);
}
function __PRIVATE_toListenRequestLabels(e2, t) {
  const n = function __PRIVATE_toLabel(e3) {
    switch (e3) {
...
function __PRIVATE_toPipelineTarget(e2, t) {
  return {
    structuredPipeline: {
      pipeline: {
        stages: t.stages.map((t2) => t2._toProto(e2))
      }
    }
  };
}
function __PRIVATE_fromFilter(e2) {
  return e2.unaryFilter !== undefined ? function __PRIVATE_fromUnaryFilter(e3) {
    switch (e3.unaryFilter.op) {
      case "IS_NAN":
        const t = __PRIVATE_fromFieldPathReference(e3.unaryFilter.field);
        return FieldFilter.create(t, "==", {
          doubleValue: NaN
        });
      case "IS_NULL":
        const n = __PRIVATE_fromFieldPathReference(e3.unaryFilter.field);
        return FieldFilter.create(n, "==", {
          nullValue: "NULL_VALUE"
        });
      case "IS_NOT_NAN":
        const r = __PRIVATE_fromFieldPathReference(e3.unaryFilter.field);
        return FieldFilter.create(r, "!=", {
          doubleValue: NaN
        });
      case "IS_NOT_NULL":
        const i = __PRIVATE_fromFieldPathReference(e3.unaryFilter.field);
  ...
  }(e2) : e2.fieldFilter !== undefined ? function __PRIVATE_fromFieldFilter(e3) {
  ...
  }(e2) : e2.compositeFilter !== undefined ? function __PRIVATE_fromCompositeFilter(e3) {
...
function __PRIVATE_toFilter(e2) {
  return e2 instanceof FieldFilter ? function __PRIVATE_toUnaryOrFieldFilter(e3) {
    if (e3.op === "==") {
      ...
      if (__PRIVATE_isNullValue(e3.value))
        return {
          unaryFilter: {
            field: __PRIVATE_toFieldPathReference(e3.field),
            op: "IS_NULL"
    ...
    const t = e3.getFilters().map((e4) => __PRIVATE_toFilter(e4));
    if (t.length === 1)
      return t[0];
    return {
      compositeFilter: {
...
function __PRIVATE_toDocumentMask(e2) {
  const t = [];
  return e2.fields.forEach((e3) => t.push(e3.canonicalString())), {
    fieldPaths: t
  };
}
function __PRIVATE_isValidResourceName(e2) {
  return e2.length >= 4 && e2.get(0) === "projects" && e2.get(2) === "databases";
}
function __PRIVATE_isProtoValueSerializable(e2) {
  return !!e2 && typeof e2._toProto == "function" && e2._protoValueType === "ProtoValue";
}
function toMapValue(e2, t) {
  const n = {
    fields: {}
  };
  return t.forEach((t2, r) => {
    ...
    n.fields[r] = t2._toProto(e2);
  }), {
    mapValue: n
  };
}
function __PRIVATE_toStringValue(e2) {
  return {
    stringValue: e2
  };
}
function __PRIVATE_newSerializer(e2) {
  return new JsonProtoSerializer(e2, true);
}
...
class Bytes {
  ...
  static fromBase64String(t) {
    try {
      ...
      throw new e(ta.INVALID_ARGUMENT, "Failed to construct data from Base64 string: " + t2);
    }
  }
  static fromUint8Array(e2) {
  ...
  static fromJSON(e2) {
    if (__PRIVATE_validateJSON(e2, Bytes._jsonSchema))
      return Bytes.fromBase64String(e2.bytes);
  }
}
...
class FieldPath2 {
  constructor(...t) {
    for (let n = 0;n < t.length; ++n)
      if (t[n].length === 0)
        throw new e(ta.INVALID_ARGUMENT, "Invalid field name at argument $(i + 1). Field names must not be empty.");
...
class GeoPoint {
  constructor(t, n) {
    if (!isFinite(t) || t < -90 || t > 90)
      throw new e(ta.INVALID_ARGUMENT, "Latitude must be a number between -90 and 90, but was: " + t);
    if (!isFinite(n) || n < -180 || n > 180)
      throw new e(ta.INVALID_ARGUMENT, "Longitude must be a number between -180 and 180, but was: " + n);
  ...
  static fromJSON(e2) {
    if (__PRIVATE_validateJSON(e2, GeoPoint._jsonSchema))
      return new GeoPoint(e2.latitude, e2.longitude);
  }
}
...
class __PRIVATE_Deferred {
  constructor() {
    this.promise = new Promise((e2, t) => {
      this.resolve = e2, this.reject = t;
    });
  }
}
...
class __PRIVATE_OAuthToken {
  constructor(e2, t) {
    this.user = t, this.type = "OAuth", this.headers = new Map, this.headers.set("Authorization", `Bearer ${e2}`);
  }
}
...
class __PRIVATE_EmptyAuthCredentialsProvider {
  ...
  invalidateToken() {}
  start(e2, t) {
    e2.enqueueRetryable(() => t(User.UNAUTHENTICATED));
  }
  shutdown() {}
}
...
class __PRIVATE_EmulatorAuthCredentialsProvider {
  constructor(e2) {
    this.token = e2, this.changeListener = null;
  }
  getToken() {
  ...
  invalidateToken() {}
  start(e2, t) {
    this.changeListener = t, e2.enqueueRetryable(() => t(this.token.user));
  }
  shutdown() {
...
class __PRIVATE_FirebaseAuthCredentialsProvider {
  constructor(e2) {
    this.De = e2, this.currentUser = User.UNAUTHENTICATED, this.xe = 0, this.forceRefresh = false, this.auth = null;
  }
  start(e2, t) {
    __PRIVATE_hardAssert(this.Ce === undefined, 42304);
    let n = this.xe;
    const __PRIVATE_guardedChangeListener = (e3) => this.xe !== n ? (n = this.xe, t(e3)) : Promise.resolve();
    let r = new __PRIVATE_Deferred;
    this.Ce = () => {
      this.xe++, this.currentUser = this.Fe(), r.resolve(), r = new __PRIVATE_Deferred, e2.enqueueRetryable(() => __PRIVATE_guardedChangeListener(this.currentUser));
    };
    const __PRIVATE_awaitNextToken = () => {
      ...
      __PRIVATE_logDebug("FirebaseAuthCredentialsProvider", "Auth detected"), this.auth = e3, this.Ce && (this.auth.addAuthTokenListener(this.Ce), __PRIVATE_awaitNextToken());
    };
    this.De.onInit((e3) => __PRIVATE_registerAuth(e3)), setTimeout(() => {
      if (!this.auth) {
        const e3 = this.De.getImmediate({
          optional: true
        });
        e3 ? __PRIVATE_registerAuth(e3) : (__PRIVATE_logDebug("FirebaseAuthCredentialsProvider", "Auth not yet detected"), r.resolve(), r = new __PRIVATE_Deferred);
      }
    }, 0), __PRIVATE_awaitNextToken();
  }
  getToken() {
    const e2 = this.xe, t = this.forceRefresh;
    return this.forceRefresh = false, this.auth ? this.auth.getToken(t).then((t2) => this.xe !== e2 ? (__PRIVATE_logDebug("FirebaseAuthCredentialsProvider", "getToken aborted due to token change."), this.getToken()) : t2 ? (__PRIVATE_hardAssert(typeof t2.accessToken == "string", 31837, {
      Oe: t2
  ...
  shutdown() {
    this.auth && this.Ce && this.auth.removeAuthTokenListener(this.Ce), this.Ce = undefined;
  }
  Fe() {
    const e2 = this.auth && this.auth.getUid();
    return __PRIVATE_hardAssert(e2 === null || typeof e2 == "string", 2055, {
      Me: e2
    }), new User(e2);
  }
}
...
class __PRIVATE_FirstPartyToken {
  constructor(e2, t, n) {
    this.Ne = e2, this.Le = t, this.Be = n, this.type = "FirstParty", this.user = User.FIRST_PARTY, this.Ue = new Map;
  }
  ke() {
  ...
  get headers() {
    ...
    const e2 = this.ke();
    return e2 && this.Ue.set("Authorization", e2), this.Le && this.Ue.set("X-Goog-Iam-Authorization-Token", this.Le), this.Ue;
  }
}

class __PRIVATE_FirstPartyAuthCredentialsProvider {
  constructor(e2, t, n) {
    this.Ne = e2, this.Le = t, this.Be = n;
  }
  getToken() {
    return Promise.resolve(new __PRIVATE_FirstPartyToken(this.Ne, this.Le, this.Be));
  }
  start(e2, t) {
    e2.enqueueRetryable(() => t(User.FIRST_PARTY));
  }
  shutdown() {}
  invalidateToken() {}
}
...
class AppCheckToken {
  constructor(e2) {
    this.value = e2, this.type = "AppCheck", this.headers = new Map, e2 && e2.length > 0 && this.headers.set("x-firebase-appcheck", this.value);
  }
}

class __PRIVATE_FirebaseAppCheckTokenProvider {
  constructor(e2, t) {
    this.qe = t, this.forceRefresh = false, this.appCheck = null, this.$e = null, this.Ke = null, _isFirebaseServerApp(e2) && e2.settings.appCheckToken && (this.Ke = e2.settings.appCheckToken);
  }
  start(e2, t) {
    __PRIVATE_hardAssert(this.Ce === undefined, 3512);
    const onTokenChanged = (e3) => {
      e3.error != null && __PRIVATE_logDebug("FirebaseAppCheckTokenProvider", `Error getting App Check token; using placeholder token instead. Error: ${e3.error.message}`);
      const n = e3.token !== this.$e;
      return this.$e = e3.token, __PRIVATE_logDebug("FirebaseAppCheckTokenProvider", `Received ${n ? "new" : "existing"} token.`), n ? t(e3.token) : Promise.resolve();
    };
    this.Ce = (t2) => {
      e2.enqueueRetryable(() => onTokenChanged(t2));
    };
    const __PRIVATE_registerAppCheck = (e3) => {
      __PRIVATE_logDebug("FirebaseAppCheckTokenProvider", "AppCheck detected"), this.appCheck = e3, this.Ce && this.appCheck.addTokenListener(this.Ce);
    };
    this.qe.onInit((e3) => __PRIVATE_registerAppCheck(e3)), setTimeout(() => {
      if (!this.appCheck) {
        const e3 = this.qe.getImmediate({
          optional: true
        });
        e3 ? __PRIVATE_registerAppCheck(e3) : __PRIVATE_logDebug("FirebaseAppCheckTokenProvider", "AppCheck not yet detected");
      }
    }, 0);
  }
  getToken() {
    if (this.Ke)
      return Promise.resolve(new AppCheckToken(this.Ke));
    const e2 = this.forceRefresh;
    return this.forceRefresh = false, this.appCheck ? this.appCheck.getToken(e2).then((e3) => e3 ? (__PRIVATE_hardAssert(typeof e3.token == "string", 44558, {
      tokenResult: e3
    }), this.$e = e3.token, new AppCheckToken(e3.token)) : null) : Promise.resolve(null);
  }
  invalidateToken() {
    this.forceRefresh = true;
  }
  shutdown() {
    this.appCheck && this.Ce && this.appCheck.removeTokenListener(this.Ce), this.Ce = undefined;
  }
}
function __PRIVATE_cloneLongPollingOptions(e2) {
  const t = {};
  return e2.timeoutSeconds !== undefined && (t.timeoutSeconds = e2.timeoutSeconds), t;
}
...
function __PRIVATE_generateUniqueDebugId() {
  return Tt === null ? Tt = function __PRIVATE_generateInitialUniqueDebugId() {
...
class __PRIVATE_RestConnection {
  ...
  constructor(e2) {
    ...
    const t = e2.ssl ? "https" : "http", n = encodeURIComponent(this.databaseId.projectId), r = encodeURIComponent(this.databaseId.database);
    this.Xe = t + "://" + e2.host, this.et = `projects/${n}/databases/${r}`, this.tt = this.databaseId.database === $e ? `project_id=${n}` : `project_id=${n}&database_id=${r}`;
  }
  nt(e2, t, n, r, i) {
    const s = __PRIVATE_generateUniqueDebugId(), _ = this.rt(e2, t.toUriEncodedString());
    __PRIVATE_logDebug(Pt, `Sending RPC '${e2}' ${s}:`, _, n);
    const o = {
    ...
    return this.st(e2, _, o, n, u).then((t2) => (__PRIVATE_logDebug(Pt, `Received RPC '${e2}' ${s}: `, t2), t2), (t2) => {
      throw __PRIVATE_logWarn(Pt, `RPC '${e2}' ${s} failed with error: `, t2, "url: ", _, "request:", n), t2;
    });
  }
  _t(e2, t, n, r, i, s) {
    return this.nt(e2, t, n, r, i);
  }
  it(e2, t, n) {
    if (e2["X-Goog-Api-Client"] = function __PRIVATE_getGoogApiClientValue() {
      return "gl-js/ fire/" + Se;
    }(), e2["Content-Type"] = "text/plain", this.databaseInfo.appId && (e2["X-Firebase-GMPID"] = this.databaseInfo.appId), t && t.headers.forEach((t2, n2) => e2[n2] = t2), n && n.headers.forEach((t2, n2) => e2[n2] = t2), this.databaseInfo._customHeaders)
      for (const t2 of Object.keys(this.databaseInfo._customHeaders))
        e2[t2] = this.databaseInfo._customHeaders[t2];
  }
  rt(e2, t) {
    const n = It[e2];
...
var __PRIVATE_unguardedEventListen = (e2, t, n) => {
  e2.listen(t, (e3) => {
    try {
      n(e3);
    } catch (e4) {
...
class __PRIVATE_WebChannelConnection extends __PRIVATE_RestConnection {
  constructor(e2) {
    super(e2), this.ft = [], this.forceLongPolling = e2.forceLongPolling, this.autoDetectLongPolling = e2.autoDetectLongPolling, this.useFetchStreams = e2.useFetchStreams, this.longPollingOptions = e2.longPollingOptions;
  }
  static gt() {
    if (!__PRIVATE_WebChannelConnection.yt) {
      const e2 = getStatEventTarget();
      __PRIVATE_unguardedEventListen(e2, Event.STAT_EVENT, (e3) => {
        e3.stat === Stat.PROXY ? __PRIVATE_logDebug(Rt, "STAT_EVENT: detected buffering proxy") : e3.stat === Stat.NOPROXY && __PRIVATE_logDebug(Rt, "STAT_EVENT: detected no buffering proxy");
        ...
        try {
          switch (a.getLastErrorCode()) {
            case ErrorCode.NO_ERROR:
              const n2 = a.getResponseJson();
              __PRIVATE_logDebug(Rt, `XHR for RPC '${t}' ${_} received:`, JSON.stringify(n2)), s2(n2);
              break;
            case ErrorCode.TIMEOUT:
              __PRIVATE_logDebug(Rt, `RPC '${t}' ${_} timed out`), o(new e(ta.DEADLINE_EXCEEDED, "Request time out"));
              break;
            case ErrorCode.HTTP_ERROR:
              const r2 = a.getStatus();
              if (__PRIVATE_logDebug(Rt, `RPC '${t}' ${_} failed with status:`, r2, "response text:", a.getResponseText()), r2 > 0) {
                let t2 = a.getResponseJson();
                Array.isArray(t2) && (t2 = t2[0]);
                const n3 = t2?.error;
                if (n3 && n3.status && n3.message) {
                  const t3 = function __PRIVATE_mapCodeFromHttpResponseErrorStatus(e2) {
                    const t4 = e2.toLowerCase().replace(/_/g, "-");
                    return Object.values(ta).indexOf(t4) >= 0 ? t4 : ta.UNKNOWN;
                  }(n3.status);
                  o(new e(t3, n3.message));
                } else
                  o(new e(ta.UNKNOWN, "Server responded with status " + a.getStatus()));
              } else
                o(new e(ta.UNAVAILABLE, "Connection failed."));
              break;
            default:
              l(9055, {
                wt: t,
  ...
  vt(t, n, r) {
    const i = __PRIVATE_generateUniqueDebugId(), s = [this.Xe, "/", "google.firestore.v1.Firestore", "/", t, "/channel"], _ = this.createWebChannelTransport(), o = {
    ...
    a !== undefined && (o.longPollingTimeout = Math.round(1000 * a)), this.useFetchStreams && (o.useFetchStreams = true), this.it(o.initMessageHeaders, n, r), o.encodeInitMessageHeaders = true;
    const u = s.join("");
    __PRIVATE_logDebug(Rt, `Creating RPC '${t}' stream ${i}: ${u}`, o);
    const c = _.createWebChannel(u, o);
    this.Dt(c);
    let l2 = false, E = false;
    const h = new __PRIVATE_StreamBridge({
      ot: (e2) => {
        E ? __PRIVATE_logDebug(Rt, `Not sending because RPC '${t}' stream ${i} is closed:`, e2) : (l2 || (__PRIVATE_logDebug(Rt, `Opening RPC '${t}' stream ${i} transport.`), c.open(), l2 = true), __PRIVATE_logDebug(Rt, `RPC '${t}' stream ${i} sending:`, e2), c.send(e2));
      },
      ut: () => c.close()
    });
    return __PRIVATE_unguardedEventListen(c, WebChannel.EventType.OPEN, () => {
      ...
      E || (E = true, __PRIVATE_logDebug(Rt, `RPC '${t}' stream ${i} transport closed`), h.Vt(), this.xt(c));
    }), __PRIVATE_unguardedEventListen(c, WebChannel.EventType.ERROR, (n2) => {
      E || (E = true, __PRIVATE_logWarn(Rt, `RPC '${t}' stream ${i} transport errored. Name:`, n2.name, "Message:", n2.message), h.Vt(new e(ta.UNAVAILABLE, "The operation could not be completed")));
    }), __PRIVATE_unguardedEventListen(c, WebChannel.EventType.MESSAGE, (n2) => {
      if (!E) {
        const r2 = n2.data[0];
        __PRIVATE_hardAssert(!!r2, 16349);
        const s2 = r2, _2 = s2?.error || s2[0]?.error;
        if (_2) {
          __PRIVATE_logDebug(Rt, `RPC '${t}' stream ${i} received error:`, _2);
          const n3 = _2.status;
          let r3 = function __PRIVATE_mapCodeFromRpcStatus(e2) {
            const t2 = et[e2];
            if (t2 !== undefined)
              return __PRIVATE_mapCodeFromRpcCode(t2);
          }(n3), s3 = _2.message;
          n3 === "NOT_FOUND" && s3.includes("database") && s3.includes("does not exist") && s3.includes(this.databaseId.database) && __PRIVATE_logWarn(`Database '${this.databaseId.database}' not found. Please check your project configuration.`), r3 === undefined && (r3 = ta.INTERNAL, s3 = "Unknown error status: " + n3 + " with message " + _2.message), E = true, h.Vt(new e(r3, s3)), c.close();
        } else
          __PRIVATE_logDebug(Rt, `RPC '${t}' stream ${i} received:`, r2), h.dt(r2);
      }
  ...
  terminate() {
    this.ft.forEach((e2) => e2.close()), this.ft = [];
  }
  Dt(e2) {
    this.ft.push(e2);
  }
  xt(e2) {
    this.ft = this.ft.filter((t) => t === e2);
  }
  it(e2, t, n) {
    super.it(e2, t, n), this.databaseInfo.apiKey && (e2["x-goog-api-key"] = this.databaseInfo.apiKey);
  }
...
__PRIVATE_WebChannelConnection.yt = false;

class __PRIVATE_ExponentialBackoff {
  constructor(e2, t, n = 1000, r = 1.5, i = 60000) {
    this.Ct = e2, this.timerId = t, this.Ft = n, this.Ot = r, this.Mt = i, this.Nt = 0, this.Lt = null, this.Bt = Date.now(), this.reset();
  }
  reset() {
    this.Nt = 0;
  }
  ...
  kt(e2) {
    ...
    r > 0 && __PRIVATE_logDebug("ExponentialBackoff", `Backing off for ${r} ms (base delay: ${this.Nt} ms, delay with jitter: ${t} ms, last attempt: ${n} ms ago)`), this.Lt = this.Ct.enqueueAfterDelay(this.timerId, r, () => (this.Bt = Date.now(), e2())), this.Nt *= this.Ot, this.Nt < this.Ft && (this.Nt = this.Ft), this.Nt > this.Mt && (this.Nt = this.Mt);
  }
  $t() {
...
class __PRIVATE_PersistentStream {
  constructor(e2, t, n, r, i, s, _, o) {
    this.Ct = e2, this.Kt = n, this.Qt = r, this.connection = i, this.authCredentialsProvider = s, this.appCheckCredentialsProvider = _, this.listener = o, this.state = 0, this.Wt = 0, this.Gt = null, this.zt = null, this.stream = null, this.jt = 0, this.Ht = new __PRIVATE_ExponentialBackoff(e2, t);
  }
  Jt() {
  ...
  en() {
    this.Yt() && this.Gt === null && (this.Gt = this.Ct.enqueueAfterDelay(this.Kt, 60000, () => this.tn()));
  }
  nn(e2) {
    this.rn(), this.stream.send(e2);
  }
  async tn() {
    if (this.Yt())
      return this.close(0);
  ...
  async close(e2, t) {
    this.rn(), this.sn(), this.Ht.cancel(), this.Wt++, e2 !== 4 ? this.Ht.reset() : t && t.code === ta.RESOURCE_EXHAUSTED ? (__PRIVATE_logError(t.toString()), __PRIVATE_logError("Using maximum backoff delay to prevent overloading the backend."), this.Ht.Ut()) : t && t.code === ta.UNAUTHENTICATED && this.state !== 3 && (this.authCredentialsProvider.invalidateToken(), this.appCheckCredentialsProvider.invalidateToken()), this.stream !== null && (this._n(), this.stream.close(), this.stream = null), this.state = e2, await this.listener.Tt(t);
  }
  _n() {}
  auth() {
    this.state = 1;
    const t = this.an(this.Wt), n = this.Wt;
    Promise.all([this.authCredentialsProvider.getToken(), this.appCheckCredentialsProvider.getToken()]).then(([e2, t2]) => {
      this.Wt === n && this.un(e2, t2);
    }, (n2) => {
      t(() => {
        const t2 = new e(ta.UNKNOWN, "Fetching auth token failed: " + n2.message);
        return this.cn(t2);
      });
    });
  }
  un(e2, t) {
    ...
    this.stream = this.En(e2, t), this.stream.ct(() => {
      n(() => this.listener.ct());
    }), this.stream.Et(() => {
      ...
      n(() => ++this.jt == 1 ? this.hn(e3) : this.onNext(e3));
    });
  }
  Zt() {
  ...
  cn(e2) {
    return __PRIVATE_logDebug(At, `close with error: ${e2}`), this.stream = null, this.close(4, e2);
  }
  an(e2) {
    return (t) => {
      this.Ct.enqueueAndForget(() => this.Wt === e2 ? t() : (__PRIVATE_logDebug(At, "stream callback skipped by getCloseGuardedDispatcher."), Promise.resolve()));
    };
  }
}
...
class __PRIVATE_PersistentListenStream extends __PRIVATE_PersistentStream {
  constructor(e2, t, n, r, i, s) {
    super(e2, "listen_stream_connection_backoff", "listen_stream_idle", "health_check_timeout", t, n, r, s), this.serializer = i;
  }
  En(e2, t) {
    return this.connection.vt("Listen", e2, t);
  }
  hn(e2) {
  ...
  onNext(e2) {
    this.Ht.reset();
    const t = __PRIVATE_fromWatchChange(this.serializer, e2), n = function __PRIVATE_versionFromListenResponse(e3) {
      if (!("targetChange" in e3))
        return SnapshotVersion.min();
      const t2 = e3.targetChange;
  ...
  Pn(e2) {
    const t = {};
    t.database = __PRIVATE_getEncodedDatabaseId(this.serializer), t.addTarget = function __PRIVATE_toTarget(e3, t2) {
      let n2;
      const r = t2.target;
      if (n2 = __PRIVATE_targetIsPipelineTarget(r) ? {
        pipelineQuery: __PRIVATE_toPipelineTarget(e3, r)
      } : __PRIVATE_targetIsDocumentTarget(r) ? {
        documents: __PRIVATE_toDocumentsTarget(e3, r)
      } : {
        query: __PRIVATE_toQueryTarget(e3, r).Se
      }, n2.targetId = t2.targetId, t2.resumeToken.approximateByteSize() > 0) {
        n2.resumeToken = __PRIVATE_toBytes(e3, t2.resumeToken);
        const r2 = __PRIVATE_toInt32Proto(e3, t2.expectedCount);
        r2 !== null && (n2.expectedCount = r2);
      } else if (t2.snapshotVersion.compareTo(SnapshotVersion.min()) > 0) {
        n2.readTime = toTimestamp(e3, t2.snapshotVersion.toTimestamp());
        const r2 = __PRIVATE_toInt32Proto(e3, t2.expectedCount);
        r2 !== null && (n2.expectedCount = r2);
      }
      return n2;
    }(this.serializer, e2);
    const n = __PRIVATE_toListenRequestLabels(this.serializer, e2);
    n && (t.labels = n), this.nn(t);
  }
  In(e2) {
    const t = {};
    t.database = __PRIVATE_getEncodedDatabaseId(this.serializer), t.removeTarget = e2, this.nn(t);
  }
}

class __PRIVATE_PersistentWriteStream extends __PRIVATE_PersistentStream {
  constructor(e2, t, n, r, i, s) {
    super(e2, "write_stream_connection_backoff", "write_stream_idle", "health_check_timeout", t, n, r, s), this.serializer = i;
  }
  get Rn() {
    return this.jt > 0;
  }
  ...
  En(e2, t) {
    return this.connection.vt("Write", e2, t);
  }
  hn(e2) {
    return __PRIVATE_hardAssert(!!e2.streamToken, 31322), this.lastStreamToken = e2.streamToken, __PRIVATE_hardAssert(!e2.writeResults || e2.writeResults.length === 0, 55816), this.listener.Vn();
  }
  onNext(e2) {
    __PRIVATE_hardAssert(!!e2.streamToken, 12678), this.lastStreamToken = e2.streamToken, this.Ht.reset();
    const t = __PRIVATE_fromWriteResults(e2.writeResults, e2.commitTime), n = __PRIVATE_fromVersion(e2.commitTime);
    return this.listener.dn(n, t);
  }
  fn() {
    const e2 = {};
    e2.database = __PRIVATE_getEncodedDatabaseId(this.serializer), this.nn(e2);
  }
  An(e2) {
    const t = {
...
class __PRIVATE_DatastoreImpl extends Datastore {
  constructor(e2, t, n, r) {
    super(), this.authCredentials = e2, this.appCheckCredentials = t, this.connection = n, this.serializer = r, this.mn = false;
  }
  pn() {
    if (this.mn)
      throw new e(ta.FAILED_PRECONDITION, "The client has already been terminated.");
  }
  nt(t, n, r, i) {
    return this.pn(), Promise.all([this.authCredentials.getToken(), this.appCheckCredentials.getToken()]).then(([e2, s]) => this.connection.nt(t, __PRIVATE_toResourcePath(n, r), i, e2, s)).catch((t2) => {
      throw t2.name === "FirebaseError" ? (t2.code === ta.UNAUTHENTICATED && (this.authCredentials.invalidateToken(), this.appCheckCredentials.invalidateToken()), t2) : new e(ta.UNKNOWN, t2.toString());
    });
  }
  _t(t, n, r, i, s) {
    return this.pn(), Promise.all([this.authCredentials.getToken(), this.appCheckCredentials.getToken()]).then(([e2, _]) => this.connection._t(t, __PRIVATE_toResourcePath(n, r), i, e2, _, s)).catch((t2) => {
      throw t2.name === "FirebaseError" ? (t2.code === ta.UNAUTHENTICATED && (this.authCredentials.invalidateToken(), this.appCheckCredentials.invalidateToken()), t2) : new e(ta.UNKNOWN, t2.toString());
    });
  }
  terminate() {
...
function __PRIVATE_newDatastore(e2, t, n, r) {
  return new __PRIVATE_DatastoreImpl(e2, t, n, r);
}
var Vt = "ComponentProvider";
var dt = new Map;
function __PRIVATE_makeDatabaseInfo(e2, t, n, r, i) {
  return new DatabaseInfo(e2, t, n, i.host, i.ssl, i.experimentalForceLongPolling, i.experimentalAutoDetectLongPolling, __PRIVATE_cloneLongPollingOptions(i.experimentalLongPollingOptions), i.useFetchStreams, i.isUsingEmulator, r, i._customHeaders, i.grpcFlowControlWindow);
}
var ft = {
  didRun: false,
...
class LruParams {
  ...
  constructor(e2, t, n) {
    this.cacheSizeCollectionThreshold = e2, this.percentileToCollect = t, this.maximumSequenceNumbersToCollect = n;
  }
}
LruParams.DEFAULT_COLLECTION_PERCENTILE = 10, LruParams.DEFAULT_MAX_SEQUENCE_NUMBERS_TO_COLLECT = 1000, LruParams.DEFAULT = new LruParams(mt, LruParams.DEFAULT_COLLECTION_PERCENTILE, LruParams.DEFAULT_MAX_SEQUENCE_NUMBERS_TO_COLLECT), LruParams.DISABLED = new LruParams(-1, 0, 0);

class __PRIVATE_ListenSequence {
  constructor(e2, t) {
    this.previousValue = e2, t && (t.sequenceNumberHandler = (e3) => this.gn(e3), this.yn = (e3) => t.writeSequenceNumber(e3));
  }
  gn(e2) {
...
async function __PRIVATE_ignoreIfPrimaryLeaseLoss(e2) {
  if (e2.code !== ta.FAILED_PRECONDITION || e2.message !== pt)
    throw e2;
...
class PersistencePromise {
  constructor(e2) {
    this.nextCallback = null, this.catchCallback = null, this.result = undefined, this.error = undefined, this.isDone = false, this.callbackAttached = false, e2((e3) => {
      this.isDone = true, this.result = e3, this.nextCallback && this.nextCallback(e3);
    }, (e3) => {
      this.isDone = true, this.error = e3, this.catchCallback && this.catchCallback(e3);
    });
  }
  catch(e2) {
    return this.next(undefined, e2);
  }
  next(e2, t) {
    return this.callbackAttached && l(59440), this.callbackAttached = true, this.isDone ? this.error ? this.wrapFailure(t, this.error) : this.wrapSuccess(e2, this.result) : new PersistencePromise((n, r) => {
      this.nextCallback = (t2) => {
        this.wrapSuccess(e2, t2).next(n, r);
      }, this.catchCallback = (e3) => {
        this.wrapFailure(t, e3).next(n, r);
      };
    });
  }
  toPromise() {
  ...
  wrapSuccess(e2, t) {
    return e2 ? this.wrapUserFunction(() => e2(t)) : PersistencePromise.resolve(t);
  }
  wrapFailure(e2, t) {
    return e2 ? this.wrapUserFunction(() => e2(t)) : PersistencePromise.reject(t);
  }
  static resolve(e2) {
  ...
  static or(e2) {
    ...
    for (const n of e2)
      t = t.next((e3) => e3 ? PersistencePromise.resolve(e3) : n());
    return t;
  }
  static forEach(e2, t) {
    const n = [];
    return e2.forEach((e3, r) => {
      n.push(t.call(this, e3, r));
    }), this.waitFor(n);
  }
  static mapArray(e2, t) {
    return new PersistencePromise((n, r) => {
      const i = e2.length, s = new Array(i);
      let _ = 0;
      for (let o = 0;o < i; o++) {
        const a = o;
        t(e2[a]).next((e3) => {
...
function __PRIVATE_getAndroidVersion(e2) {
  const t = e2.match(/Android ([\d.]+)/i), n = t ? t[1].split(".").slice(0, 2).join(".") : "-1";
  return Number(n);
}
...
var bt = 1048576;
function __PRIVATE_bufferEntryComparator([e2, t], [n, r]) {
  const i = __PRIVATE_primitiveComparator(e2, n);
  return i === 0 ? __PRIVATE_primitiveComparator(t, r) : i;
}

class __PRIVATE_RollingSequenceNumberBuffer {
  constructor(e2) {
    this.Yn = e2, this.buffer = new SortedSet(__PRIVATE_bufferEntryComparator), this.Zn = 0;
  }
  Xn() {
    return ++this.Zn;
  }
  er(e2) {
    const t = [e2, this.Xn()];
    if (this.buffer.size < this.Yn)
      this.buffer = this.buffer.add(t);
    else {
      const e3 = this.buffer.last();
      __PRIVATE_bufferEntryComparator(t, e3) < 0 && (this.buffer = this.buffer.delete(e3).add(t));
    }
  }
  get maxValue() {
...
class __PRIVATE_LruScheduler {
  constructor(e2, t, n) {
    this.garbageCollector = e2, this.asyncQueue = t, this.localStore = n, this.tr = null;
  }
  start() {
    this.garbageCollector.params.cacheSizeCollectionThreshold !== -1 && this.nr(60000);
  }
  stop() {
  ...
  nr(e2) {
    __PRIVATE_logDebug(wt, `Garbage collection scheduled in ${e2}ms`), this.tr = this.asyncQueue.enqueueAfterDelay("lru_garbage_collection", e2, async () => {
      this.tr = null;
      try {
        await this.localStore.collectGarbage(this.garbageCollector);
      } catch (e3) {
        __PRIVATE_isIndexedDbTransactionError(e3) ? __PRIVATE_logDebug(wt, "Ignoring IndexedDB error during garbage collection: ", e3) : await __PRIVATE_ignoreIfPrimaryLeaseLoss(e3);
      }
      await this.nr(300000);
    });
  }
}
...
class __PRIVATE_LruGarbageCollectorImpl {
  constructor(e2, t) {
    this.rr = e2, this.params = t;
  }
  calculateTargetCount(e2, t) {
    return this.rr.ir(e2).next((e3) => Math.floor(t / 100 * e3));
  }
  nthSequenceNumber(e2, t) {
    if (t === 0)
      return PersistencePromise.resolve(__PRIVATE_ListenSequence.wn);
    const n = new __PRIVATE_RollingSequenceNumberBuffer(t);
    return this.rr.forEachTarget(e2, (e3) => n.er(e3.sequenceNumber)).next(() => this.rr.sr(e2, (e3) => n.er(e3))).next(() => n.maxValue);
  }
  removeTargets(e2, t, n) {
    return this.rr.removeTargets(e2, t, n);
  }
  removeOrphanedDocuments(e2, t) {
    return this.rr.removeOrphanedDocuments(e2, t);
  }
  collect(e2, t) {
    return this.params.cacheSizeCollectionThreshold === -1 ? (__PRIVATE_logDebug("LruGarbageCollector", "Garbage collection skipped; disabled"), PersistencePromise.resolve(ft)) : this.getCacheSize(e2).next((n) => n < this.params.cacheSizeCollectionThreshold ? (__PRIVATE_logDebug("LruGarbageCollector", `Garbage collection skipped; Cache size ${n} is lower than threshold ${this.params.cacheSizeCollectionThreshold}`), ft) : this._r(e2, t));
  }
  getCacheSize(e2) {
    return this.rr.getCacheSize(e2);
  }
  _r(e2, t) {
    let n, r, i, s, _, o, a;
    const u = Date.now();
    return this.calculateTargetCount(e2, this.params.percentileToCollect).next((t2) => (t2 > this.params.maximumSequenceNumbersToCollect ? (__PRIVATE_logDebug("LruGarbageCollector", `Capping sequence numbers to collect down to the maximum of ${this.params.maximumSequenceNumbersToCollect} from ${t2}`), r = this.params.maximumSequenceNumbersToCollect) : r = t2, s = Date.now(), this.nthSequenceNumber(e2, r))).next((r2) => (n = r2, _ = Date.now(), this.removeTargets(e2, n, t))).next((t2) => (i = t2, o = Date.now(), this.removeOrphanedDocuments(e2, n))).next((e3) => {
...
` + `	Removed ${e3} documents in ` + (a - o) + `ms
...
function __PRIVATE_newLruGarbageCollector(e2, t) {
  return new __PRIVATE_LruGarbageCollectorImpl(e2, t);
}
...
class FirestoreSettingsImpl {
  constructor(t) {
    if (t.host === undefined) {
      if (t.ssl !== undefined)
        throw new e(ta.INVALID_ARGUMENT, "Can't provide ssl option if host option is not set");
      this.host = St, this.ssl = vt;
    } else
      this.host = t.host, this.ssl = t.ssl ?? vt;
    if (this.isUsingEmulator = t.emulatorOptions !== undefined, this.credentials = t.credentials, this.ignoreUndefinedProperties = !!t.ignoreUndefinedProperties, this.localCache = t.localCache, t._customHeaders && (this._customHeaders = {
      ...t._customHeaders
    }), t.cacheSizeBytes === undefined)
      this.cacheSizeBytes = mt;
    else {
      if (t.cacheSizeBytes !== -1 && t.cacheSizeBytes < bt)
        throw new e(ta.INVALID_ARGUMENT, "cacheSizeBytes must be at least 1048576");
      this.cacheSizeBytes = t.cacheSizeBytes;
    }
    if (__PRIVATE_validateIsNotUsedTogether("experimentalForceLongPolling", t.experimentalForceLongPolling, "experimentalAutoDetectLongPolling", t.experimentalAutoDetectLongPolling), this.experimentalForceLongPolling = !!t.experimentalForceLongPolling, this.experimentalForceLongPolling ? this.experimentalAutoDetectLongPolling = false : t.experimentalAutoDetectLongPolling === undefined ? this.experimentalAutoDetectLongPolling = true : this.experimentalAutoDetectLongPolling = !!t.experimentalAutoDetectLongPolling, this.experimentalLongPollingOptions = __PRIVATE_cloneLongPollingOptions(t.experimentalLongPollingOptions ?? {}), function __PRIVATE_validateLongPollingOptions(t2) {
      if (t2.timeoutSeconds !== undefined) {
        if (isNaN(t2.timeoutSeconds))
          throw new e(ta.INVALID_ARGUMENT, `invalid long polling timeout: ${t2.timeoutSeconds} (must not be NaN)`);
        if (t2.timeoutSeconds < 5)
          throw new e(ta.INVALID_ARGUMENT, `invalid long polling timeout: ${t2.timeoutSeconds} (minimum allowed value is 5)`);
        if (t2.timeoutSeconds > 30)
          throw new e(ta.INVALID_ARGUMENT, `invalid long polling timeout: ${t2.timeoutSeconds} (maximum allowed value is 30)`);
      }
    }(this.experimentalLongPollingOptions), this.useFetchStreams = !!t.useFetchStreams, t.grpcFlowControlWindow !== undefined) {
      if (typeof t.grpcFlowControlWindow != "number" || t.grpcFlowControlWindow <= 0 || t.grpcFlowControlWindow > 2147483647 || !Number.isInteger(t.grpcFlowControlWindow))
        throw new e(ta.INVALID_ARGUMENT, "grpcFlowControlWindow must be a positive integer and cannot exceed 2147483647");
      this.grpcFlowControlWindow = t.grpcFlowControlWindow;
    }
  }
  isEqual(e2) {
    return this.host === e2.host && this.ssl === e2.ssl && this.credentials === e2.credentials && this.cacheSizeBytes === e2.cacheSizeBytes && this.experimentalForceLongPolling === e2.experimentalForceLongPolling && this.experimentalAutoDetectLongPolling === e2.experimentalAutoDetectLongPolling && function __PRIVATE_longPollingOptionsEqual(e3, t) {
      return e3.timeoutSeconds === t.timeoutSeconds;
    }(this.experimentalLongPollingOptions, e2.experimentalLongPollingOptions) && this.ignoreUndefinedProperties === e2.ignoreUndefinedProperties && this.useFetchStreams === e2.useFetchStreams && this.grpcFlowControlWindow === e2.grpcFlowControlWindow && function __PRIVATE_customHeadersEqual(e3, t) {
      if (e3 === t)
        return true;
      if (!e3 || !t)
        return false;
      const n = Object.keys(e3), r = Object.keys(t);
      ...
      for (const r2 of n)
        if (e3[r2] !== t[r2])
          return false;
      return true;
    }(this._customHeaders, e2._customHeaders);
  }
}
var Dt = class Firestore {
  constructor(e2, t, n, r) {
    this._authCredentials = e2, this._appCheckCredentials = t, this._databaseId = n, this._app = r, this.type = "firestore-lite", this._persistenceKey = "(lite)", this._settings = new FirestoreSettingsImpl({}), this._settingsFrozen = false, this._emulatorOptions = {}, this._terminateTask = "notTerminated";
  }
  get app() {
    if (!this._app)
      throw new e(ta.FAILED_PRECONDITION, "Firestore was not initialized using the Firebase SDK. 'app' is not available");
    return this._app;
  }
  get _initialized() {
  ...
  _setSettings(t) {
    if (this._settingsFrozen)
      throw new e(ta.FAILED_PRECONDITION, "Firestore has already been started and its settings can no longer be changed. You can only modify settings before calling any other methods on a Firestore object.");
    this._settings = new FirestoreSettingsImpl(t), this._emulatorOptions = t.emulatorOptions || {}, t.credentials !== undefined && (this._authCredentials = function __PRIVATE_makeAuthCredentialsProvider(t2) {
      if (!t2)
      ...
      switch (t2.type) {
        ...
        case "provider":
          return t2.client;
        default:
          throw new e(ta.INVALID_ARGUMENT, "makeAuthCredentialsProvider failed due to invalid credential type");
      }
    }(t.credentials));
  }
  _getSettings() {
  ...
  _terminate() {
    return function __PRIVATE_removeComponents(e2) {
      const t = dt.get(e2);
      t && (__PRIVATE_logDebug(Vt, "Removing Datastore"), dt.delete(e2), t.terminate());
...
function connectFirestoreEmulator(t, n, r, i = {}) {
  ...
  if (!deepEqual(u, o) && (t._setSettings(u), i.mockUserToken)) {
    ...
    if (typeof i.mockUserToken == "string")
      n2 = i.mockUserToken, r2 = User.MOCK_USER;
    else {
      n2 = createMockUserToken(i.mockUserToken, t._app?.options.projectId);
      const s2 = i.mockUserToken.sub || i.mockUserToken.user_id;
      if (!s2)
        throw new e(ta.INVALID_ARGUMENT, "mockUserToken must contain 'sub' or 'user_id' field!");
      r2 = new User(s2);
    }
    t._authCredentials = new __PRIVATE_EmulatorAuthCredentialsProvider(new __PRIVATE_OAuthToken(n2, r2));
  }
}

class Query {
  constructor(e2, t, n) {
    this.converter = t, this._query = n, this.type = "query", this.firestore = e2;
  }
  withConverter(e2) {
...
class aa {
  constructor(e2, t, n) {
    this.converter = t, this._key = n, this.type = "document", this.firestore = e2;
  }
  get _path() {
  ...
  withConverter(e2) {
    return new aa(this.firestore, e2, this._key);
  }
  toJSON() {
    return {
  ...
  static fromJSON(e2, t, n) {
    if (__PRIVATE_validateJSON(t, aa._jsonSchema))
      return new aa(e2, n || null, new DocumentKey(ResourcePath.fromString(t.referencePath)));
  }
}
...
class na extends Query {
  constructor(e2, t, n) {
    super(e2, t, __PRIVATE_newQueryForPath(n)), this._path = n, this.type = "collection";
  }
  get id() {
  ...
  get parent() {
    ...
    return e2.isEmpty() ? null : new aa(this.firestore, null, new DocumentKey(e2));
  }
  withConverter(e2) {
    return new na(this.firestore, e2, this._path);
  }
}
function collection(t, n, ...r) {
  if (t = getModularInstance(t), __PRIVATE_validateNonEmptyArgument("collection", "path", n), t instanceof Dt) {
    const e2 = ResourcePath.fromString(n, ...r);
    return __PRIVATE_validateCollectionPath(e2), new na(t, null, e2);
  }
  {
    if (!(t instanceof aa || t instanceof na))
      throw new e(ta.INVALID_ARGUMENT, "Expected first argument to collection() to be a CollectionReference, a DocumentReference or FirebaseFirestore");
    const i = t._path.child(ResourcePath.fromString(n, ...r));
    return __PRIVATE_validateCollectionPath(i), new na(t.firestore, null, i);
  }
}
function doc(t, n, ...r) {
  if (t = getModularInstance(t), arguments.length === 1 && (n = __PRIVATE_AutoId.newId()), __PRIVATE_validateNonEmptyArgument("doc", "path", n), t instanceof Dt) {
    const e2 = ResourcePath.fromString(n, ...r);
    return __PRIVATE_validateDocumentPath(e2), new aa(t, null, new DocumentKey(e2));
  }
  {
    if (!(t instanceof aa || t instanceof na))
      throw new e(ta.INVALID_ARGUMENT, "Expected first argument to doc() to be a CollectionReference, a DocumentReference or FirebaseFirestore");
    const i = t._path.child(ResourcePath.fromString(n, ...r));
...
class n {
  ...
  isEqual(e2) {
    return function __PRIVATE_isPrimitiveArrayEqual(e3, t) {
  ...
  static fromJSON(t) {
    if (__PRIVATE_validateJSON(t, n._jsonSchema)) {
      if (Array.isArray(t.vectorValues) && t.vectorValues.every((e2) => typeof e2 == "number"))
        return new n(t.vectorValues);
...
class ParsedSetData {
  constructor(e2, t, n2) {
    this.data = e2, this.fieldMask = t, this.fieldTransforms = n2;
  }
  toMutation(e2, t) {
    return this.fieldMask !== null ? new __PRIVATE_PatchMutation(e2, this.data, this.fieldMask, t, this.fieldTransforms) : new __PRIVATE_SetMutation(e2, this.data, t, this.fieldTransforms);
  }
}
...
class ParseContextImpl {
  constructor(e2, t, n2, r, i, s) {
    this.settings = e2, this.databaseId = t, this.serializer = n2, this.ignoreUndefinedProperties = r, i === undefined && this.validatePath(), this.fieldTransforms = i || [], this.fieldMask = s || [];
  }
  get path() {
  ...
  childContextForField(e2) {
    const t = this.path?.child(e2), n2 = this.contextWith({
      path: t,
      arrayElement: false
    });
  ...
  contains(e2) {
    return this.fieldMask.find((t) => e2.isPrefixOf(t)) !== undefined || this.fieldTransforms.find((t) => e2.isPrefixOf(t.field)) !== undefined;
  }
  validatePath() {
    if (this.path)
      for (let e2 = 0;e2 < this.path.length; e2++)
        this.validatePathSegment(this.path.get(e2));
  }
  validatePathSegment(e2) {
    if (e2.length === 0)
      throw this.createError("Document fields must not be empty");
    if (__PRIVATE_isWrite(this.dataSource) && xt.test(e2))
...
class UserDataReader {
  constructor(e2, t, n2) {
    this.databaseId = e2, this.ignoreUndefinedProperties = t, this.serializer = n2 || __PRIVATE_newSerializer(e2);
  }
  createContext(e2, t, n2, r = false) {
    return new ParseContextImpl({
      dataSource: e2,
      methodName: t,
      targetDoc: n2,
...
function la(e2) {
  const t = e2._freezeSettings(), n2 = __PRIVATE_newSerializer(e2._databaseId);
  return new UserDataReader(e2._databaseId, !!t.ignoreUndefinedProperties, n2);
}
function __PRIVATE_parseSetData(t, n2, r, i, s, _ = {}) {
  const o = t.createContext(_.merge || _.mergeFields ? 2 : 0, n2, r, s);
  __PRIVATE_validatePlainObject("Data must be an object, but it was:", o, i);
  const a = __PRIVATE_parseObject(i, o);
  let u, c;
  if (_.merge)
    u = new FieldMask(o.fieldMask), c = o.fieldTransforms;
  else if (_.mergeFields) {
    const t2 = [];
    for (const i2 of _.mergeFields) {
      const s2 = K(n2, i2, r);
      if (!o.contains(s2))
        throw new e(ta.INVALID_ARGUMENT, `Field '${s2}' is specified in your field mask but missing from your input data.`);
      __PRIVATE_fieldMaskContains(t2, s2) || t2.push(s2);
    }
    u = new FieldMask(t2), c = o.fieldTransforms.filter((e2) => u.covers(e2.field));
  } else
    u = null, c = o.fieldTransforms;
  return new ParsedSetData(new ObjectValue(a), u, c);
}
function __PRIVATE_parseData(e2, t, r) {
  if (__PRIVATE_looksLikeJsonObject(e2 = getModularInstance(e2)))
    return __PRIVATE_validatePlainObject("Unsupported field value:", t, e2), __PRIVATE_parseObject(e2, t);
  if (e2 instanceof FieldValue)
    return function __PRIVATE_parseSentinelFieldValue(e3, t2) {
      if (!__PRIVATE_isWrite(t2.dataSource))
        throw t2.createError(`${e3._methodName}() can only be used with update() and set()`);
      if (!t2.path)
        throw t2.createError(`${e3._methodName}() is not currently supported inside arrays`);
      const n2 = e3._toFieldTransform(t2);
      n2 && t2.fieldTransforms.push(n2);
    }(e2, t), null;
  if (e2 === undefined && t.ignoreUndefinedProperties)
    return null;
  if (t.path && t.fieldMask.push(t.path), e2 instanceof Array) {
    if (t.settings.arrayElement && t.dataSource !== 4)
      throw t.createError("Nested arrays are not supported");
    return function __PRIVATE_parseArray(e3, t2) {
      const n2 = [];
      let r2 = 0;
      for (const i of e3) {
        let e4 = __PRIVATE_parseData(i, t2.childContextForArray(r2));
        e4 == null && (e4 = {
          nullValue: "NULL_VALUE"
        }), n2.push(e4), r2++;
      }
  ...
  return function __PRIVATE_parseScalarValue(e3, t2, r2) {
    if ((e3 = getModularInstance(e3)) === null)
      return {
        nullValue: "NULL_VALUE"
      };
    if (typeof e3 == "number")
      return toNumber(t2.serializer, e3);
    ...
    if (e3 instanceof Date) {
      const n2 = Timestamp.fromDate(e3);
      return {
        timestampValue: toTimestamp(t2.serializer, n2)
      };
    }
    if (e3 instanceof Timestamp) {
      const n2 = new Timestamp(e3.seconds, 1000 * Math.floor(e3.nanoseconds / 1000));
      return {
        timestampValue: toTimestamp(t2.serializer, n2)
      };
    }
    if (__PRIVATE_isTemporalInstant(e3)) {
      const n2 = Timestamp.fromInstant(e3), r3 = new Timestamp(n2.seconds, 1000 * Math.floor(n2.nanoseconds / 1000));
      return {
        timestampValue: toTimestamp(t2.serializer, r3)
      };
    }
    if (e3 instanceof GeoPoint)
      return {
        geoPointValue: {
    ...
    if (e3 instanceof Bytes)
      return {
        bytesValue: __PRIVATE_toBytes(t2.serializer, e3._byteString)
      };
    if (e3 instanceof aa) {
      const n2 = t2.databaseId, r3 = e3.firestore._databaseId;
      if (!r3.isEqual(n2))
        throw t2.createError(`Document reference is for database ${r3.projectId}/${r3.database} but should be for database ${n2.projectId}/${n2.database}`);
      return {
        referenceValue: __PRIVATE_toResourceName(e3.firestore._databaseId || t2.databaseId, e3._key.path)
      };
    }
    if (e3 instanceof n)
      return function __PRIVATE_parseVectorValue(e4, t3) {
        const r3 = e4 instanceof n ? e4.toArray() : e4, i = {
          fields: {
            [Qe]: {
              stringValue: ze
            },
            [je]: {
              arrayValue: {
                values: r3.map((e5) => {
                  ...
                  return __PRIVATE_toDouble(t3.serializer, e5);
                })
              }
            }
          }
    ...
    if (__PRIVATE_isProtoValueSerializable(e3))
      return e3._toProto(t2.serializer);
    throw t2.createError(`Unsupported field value: ${__PRIVATE_valueDescription(e3)}`);
  }(e2, t);
}
function __PRIVATE_parseObject(e2, t) {
  const n2 = {};
  return isEmpty2(e2) ? t.path && t.path.length > 0 && t.fieldMask.push(t.path) : forEach(e2, (e3, r) => {
    const i = __PRIVATE_parseData(r, t.childContextForField(e3));
    i != null && (n2[e3] = i);
  }), {
    mapValue: {
      fields: n2
...
function __PRIVATE_isTemporalInstant(e2) {
  if (typeof e2 != "object" || e2 === null)
    return false;
  if (typeof Temporal != "undefined" && typeof Temporal.Instant == "function") {
    if (e2 instanceof Temporal.Instant)
      return true;
  }
  const t = e2;
...
function __PRIVATE_looksLikeJsonObject(e2) {
  return !(typeof e2 != "object" || e2 === null || e2 instanceof Array || e2 instanceof Date || e2 instanceof Timestamp || e2 instanceof GeoPoint || e2 instanceof Bytes || e2 instanceof aa || e2 instanceof FieldValue || e2 instanceof n || __PRIVATE_isTemporalInstant(e2) || __PRIVATE_isProtoValueSerializable(e2));
}
function __PRIVATE_validatePlainObject(e2, t, n2) {
  if (!__PRIVATE_looksLikeJsonObject(n2) || !p(n2)) {
    const r = __PRIVATE_valueDescription(n2);
    throw r === "an object" ? t.createError(e2 + " a custom object") : t.createError(e2 + " " + r);
  }
}
function K(e2, t, n2) {
  if ((t = getModularInstance(t)) instanceof FieldPath2)
    return t._internalPath;
  if (typeof t == "string")
    return __PRIVATE_fieldPathFromDotSeparatedString(e2, t);
  throw createError("Field path arguments must be of type string or ", e2, false, undefined, n2);
}
var Ct = new RegExp("[~\\*/\\[\\]]");
function __PRIVATE_fieldPathFromDotSeparatedString(e2, t, n2) {
  if (t.search(Ct) >= 0)
    throw createError(`Invalid field path (${t}). Paths must not contain '~', '*', '/', '[', or ']'`, e2, false, undefined, n2);
  try {
    return new FieldPath2(...t.split("."))._internalPath;
  } catch (r) {
    throw createError(`Invalid field path (${t}). Paths must not be empty, begin with '.', end with '.', or contain '..'`, e2, false, undefined, n2);
  }
}
function createError(t, n2, r, i, s) {
  const _ = i && !i.isEmpty(), o = s !== undefined;
  let a = `Function ${n2}() called with invalid data`;
  r && (a += " (via `toFirestore()`)"), a += ". ";
  let u = "";
  return (_ || o) && (u += " (found", _ && (u += ` in field ${i}`), o && (u += ` in document ${s}`), u += ")"), new e(ta.INVALID_ARGUMENT, a + t + u);
}
function __PRIVATE_fieldMaskContains(e2, t) {
  return e2.some((e3) => e3.isEqual(t));
}
function __PRIVATE_isUserData(e2) {
  return typeof e2._readUserData == "function";
}

class OptionsUtil {
  constructor(e2) {
    this.optionDefinitions = e2;
  }
  _getKnownOptions(e2, t) {
    const n2 = ObjectValue.empty();
      ...
      if (this.optionDefinitions.hasOwnProperty(r)) {
        ...
        if (r in e2) {
          ...
          if (i.nestedOptions && p(s)) {
            ...
            s && (_ = __PRIVATE_parseData(s, t) ?? undefined);
          _ && n2.set(Oe.fromServerFormat(i.serverName), _);
        }
      }
    return n2;
  }
  getOptionsProto(e2, t, n2) {
    const r = this._getKnownOptions(t, e2);
    if (n2) {
      const t2 = new Map(__PRIVATE_mapToArray(n2, (t3, n3) => [Oe.fromServerFormat(n3), t3 !== undefined ? __PRIVATE_parseData(t3, e2) : null]));
      r.setAll(t2);
    }
...
function u(e2) {
  return typeof e2 == "object" && e2 !== null && !!(("nullValue" in e2) && (e2.nullValue === null || e2.nullValue === "NULL_VALUE") || ("booleanValue" in e2) && (e2.booleanValue === null || typeof e2.booleanValue == "boolean") || ("integerValue" in e2) && (e2.integerValue === null || typeof e2.integerValue == "number" || typeof e2.integerValue == "string") || ("doubleValue" in e2) && (e2.doubleValue === null || typeof e2.doubleValue == "number") || ("timestampValue" in e2) && (e2.timestampValue === null || function __PRIVATE_isITimestamp(e3) {
    return typeof e3 == "object" && e3 !== null && "seconds" in e3 && (e3.seconds === null || typeof e3.seconds == "number" || typeof e3.seconds == "string") && "nanos" in e3 && (e3.nanos === null || typeof e3.nanos == "number");
  }(e2.timestampValue)) || ("stringValue" in e2) && (e2.stringValue === null || typeof e2.stringValue == "string") || ("bytesValue" in e2) && (e2.bytesValue === null || e2.bytesValue instanceof Uint8Array) || ("referenceValue" in e2) && (e2.referenceValue === null || typeof e2.referenceValue == "string") || ("geoPointValue" in e2) && (e2.geoPointValue === null || function __PRIVATE_isILatLng(e3) {
    return typeof e3 == "object" && e3 !== null && "latitude" in e3 && (e3.latitude === null || typeof e3.latitude == "number") && "longitude" in e3 && (e3.longitude === null || typeof e3.longitude == "number");
  }(e2.geoPointValue)) || ("arrayValue" in e2) && (e2.arrayValue === null || function __PRIVATE_isIArrayValue(e3) {
    return typeof e3 == "object" && e3 !== null && !(!("values" in e3) || e3.values !== null && !Array.isArray(e3.values));
  }(e2.arrayValue)) || ("mapValue" in e2) && (e2.mapValue === null || function __PRIVATE_isIMapValue(e3) {
    return typeof e3 == "object" && e3 !== null && !(!("fields" in e3) || e3.fields !== null && !p(e3.fields));
  }(e2.mapValue)) || ("fieldReferenceValue" in e2) && (e2.fieldReferenceValue === null || typeof e2.fieldReferenceValue == "string") || ("functionValue" in e2) && (e2.functionValue === null || function __PRIVATE_isIFunction(e3) {
    return typeof e3 == "object" && e3 !== null && !(!("name" in e3) || e3.name !== null && typeof e3.name != "string" || !("args" in e3) || e3.args !== null && !Array.isArray(e3.args));
  }(e2.functionValue)) || ("pipelineValue" in e2) && (e2.pipelineValue === null || function __PRIVATE_isIPipeline(e3) {
    return typeof e3 == "object" && e3 !== null && !(!("stages" in e3) || e3.stages !== null && !Array.isArray(e3.stages));
  }(e2.pipelineValue)));
}
function r(e2) {
  return new n(e2);
}
function __PRIVATE_valueToDefaultExpr(e2) {
  let n2;
  return e2 instanceof t ? e2 : (n2 = p(e2) ? __PRIVATE__map(e2) : e2 instanceof Array ? b(e2) : m(e2, undefined), n2);
}
function __PRIVATE_vectorToExpr(e2) {
  if (e2 instanceof t)
    return e2;
  if (e2 instanceof n)
    return i(e2);
...
function __PRIVATE_fieldOrExpression(e2) {
  if (__PRIVATE_isString$1(e2)) {
    return s(e2);
  }
  return __PRIVATE_valueToDefaultExpr(e2);
}
...
class t {
  ...
  add(e2) {
    return new y("add", [this, __PRIVATE_valueToDefaultExpr(e2)], "add");
  }
  asBoolean() {
    if (this instanceof BooleanExpression)
      return this;
    if (this instanceof Constant)
      return new __PRIVATE_BooleanConstant(this);
    if (this instanceof o)
      return new __PRIVATE_BooleanField(this);
    if (this instanceof y)
      return new __PRIVATE_BooleanFunctionExpression(this);
    throw new e("invalid-argument", `Conversion of type ${typeof this} to BooleanExpression not supported.`);
  }
  subtract(e2) {
    return new y("subtract", [this, __PRIVATE_valueToDefaultExpr(e2)], "subtract");
  }
  multiply(e2) {
    return new y("multiply", [this, __PRIVATE_valueToDefaultExpr(e2)], "multiply");
  }
  divide(e2) {
    return new y("divide", [this, __PRIVATE_valueToDefaultExpr(e2)], "divide");
  }
  mod(e2) {
    return new y("mod", [this, __PRIVATE_valueToDefaultExpr(e2)], "mod");
  }
  equal(e2) {
    return new y("equal", [this, __PRIVATE_valueToDefaultExpr(e2)], "equal").asBoolean();
  }
  notEqual(e2) {
    return new y("not_equal", [this, __PRIVATE_valueToDefaultExpr(e2)], "notEqual").asBoolean();
  }
  lessThan(e2) {
    return new y("less_than", [this, __PRIVATE_valueToDefaultExpr(e2)], "lessThan").asBoolean();
  }
  lessThanOrEqual(e2) {
    return new y("less_than_or_equal", [this, __PRIVATE_valueToDefaultExpr(e2)], "lessThanOrEqual").asBoolean();
  }
  greaterThan(e2) {
    return new y("greater_than", [this, __PRIVATE_valueToDefaultExpr(e2)], "greaterThan").asBoolean();
  }
  greaterThanOrEqual(e2) {
    return new y("greater_than_or_equal", [this, __PRIVATE_valueToDefaultExpr(e2)], "greaterThanOrEqual").asBoolean();
  }
  arrayConcat(e2, ...t2) {
    const n2 = [e2, ...t2].map((e3) => __PRIVATE_valueToDefaultExpr(e3));
    return new y("array_concat", [this, ...n2], "arrayConcat");
  }
  arrayContains(e2) {
    return new y("array_contains", [this, __PRIVATE_valueToDefaultExpr(e2)], "arrayContains").asBoolean();
  }
  arrayContainsAll(e2) {
    const t2 = Array.isArray(e2) ? new __PRIVATE_ListOfExprs(e2.map(__PRIVATE_valueToDefaultExpr), "arrayContainsAll") : e2;
    return new y("array_contains_all", [this, t2], "arrayContainsAll").asBoolean();
  }
  arrayContainsAny(e2) {
    const t2 = Array.isArray(e2) ? new __PRIVATE_ListOfExprs(e2.map(__PRIVATE_valueToDefaultExpr), "arrayContainsAny") : e2;
    return new y("array_contains_any", [this, t2], "arrayContainsAny").asBoolean();
  }
  arrayReverse() {
  ...
  equalAny(e2) {
    const t2 = Array.isArray(e2) ? new __PRIVATE_ListOfExprs(e2.map(__PRIVATE_valueToDefaultExpr), "equalAny") : e2;
    return new y("equal_any", [this, t2], "equalAny").asBoolean();
  }
  notEqualAny(e2) {
    const t2 = Array.isArray(e2) ? new __PRIVATE_ListOfExprs(e2.map(__PRIVATE_valueToDefaultExpr), "notEqualAny") : e2;
    return new y("not_equal_any", [this, t2], "notEqualAny").asBoolean();
  }
  exists() {
  ...
  like(e2) {
    return new y("like", [this, __PRIVATE_valueToDefaultExpr(e2)], "like").asBoolean();
  }
  regexContains(e2) {
    return new y("regex_contains", [this, __PRIVATE_valueToDefaultExpr(e2)], "regexContains").asBoolean();
  }
  regexFind(e2) {
    return new y("regex_find", [this, __PRIVATE_valueToDefaultExpr(e2)], "regexFind");
  }
  regexFindAll(e2) {
    return new y("regex_find_all", [this, __PRIVATE_valueToDefaultExpr(e2)], "regexFindAll");
  }
  regexMatch(e2) {
    return new y("regex_match", [this, __PRIVATE_valueToDefaultExpr(e2)], "regexMatch").asBoolean();
  }
  stringContains(e2) {
    return new y("string_contains", [this, __PRIVATE_valueToDefaultExpr(e2)], "stringContains").asBoolean();
  }
  startsWith(e2) {
    return new y("starts_with", [this, __PRIVATE_valueToDefaultExpr(e2)], "startsWith").asBoolean();
  }
  endsWith(e2) {
    return new y("ends_with", [this, __PRIVATE_valueToDefaultExpr(e2)], "endsWith").asBoolean();
  }
  toLower() {
  ...
  trim(e2) {
    const t2 = [this];
    return e2 && t2.push(__PRIVATE_valueToDefaultExpr(e2)), new y("trim", t2, "trim");
  }
  ltrim(e2) {
    const t2 = [this];
    return e2 && t2.push(__PRIVATE_valueToDefaultExpr(e2)), new y("ltrim", t2, "ltrim");
  }
  rtrim(e2) {
    const t2 = [this];
    return e2 && t2.push(__PRIVATE_valueToDefaultExpr(e2)), new y("rtrim", t2, "rtrim");
  }
  type() {
    return new y("type", [this]);
  }
  isType(e2) {
    return new y("is_type", [this, i(e2)], "isType").asBoolean();
  }
  stringConcat(e2, ...t2) {
    const n2 = [e2, ...t2].map(__PRIVATE_valueToDefaultExpr);
    return new y("string_concat", [this, ...n2], "stringConcat");
  }
  stringIndexOf(e2) {
    return new y("string_index_of", [this, __PRIVATE_valueToDefaultExpr(e2)], "stringIndexOf");
  }
  stringRepeat(e2) {
    return new y("string_repeat", [this, __PRIVATE_valueToDefaultExpr(e2)], "stringRepeat");
  }
  stringReplaceAll(e2, t2) {
    return new y("string_replace_all", [this, __PRIVATE_valueToDefaultExpr(e2), __PRIVATE_valueToDefaultExpr(t2)], "stringReplaceAll");
  }
  stringReplaceOne(e2, t2) {
    return new y("string_replace_one", [this, __PRIVATE_valueToDefaultExpr(e2), __PRIVATE_valueToDefaultExpr(t2)], "stringReplaceOne");
  }
  concat(e2, ...t2) {
    const n2 = [e2, ...t2].map(__PRIVATE_valueToDefaultExpr);
    return new y("concat", [this, ...n2], "concat");
  }
  reverse() {
    return new y("reverse", [this], "reverse");
  }
  arrayFilter(e2, t2) {
    return new y("array_filter", [this, __PRIVATE_valueToDefaultExpr(e2), t2], "arrayFilter");
  }
  arrayTransform(e2, t2) {
    return new y("array_transform", [this, __PRIVATE_valueToDefaultExpr(e2), t2], "arrayTransform");
  }
  arrayTransformWithIndex(e2, t2, n2) {
    return new y("array_transform", [this, __PRIVATE_valueToDefaultExpr(e2), __PRIVATE_valueToDefaultExpr(t2), n2], "arrayTransformWithIndex");
  }
  arraySlice(e2, t2) {
    const n2 = [this, __PRIVATE_valueToDefaultExpr(e2)];
    return t2 !== undefined && n2.push(__PRIVATE_valueToDefaultExpr(t2)), new y("array_slice", n2, "arraySlice");
  }
  arrayFirst() {
    return new y("array_first", [this], "arrayFirst");
  }
  arrayFirstN(e2) {
    return new y("array_first_n", [this, __PRIVATE_valueToDefaultExpr(e2)], "arrayFirstN");
  }
  arrayLast() {
    return new y("array_last", [this], "arrayLast");
  }
  arrayLastN(e2) {
    return new y("array_last_n", [this, __PRIVATE_valueToDefaultExpr(e2)], "arrayLastN");
  }
  arrayMaximum() {
    return new y("maximum", [this], "arrayMaximum");
  }
  arrayMaximumN(e2) {
    return new y("maximum_n", [this, __PRIVATE_valueToDefaultExpr(e2)], "arrayMaximumN");
  }
  arrayMinimum() {
    return new y("minimum", [this], "arrayMinimum");
  }
  arrayMinimumN(e2) {
    return new y("minimum_n", [this, __PRIVATE_valueToDefaultExpr(e2)], "arrayMinimumN");
  }
  arrayIndexOf(e2) {
    return new y("array_index_of", [this, __PRIVATE_valueToDefaultExpr(e2), __PRIVATE_valueToDefaultExpr("first")], "arrayIndexOf");
  }
  arrayLastIndexOf(e2) {
    return new y("array_index_of", [this, __PRIVATE_valueToDefaultExpr(e2), __PRIVATE_valueToDefaultExpr("last")], "arrayLastIndexOf");
  }
  arrayIndexOfAll(e2) {
    return new y("array_index_of_all", [this, __PRIVATE_valueToDefaultExpr(e2)], "arrayIndexOfAll");
  }
  byteLength() {
  ...
  mapGet(e2) {
    return new y("map_get", [this, i(e2)], "mapGet");
  }
  mapSet(e2, t2, ...n2) {
    const r2 = [this, __PRIVATE_valueToDefaultExpr(e2), __PRIVATE_valueToDefaultExpr(t2), ...n2.map(__PRIVATE_valueToDefaultExpr)];
    return new y("map_set", r2, "mapSet");
  }
  mapKeys() {
  ...
  getField(e2) {
    return new y("get_field", [this, __PRIVATE_valueToDefaultExpr(e2)], "get_field");
  }
  count() {
  ...
  logicalMaximum(e2, ...t2) {
    const n2 = [e2, ...t2];
    return new y("maximum", [this, ...n2.map(__PRIVATE_valueToDefaultExpr)], "logicalMaximum");
  }
  logicalMinimum(e2, ...t2) {
    const n2 = [e2, ...t2];
    return new y("minimum", [this, ...n2.map(__PRIVATE_valueToDefaultExpr)], "minimum");
  }
  vectorLength() {
    return new y("vector_length", [this], "vectorLength");
  }
  cosineDistance(e2) {
    return new y("cosine_distance", [this, __PRIVATE_vectorToExpr(e2)], "cosineDistance");
  }
  dotProduct(e2) {
    return new y("dot_product", [this, __PRIVATE_vectorToExpr(e2)], "dotProduct");
  }
  euclideanDistance(e2) {
    return new y("euclidean_distance", [this, __PRIVATE_vectorToExpr(e2)], "euclideanDistance");
  }
  ...
  timestampAdd(e2, t2) {
    return new y("timestamp_add", [this, __PRIVATE_valueToDefaultExpr(e2), __PRIVATE_valueToDefaultExpr(t2)], "timestampAdd");
  }
  timestampSubtract(e2, t2) {
    return new y("timestamp_subtract", [this, __PRIVATE_valueToDefaultExpr(e2), __PRIVATE_valueToDefaultExpr(t2)], "timestampSubtract");
  }
  timestampDiff(e2, t2) {
    return new y("timestamp_diff", [this, __PRIVATE_fieldOrExpression(e2), __PRIVATE_valueToDefaultExpr(t2)], "timestampDiff");
  }
  timestampExtract(e2, t2) {
    const n2 = [this, __PRIVATE_valueToDefaultExpr(e2)];
    return t2 && n2.push(__PRIVATE_valueToDefaultExpr(t2)), new y("timestamp_extract", n2, "timestampExtract");
  }
  documentId() {
  ...
  substring(e2, t2) {
    const n2 = __PRIVATE_valueToDefaultExpr(e2);
    return new y("substring", t2 === undefined ? [this, n2] : [this, n2, __PRIVATE_valueToDefaultExpr(t2)], "substring");
  }
  arrayGet(e2) {
    return new y("array_get", [this, __PRIVATE_valueToDefaultExpr(e2)], "arrayGet");
  }
  isError() {
    return new y("is_error", [this], "isError").asBoolean();
  }
  ifError(e2) {
    const t2 = new y("if_error", [this, __PRIVATE_valueToDefaultExpr(e2)], "ifError");
    return e2 instanceof BooleanExpression ? t2.asBoolean() : t2;
  }
  isAbsent() {
    return new y("is_absent", [this], "isAbsent").asBoolean();
  }
  mapRemove(e2) {
    return new y("map_remove", [this, __PRIVATE_valueToDefaultExpr(e2)], "mapRemove");
  }
  mapMerge(e2, ...t2) {
    const n2 = __PRIVATE_valueToDefaultExpr(e2), r2 = t2.map(__PRIVATE_valueToDefaultExpr);
    return new y("map_merge", [this, n2, ...r2], "mapMerge");
  }
  pow(e2) {
    return new y("pow", [this, __PRIVATE_valueToDefaultExpr(e2)]);
  }
  trunc(e2) {
    return e2 === undefined ? new y("trunc", [this]) : new y("trunc", [this, __PRIVATE_valueToDefaultExpr(e2)], "trunc");
  }
  round(e2) {
    return e2 === undefined ? new y("round", [this]) : new y("round", [this, __PRIVATE_valueToDefaultExpr(e2)], "round");
  }
  collectionId() {
  ...
  ifAbsent(e2) {
    return new y("if_absent", [this, __PRIVATE_valueToDefaultExpr(e2)], "ifAbsent");
  }
  ifNull(e2) {
    return new y("if_null", [this, __PRIVATE_valueToDefaultExpr(e2)], "ifNull");
  }
  coalesce(e2, ...t2) {
    return new y("coalesce", [this, __PRIVATE_valueToDefaultExpr(e2), ...t2.map(__PRIVATE_valueToDefaultExpr)], "coalesce");
  }
  join(e2) {
    return new y("join", [this, __PRIVATE_valueToDefaultExpr(e2)], "join");
  }
  log10() {
  ...
  split(e2) {
    return new y("split", [this, __PRIVATE_valueToDefaultExpr(e2)]);
  }
  timestampTruncate(e2, t2) {
    const n2 = [this, __PRIVATE_valueToDefaultExpr(e2)];
    return t2 && n2.push(__PRIVATE_valueToDefaultExpr(t2)), new y("timestamp_trunc", n2);
  }
  ascending() {
  ...
  as(e2) {
    return new c(this, e2, "as");
  }
}

class k {
  constructor(e2, t2) {
    this.name = e2, this.params = t2, this.exprType = "AggregateFunction", this._protoValueType = "ProtoValue";
  }
  static _create(e2, t2, n2) {
    const r2 = new k(e2, t2);
    return r2._methodName = n2, r2;
  }
  as(e2) {
    return new AliasedAggregate(this, e2, "as");
  }
  _toProto(e2) {
    return {
      functionValue: {
        name: this.name,
        args: this.params.map((t2) => t2._toProto(e2))
      }
    };
  }
  _readUserData(e2) {
    e2 = this._methodName ? e2.contextWith({
      methodName: this._methodName
    }) : e2, this.params.forEach((t2) => t2._readUserData(e2));
  }
}

class AliasedAggregate {
  constructor(e2, t2, n2) {
    this.aggregate = e2, this.alias = t2, this._methodName = n2;
  }
  _readUserData(e2) {
    this.aggregate._readUserData(e2);
  }
}

class c {
  constructor(e2, t2, n2) {
    this.expr = e2, this.alias = t2, this._methodName = n2, this.exprType = "AliasedExpression", this.selectable = true;
  }
  _readUserData(e2) {
    this.expr._readUserData(e2);
  }
}

class __PRIVATE_ListOfExprs extends t {
  constructor(e2, t2) {
    super(), this.cr = e2, this._methodName = t2, this.expressionType = "ListOfExpressions";
  }
  _toProto(e2) {
    return {
      arrayValue: {
        values: this.cr.map((t2) => t2._toProto(e2))
      }
    };
  }
  _readUserData(e2) {
    this.cr.forEach((t2) => t2._readUserData(e2));
  }
}

class o extends t {
  constructor(e2, t2) {
    super(), this.fieldPath = e2, this._methodName = t2, this.expressionType = "Field", this.selectable = true;
  }
  get _fieldPath() {
  ...
  geoDistance(e2) {
    return new y("geo_distance", [this, __PRIVATE_valueToDefaultExpr(e2)], "geoDistance");
  }
  _toProto(e2) {
    return {
...
function G(e2, t2) {
  return new o(typeof e2 == "string" ? Ce === e2 ? documentId$1()._internalPath : K("field", e2) : e2._internalPath, t2);
}

class Constant extends t {
  constructor(e2, t2) {
    super(), this.value = e2, this._methodName = t2, this.expressionType = "Constant";
  }
  static _fromProto(e2) {
    const t2 = new Constant(e2, undefined);
    return t2._protoValue = e2, t2;
  }
  _toProto(e2) {
    return __PRIVATE_hardAssert(this._protoValue !== undefined, 237), this._protoValue;
  }
  _getValue() {
  ...
  _readUserData(e2) {
    e2 = this._methodName ? e2.contextWith({
      methodName: this._methodName
    }) : e2, u(this._protoValue) || (this._protoValue = __PRIVATE_parseData(this.value, e2));
  }
}
function i(e2, t2) {
  return m(e2, "constant");
}
function m(e2, t2) {
  const n2 = new Constant(e2, t2);
  return typeof e2 == "boolean" ? new __PRIVATE_BooleanConstant(n2) : n2;
}
class y extends t {
  constructor(e2, t2, n2, r2) {
    super(), this.name = e2, this.params = t2, this.expressionType = "Function", this._optionsProto = undefined, n2 !== undefined && (this._methodName = n2), r2 !== undefined && (this._options = r2);
  }
  get _optionsUtil() {
  ...
  _toProto(e2) {
    const t2 = {
      functionValue: {
        name: this.name,
        args: this.params.map((t3) => t3._toProto(e2))
      }
    };
    return this._optionsProto && (t2.functionValue.options = this._optionsProto), t2;
  }
  _readUserData(e2) {
    e2 = this._methodName ? e2.contextWith({
      methodName: this._methodName
    }) : e2, this.params.forEach((t2) => t2._readUserData(e2)), this._options && (this._optionsProto = this._optionsUtil.getOptionsProto(e2, this._options));
  }
}

class BooleanExpression extends t {
  get _methodName() {
  ...
  conditional(e2, t2) {
    return new y("conditional", [this, e2, t2], "conditional");
  }
  ifError(e2) {
    const t2 = __PRIVATE_valueToDefaultExpr(e2), n2 = new y("if_error", [this, t2], "ifError");
    return t2 instanceof BooleanExpression ? n2.asBoolean() : n2;
  }
  _toProto(e2) {
    return this._expr._toProto(e2);
  }
  _readUserData(e2) {
    this._expr._readUserData(e2);
  }
}

class __PRIVATE_BooleanFunctionExpression extends BooleanExpression {
  constructor(e2) {
    super(), this._expr = e2, this.expressionType = "Function";
  }
}

class __PRIVATE_BooleanConstant extends BooleanExpression {
  constructor(e2) {
    super(), this._expr = e2, this.expressionType = "Constant";
  }
  _getValue() {
...
class __PRIVATE_BooleanField extends BooleanExpression {
  constructor(e2) {
    super(), this._expr = e2, this.expressionType = "Field";
  }
}
function __PRIVATE__map(e2, t2) {
  const n2 = [];
  for (const t3 in e2)
    if (Object.prototype.hasOwnProperty.call(e2, t3)) {
      const r2 = e2[t3];
      n2.push(i(t3)), n2.push(__PRIVATE_valueToDefaultExpr(r2));
    }
  return new y("map", n2, "map");
}
function b(e2) {
  return function __PRIVATE__array(e3, t2) {
    return new y("array", e3.map((e4) => __PRIVATE_valueToDefaultExpr(e4)), t2);
  }(e2, "array");
}
function ascending(e2) {
  return new Ordering(__PRIVATE_fieldOrExpression(e2), "ascending", "ascending");
}
function descending(e2) {
  return new Ordering(__PRIVATE_fieldOrExpression(e2), "descending", "descending");
}

class Ordering {
  constructor(e2, t2, n2) {
    this.expr = e2, this.direction = t2, this._methodName = n2, this._protoValueType = "ProtoValue";
  }
  _toProto(e2) {
    return {
      mapValue: {
        fields: {
          ...
          expression: this.expr._toProto(e2)
        }
      }
    };
  }
...
class Stage {
  constructor(e2) {
    this.optionsProto = undefined, { rawOptions: this.rawOptions, ...this.knownOptions } = e2;
  }
  _readUserData(e2) {
    this.optionsProto = this._optionsUtil.getOptionsProto(e2, this.knownOptions, this.rawOptions);
  }
  _toProto(e2) {
    return {
      name: this._name,
...
class w extends Stage {
  ...
  constructor(e2, t2) {
    super(t2), this.fields = e2;
  }
  _toProto(e2) {
    return {
      ...super._toProto(e2),
      args: [toMapValue(e2, this.fields)]
    };
  }
  _readUserData(e2) {
    super._readUserData(e2), __PRIVATE_readUserDataHelper(this.fields, e2);
  }
}
class O extends Stage {
  get _name() {
    return "aggregate";
  }
  get _optionsUtil() {
  ...
  constructor(e2, t2, n2) {
    super(n2), this.groups = e2, this.accumulators = t2;
  }
  _toProto(e2) {
    return {
      ...super._toProto(e2),
      args: [toMapValue(e2, this.accumulators), toMapValue(e2, this.groups)]
    };
  }
  _readUserData(e2) {
    super._readUserData(e2), __PRIVATE_readUserDataHelper(this.groups, e2), __PRIVATE_readUserDataHelper(this.accumulators, e2);
  }
}

class M extends Stage {
  get _name() {
    return "distinct";
  }
  get _optionsUtil() {
  ...
  constructor(e2, t2) {
    super(t2), this.groups = e2;
  }
  _toProto(e2) {
    return {
      ...super._toProto(e2),
      args: [toMapValue(e2, this.groups)]
    };
  }
  _readUserData(e2) {
    super._readUserData(e2), __PRIVATE_readUserDataHelper(this.groups, e2);
  }
}
...
class X extends Stage {
  ...
  constructor(e2, t2) {
    super(t2), this.hr = e2.startsWith("/") ? e2 : "/" + e2;
  }
  _toProto(e2) {
    return {
      ...super._toProto(e2),
  ...
  _readUserData(e2) {
    super._readUserData(e2);
  }
}

class Y extends Stage {
  get _name() {
...
class sa extends Stage {
  ...
  constructor(t2, n2) {
    if (super(n2), !t2 || t2.length === 0)
      throw new e(ta.INVALID_ARGUMENT, "Empty document paths are not allowed in DocumentsSource");
    const r2 = t2.map((e2) => e2.startsWith("/") ? e2 : "/" + e2), i2 = new Set(r2);
    if (i2.size !== r2.length)
      throw new e(ta.INVALID_ARGUMENT, "Duplicate document paths are not allowed in DocumentsSource");
    this.Tr = r2, this.Pr = i2;
  }
  _toProto(e2) {
    return {
      ...super._toProto(e2),
...
class A extends Stage {
  ...
  _toProto(e2) {
    return {
      ...super._toProto(e2),
      args: [this.condition._toProto(e2)]
    };
  }
  _readUserData(e2) {
    super._readUserData(e2), __PRIVATE_readUserDataHelper(this.condition, e2);
  }
}
class v extends Stage {
  get _name() {
    return "limit";
  }
  get _optionsUtil() {
  ...
  constructor(e2, t2) {
    __PRIVATE_hardAssert(!isNaN(e2) && e2 !== 1 / 0 && e2 !== -1 / 0, 34860), super(t2), this.limit = e2;
  }
  _toProto(e2) {
    return {
      ...super._toProto(e2),
      args: [toNumber(e2, this.limit)]
    };
  }
}
...
class x extends Stage {
  ...
  constructor(e2, t2) {
    super(t2), this.selections = e2;
  }
  _toProto(e2) {
    return {
      ...super._toProto(e2),
      args: [toMapValue(e2, this.selections)]
    };
  }
  _readUserData(e2) {
    super._readUserData(e2), __PRIVATE_readUserDataHelper(this.selections, e2);
  }
}
...
class L extends Stage {
  ...
  constructor(e2, t2) {
    super(t2), this.orderings = e2;
  }
  _toProto(e2) {
    return {
      ...super._toProto(e2),
      args: this.orderings.map((t2) => t2._toProto(e2))
    };
  }
  _readUserData(e2) {
    super._readUserData(e2), __PRIVATE_readUserDataHelper(this.orderings, e2);
  }
}
class $ extends Stage {
  get _name() {
  ...
  constructor(e2, t2) {
    super(t2), this.map = e2;
  }
  _toProto(e2) {
    return {
      ...super._toProto(e2),
      args: [this.map._toProto(e2), __PRIVATE_toStringValue($.Ir)]
    };
  }
  _readUserData(e2) {
    super._readUserData(e2), __PRIVATE_readUserDataHelper(this.map, e2);
  }
}
$.Ir = "full_replace";
function __PRIVATE_readUserDataHelper(e2, t2) {
  return __PRIVATE_isUserData(e2) ? e2._readUserData(t2) : Array.isArray(e2) ? e2.forEach((e3) => e3._readUserData(t2)) : e2 instanceof Map ? e2.forEach((e3) => e3._readUserData(t2)) : Object.values(e2).forEach((e3) => e3._readUserData(t2)), e2;
}

class __PRIVATE_RealtimePipeline {
  constructor(e2, t2, n2, r2) {
    this._db = e2, this.userDataReader = t2, this._userDataWriter = n2, this.stages = r2;
  }
  Vr(e2, t2) {
    const n2 = this.userDataReader.createContext(3, e2);
    return __PRIVATE_isUserData(t2) ? t2._readUserData(n2) : Array.isArray(t2) ? t2.forEach((e3) => e3._readUserData(n2)) : t2.forEach((e3) => e3._readUserData(n2)), t2;
  }
  where(e2) {
    const t2 = this.stages.map((e3) => e3);
    return this.Vr("where", e2), t2.push(new A(e2, {})), new __PRIVATE_RealtimePipeline(this._db, this.userDataReader, this._userDataWriter, t2);
  }
  limit(e2) {
    const t2 = this.stages.map((e3) => e3);
    return t2.push(new v(e2, {})), new __PRIVATE_RealtimePipeline(this._db, this.userDataReader, this._userDataWriter, t2);
  }
  sort(e2, ...t2) {
    const n2 = this.stages.map((e3) => e3);
    return "orderings" in e2 ? n2.push(new L(this.Vr("sort", e2.orderings), {})) : n2.push(new L(this.Vr("sort", [e2, ...t2]), {})), new __PRIVATE_RealtimePipeline(this._db, this.userDataReader, this._userDataWriter, n2);
  }
  dr(e2) {
    return {
      pipeline: {
        stages: this.stages.map((t2) => t2._toProto(e2))
      }
    };
  }
}
...
class CorePipeline {
  constructor(e2, t2, n2) {
    this.serializer = e2, this.stages = t2, this.listenOptions = n2, this.isCorePipeline = true;
  }
  ...
  getPipelineFlavor() {
    return function getPipelineFlavor(e2) {
      let t2 = "exact";
      return e2.stages.forEach((n2, r2) => {
        n2._name !== M.name && n2._name !== O.name || (t2 = "keyless"), n2._name === x.name && t2 === "exact" && (t2 = "augmented"), n2._name === w.name && r2 < e2.stages.length - 1 && t2 === "exact" && (t2 = "augmented");
      }), t2;
    }(this);
  }
...
function getPipelineSourceType(e2) {
  const t2 = e2.stages[0];
  return t2 instanceof X || t2 instanceof Y || t2 instanceof Z || t2 instanceof sa ? t2._name : "unknown";
}
function getPipelineCollection(e2) {
  if (getPipelineSourceType(e2) === "collection")
    return e2.stages[0].hr;
}
function getPipelineCollectionGroup(e2) {
  if (getPipelineSourceType(e2) === "collection_group")
    return e2.stages[0].collectionId;
}
function getPipelineCollectionId(e2) {
  switch (getPipelineSourceType(e2)) {
    case "collection":
      return ResourcePath.fromString(getPipelineCollection(e2)).lastSegment();
    case "collection_group":
      return getPipelineCollectionGroup(e2);
    default:
      return;
  }
}
function getPipelineDocuments(e2) {
  if (getPipelineSourceType(e2) === "documents")
    return e2.stages[0].Tr;
}
...
class __PRIVATE_EvaluateResult {
  ...
  static mr() {
    return new __PRIVATE_EvaluateResult("ERROR", undefined);
  }
  static pr() {
    return new __PRIVATE_EvaluateResult("UNSET", undefined);
  }
  static gr() {
    return new __PRIVATE_EvaluateResult("NULL", He);
  }
  static newValue(e2) {
    return __PRIVATE_isNullValue(e2) ? new __PRIVATE_EvaluateResult("NULL", He) : function __PRIVATE_isBoolean(e3) {
      return !!e3 && "booleanValue" in e3;
    }(e2) ? new __PRIVATE_EvaluateResult("BOOLEAN", e2) : isInteger(e2) ? new __PRIVATE_EvaluateResult("INT", e2) : __PRIVATE_isDouble(e2) ? new __PRIVATE_EvaluateResult("DOUBLE", e2) : function __PRIVATE_isTimestampValue(e3) {
      return !!e3 && "timestampValue" in e3 && !!e3.timestampValue;
    }(e2) ? new __PRIVATE_EvaluateResult("TIMESTAMP", e2) : function __PRIVATE_isString(e3) {
      return !!e3 && "stringValue" in e3;
    }(e2) ? new __PRIVATE_EvaluateResult("STRING", e2) : function __PRIVATE_isBytes(e3) {
      return !!e3 && "bytesValue" in e3;
    }(e2) ? new __PRIVATE_EvaluateResult("BYTES", e2) : e2.referenceValue ? new __PRIVATE_EvaluateResult("REFERENCE", e2) : e2.geoPointValue ? new __PRIVATE_EvaluateResult("GEO_POINT", e2) : isArray(e2) ? new __PRIVATE_EvaluateResult("ARRAY", e2) : __PRIVATE_isVectorValue(e2) ? new __PRIVATE_EvaluateResult("VECTOR", e2) : __PRIVATE_isMapValue(e2) ? new __PRIVATE_EvaluateResult("MAP", e2) : new __PRIVATE_EvaluateResult("ERROR", undefined);
  }
  yr() {
...
function __PRIVATE_valueOrUndefined(e2) {
  if (!e2.yr())
    return e2.value;
}
function __PRIVATE_unwrapExpression(e2) {
  return e2 instanceof BooleanExpression ? e2._expr : e2;
}
function __PRIVATE_toEvaluable(e2) {
  if ((e2 = __PRIVATE_unwrapExpression(e2)) instanceof o)
    return new __PRIVATE_CoreField(e2);
  if (e2 instanceof Constant)
    return new __PRIVATE_CoreConstant(e2);
  if (e2 instanceof __PRIVATE_ListOfExprs)
    return new __PRIVATE_CoreListOfExprs(e2);
  if (e2 instanceof y) {
    if (e2.name === "add")
      return new __PRIVATE_CoreAdd(e2);
    if (e2.name === "subtract")
      return new __PRIVATE_CoreSubtract(e2);
    if (e2.name === "multiply")
      return new __PRIVATE_CoreMultiply(e2);
    if (e2.name === "divide")
      return new __PRIVATE_CoreDivide(e2);
    if (e2.name === "mod")
      return new __PRIVATE_CoreMod(e2);
    if (e2.name === "and")
      return new __PRIVATE_CoreAnd(e2);
    if (e2.name === "equal")
      return new __PRIVATE_CoreEq(e2);
    if (e2.name === "not_equal")
      return new __PRIVATE_CoreNeq(e2);
    if (e2.name === "less_than")
      return new __PRIVATE_CoreLt(e2);
    if (e2.name === "less_than_or_equal")
      return new __PRIVATE_CoreLte(e2);
    if (e2.name === "greater_than")
      return new __PRIVATE_CoreGt(e2);
    if (e2.name === "greater_than_or_equal")
      return new __PRIVATE_CoreGte(e2);
    if (e2.name === "array_concat")
      return new __PRIVATE_CoreArrayConcat(e2);
    if (e2.name === "array_reverse")
      return new __PRIVATE_CoreArrayReverse(e2);
    if (e2.name === "array_contains")
      return new __PRIVATE_CoreArrayContains(e2);
    if (e2.name === "array_contains_all")
      return new __PRIVATE_CoreArrayContainsAll(e2);
    if (e2.name === "array_contains_any")
      return new __PRIVATE_CoreArrayContainsAny(e2);
    if (e2.name === "array_length")
      return new __PRIVATE_CoreArrayLength(e2);
    if (e2.name === "array_element")
      return new __PRIVATE_CoreArrayElement(e2);
    if (e2.name === "equal_any")
      return new __PRIVATE_CoreEqAny(e2);
    if (e2.name === "not_equal_any")
      return new __PRIVATE_CoreNotEqAny(e2);
    if (e2.name === "is_nan")
      return new __PRIVATE_CoreIsNan(e2);
    if (e2.name === "is_not_nan")
      return new __PRIVATE_CoreIsNotNan(e2);
    if (e2.name === "is_null")
      return new __PRIVATE_CoreIsNull(e2);
    if (e2.name === "is_not_null")
      return new __PRIVATE_CoreIsNotNull(e2);
    if (e2.name === "is_error")
      return new __PRIVATE_CoreIsError(e2);
    if (e2.name === "exists")
      return new __PRIVATE_CoreExists(e2);
    if (e2.name === "not")
      return new __PRIVATE_CoreNot(e2);
    if (e2.name === "or")
      return new __PRIVATE_CoreOr(e2);
    if (e2.name === "xor")
      return new __PRIVATE_CoreXor(e2);
    if (e2.name === "conditional")
      return new __PRIVATE_CoreCond(e2);
    if (e2.name === "maximum")
      return new __PRIVATE_CoreLogicalMaximum(e2);
    if (e2.name === "minimum")
      return new __PRIVATE_CoreLogicalMinimum(e2);
    if (e2.name === "reverse")
      return new __PRIVATE_CoreReverse(e2);
    if (e2.name === "replace_first")
      return new __PRIVATE_CoreReplaceFirst(e2);
    if (e2.name === "replace_all")
      return new __PRIVATE_CoreReplaceAll(e2);
    if (e2.name === "char_length")
      return new __PRIVATE_CoreCharLength(e2);
    if (e2.name === "byte_length")
      return new __PRIVATE_CoreByteLength(e2);
    if (e2.name === "like")
      return new __PRIVATE_CoreLike(e2);
    if (e2.name === "regex_contains")
      return new __PRIVATE_CoreRegexContains(e2);
    if (e2.name === "regex_match")
      return new __PRIVATE_CoreRegexMatch(e2);
    if (e2.name === "string_contains")
      return new __PRIVATE_CoreStrContains(e2);
    if (e2.name === "starts_with")
      return new __PRIVATE_CoreStartsWith(e2);
    if (e2.name === "ends_with")
      return new __PRIVATE_CoreEndsWith(e2);
    if (e2.name === "to_lower")
      return new __PRIVATE_CoreToLower(e2);
    if (e2.name === "to_upper")
      return new __PRIVATE_CoreToUpper(e2);
    if (e2.name === "trim")
      return new __PRIVATE_CoreTrim(e2);
    if (e2.name === "string_concat")
      return new __PRIVATE_CoreStrConcat(e2);
    if (e2.name === "map_get")
      return new __PRIVATE_CoreMapGet(e2);
    if (e2.name === "cosine_distance")
      return new __PRIVATE_CoreCosineDistance(e2);
    if (e2.name === "dot_product")
      return new __PRIVATE_CoreDotProduct(e2);
    if (e2.name === "euclidean_distance")
      return new __PRIVATE_CoreEuclideanDistance(e2);
    if (e2.name === "vector_length")
      return new __PRIVATE_CoreVectorLength(e2);
    if (e2.name === "unix_micros_to_timestamp")
      return new __PRIVATE_CoreUnixMicrosToTimestamp(e2);
    if (e2.name === "timestamp_to_unix_micros")
      return new __PRIVATE_CoreTimestampToUnixMicros(e2);
    if (e2.name === "unix_millis_to_timestamp")
      return new __PRIVATE_CoreUnixMillisToTimestamp(e2);
    if (e2.name === "timestamp_to_unix_millis")
      return new __PRIVATE_CoreTimestampToUnixMillis(e2);
    if (e2.name === "unix_seconds_to_timestamp")
      return new __PRIVATE_CoreUnixSecondsToTimestamp(e2);
    if (e2.name === "timestamp_to_unix_seconds")
      return new __PRIVATE_CoreTimestampToUnixSeconds(e2);
    if (e2.name === "timestamp_add")
      return new __PRIVATE_CoreTimestampAdd(e2);
    if (e2.name === "timestamp_subtract")
      return new __PRIVATE_CoreTimestampSub(e2);
  }
  throw new Error(`Unknown Expr : ${e2}`);
}
...
class __PRIVATE_CoreField {
  ...
  evaluate(e2, t2) {
    if (this.expr.fieldName === Ce)
      return __PRIVATE_EvaluateResult.newValue({
        referenceValue: __PRIVATE_toName(e2.serializer, t2.key)
      });
    if (this.expr.fieldName === "__update_time__")
      return __PRIVATE_EvaluateResult.newValue({
        timestampValue: __PRIVATE_toVersion(e2.serializer, t2.version)
      });
    if (this.expr.fieldName === "__create_time__")
      return __PRIVATE_EvaluateResult.newValue({
        timestampValue: __PRIVATE_toVersion(e2.serializer, t2.createTime)
      });
    const n2 = t2.data.field(this.expr._fieldPath);
    return n2 ? __PRIVATE_isServerTimestamp(n2) ? __PRIVATE_EvaluateResult.newValue(function __PRIVATE_getServerTimestampValue(e3, t3) {
      if (e3.serverTimestampBehavior === "estimate")
        return {
          timestampValue: __PRIVATE_toVersion(e3.serializer, SnapshotVersion.fromTimestamp(__PRIVATE_getLocalWriteTime(t3)))
        };
      if (e3.serverTimestampBehavior === "previous") {
        const e4 = __PRIVATE_getPreviousValue(t3);
        if (e4)
          return e4;
      }
      return {
    ...
    }(e2, n2)) : __PRIVATE_EvaluateResult.newValue(n2) : __PRIVATE_EvaluateResult.pr();
  }
}
...
class __PRIVATE_CoreListOfExprs {
  ...
  evaluate(e2, t2) {
    const n2 = this.expr.cr.map((n3) => __PRIVATE_toEvaluable(n3).evaluate(e2, t2));
    return n2.some((e3) => e3.yr()) ? __PRIVATE_EvaluateResult.mr() : __PRIVATE_EvaluateResult.newValue({
      arrayValue: {
        values: n2.map((e3) => e3.value)
      }
    });
  }
}
function __PRIVATE_asDouble(e2) {
  return __PRIVATE_isDouble(e2) ? Number(e2.doubleValue) : Number(e2.integerValue);
}
function __PRIVATE_asBigInt(e2) {
  return BigInt(e2.integerValue);
}
...
class __PRIVATE_BigIntOrDoubleArithmetics {
  ...
  evaluate(e2, t2) {
    ...
    const n2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2), r2 = __PRIVATE_toEvaluable(this.expr.params[1]).evaluate(e2, t2);
    let i2 = this.br(n2, r2);
    for (const n3 of this.expr.params.slice(2)) {
      const r3 = __PRIVATE_toEvaluable(n3).evaluate(e2, t2);
      i2 = this.br(i2, r3);
    }
    return i2;
  }
  br(e2, t2) {
    ...
    if (e2.wr() || t2.wr())
      return __PRIVATE_EvaluateResult.gr();
    const n2 = e2.value, r2 = t2.value;
    if (!__PRIVATE_isDouble(n2) && !isInteger(n2) || !__PRIVATE_isDouble(r2) && !isInteger(r2))
      return __PRIVATE_EvaluateResult.mr();
    if (__PRIVATE_isDouble(n2) || __PRIVATE_isDouble(r2)) {
      const e3 = this.Sr(n2, r2);
      return e3 ? __PRIVATE_EvaluateResult.newValue(e3) : __PRIVATE_EvaluateResult.mr();
    }
    if (isInteger(n2) && isInteger(r2)) {
      const e3 = this.vr(n2, r2);
      return e3 === undefined ? __PRIVATE_EvaluateResult.mr() : typeof e3 == "number" ? __PRIVATE_EvaluateResult.newValue({
        doubleValue: e3
      }) : e3 < Ot || e3 > Ft ? __PRIVATE_EvaluateResult.mr() : __PRIVATE_EvaluateResult.newValue({
        integerValue: `${e3}`
      });
    }
    return __PRIVATE_EvaluateResult.mr();
...
function __PRIVATE_strictValueEquals(e2, t2) {
  return __PRIVATE_typeOrder(e2) !== __PRIVATE_typeOrder(t2) ? "TYPE_MISMATCH" : __PRIVATE_isNanValue(e2) || __PRIVATE_isNanValue(t2) ? "NOT_EQ" : __PRIVATE_isNullValue(e2) && __PRIVATE_isNullValue(t2) ? "EQ" : __PRIVATE_isNullValue(e2) || __PRIVATE_isNullValue(t2) ? "NULL" : isArray(e2) && isArray(t2) ? function __PRIVATE_strictArrayValueEquals(e3, t3) {
    if (e3.values?.length !== t3.values?.length)
      return "NOT_EQ";
    let n2 = false;
    for (let r2 = 0;r2 < (e3.values?.length ?? 0); r2++) {
      const i2 = e3.values[r2], s2 = t3.values[r2];
      switch (__PRIVATE_strictValueEquals(i2, s2)) {
        case "EQ":
          break;
        case "NOT_EQ":
        case "TYPE_MISMATCH":
    ...
    if (n2)
      return "NULL";
    return "EQ";
  }(e2.arrayValue, t2.arrayValue) : __PRIVATE_isVectorValue(e2) && __PRIVATE_isVectorValue(t2) || __PRIVATE_isMapValue(e2) && __PRIVATE_isMapValue(t2) ? function __PRIVATE_strictObjectValueEquals(e3, t3) {
    const n2 = e3.fields || {}, r2 = t3.fields || {};
    if (__PRIVATE_objectSize(n2) !== __PRIVATE_objectSize(r2))
      return "NOT_EQ";
    let i2 = false;
    for (const e4 in n2)
      if (n2.hasOwnProperty(e4)) {
        if (r2[e4] === undefined)
          return "NOT_EQ";
        switch (__PRIVATE_strictValueEquals(n2[e4], r2[e4])) {
          case "NOT_EQ":
          case "TYPE_MISMATCH":
            return "NOT_EQ";
          case "NULL":
    ...
    if (i2)
      return "NULL";
    return "EQ";
  }(e2.mapValue, t2.mapValue) : function __PRIVATE_valueEquals(e3, t3) {
    return __PRIVATE_valueEquals$1(e3, t3, {
      u: false,
      i: true,
      o: true
    });
...
class __PRIVATE_CoreAdd extends __PRIVATE_BigIntOrDoubleArithmetics {
  vr(e2, t2) {
    return __PRIVATE_asBigInt(e2) + __PRIVATE_asBigInt(t2);
  }
  Sr(e2, t2) {
    return {
      doubleValue: __PRIVATE_asDouble(e2) + __PRIVATE_asDouble(t2)
    };
  }
}
...
class __PRIVATE_CoreDivide extends __PRIVATE_BigIntOrDoubleArithmetics {
  ...
  vr(e2, t2) {
    const n2 = __PRIVATE_asBigInt(t2);
    if (n2 !== BigInt(0))
      return __PRIVATE_asBigInt(e2) / n2;
  }
  Sr(e2, t2) {
    const n2 = __PRIVATE_asDouble(t2);
    return n2 === 0 ? {
      doubleValue: __PRIVATE_isNegativeZero(n2) ? Number.NEGATIVE_INFINITY : Number.POSITIVE_INFINITY
    } : {
      doubleValue: __PRIVATE_asDouble(e2) / n2
    };
  }
}
...
class __PRIVATE_CoreAnd {
  ...
  evaluate(e2, t2) {
    ...
    for (const i2 of this.expr.params) {
      const s2 = __PRIVATE_toEvaluable(i2).evaluate(e2, t2);
      switch (s2.type) {
        case "BOOLEAN":
          if (!s2.value?.booleanValue)
            return __PRIVATE_EvaluateResult.newValue(Ye);
          break;
        case "NULL":
          r2 = true;
          break;
    ...
    return n2 ? __PRIVATE_EvaluateResult.mr() : r2 ? __PRIVATE_EvaluateResult.gr() : __PRIVATE_EvaluateResult.newValue(Je);
  }
}
...
class __PRIVATE_CoreNot {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 1, 9634);
    const n2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (n2.type) {
      case "BOOLEAN":
        return __PRIVATE_EvaluateResult.newValue({
          booleanValue: !n2.value?.booleanValue
        });
      case "NULL":
        return __PRIVATE_EvaluateResult.gr();
      default:
...
class __PRIVATE_CoreOr {
  ...
  evaluate(e2, t2) {
    ...
    return n2 ? __PRIVATE_EvaluateResult.mr() : r2 ? __PRIVATE_EvaluateResult.gr() : __PRIVATE_EvaluateResult.newValue(Ye);
  }
}
...
class __PRIVATE_CoreXor {
  ...
  evaluate(e2, t2) {
    ...
    for (const i2 of this.expr.params) {
      ...
      switch (s2.type) {
        case "BOOLEAN":
          n2 = __PRIVATE_CoreXor.xor(n2, !!s2.value?.booleanValue);
          break;
        case "NULL":
          r2 = true;
          break;
    ...
    return r2 ? __PRIVATE_EvaluateResult.gr() : __PRIVATE_EvaluateResult.newValue({
      booleanValue: n2
    });
  }
  static xor(e2, t2) {
...
class __PRIVATE_CoreEqAny {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 2, 55094);
    let n2 = false;
    const r2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (r2.type) {
      case "NULL":
        n2 = true;
        break;
    ...
    const i2 = __PRIVATE_toEvaluable(this.expr.params[1]).evaluate(e2, t2);
    switch (i2.type) {
      case "ARRAY":
        break;
      case "NULL":
    ...
    if (n2)
      return __PRIVATE_EvaluateResult.gr();
    for (const e3 of i2.value?.arrayValue?.values ?? []) {
      switch (__PRIVATE_isNullValue(r2.value) && __PRIVATE_isNullValue(e3) ? "EQ" : __PRIVATE_strictValueEquals(r2.value, e3)) {
        case "EQ":
          return __PRIVATE_EvaluateResult.newValue(Je);
        case "NOT_EQ":
        case "TYPE_MISMATCH":
    ...
    return n2 ? __PRIVATE_EvaluateResult.gr() : __PRIVATE_EvaluateResult.newValue(Ye);
  }
}
...
class __PRIVATE_CoreNotEqAny {
  ...
  evaluate(e2, t2) {
    return new __PRIVATE_CoreNot(new y("not", [new y("equal_any", this.expr.params)])).evaluate(e2, t2);
  }
}
...
class __PRIVATE_CoreIsNan {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 1, 23322);
    const n2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (n2.type) {
      case "INT":
        return __PRIVATE_EvaluateResult.newValue(Ye);
      case "DOUBLE":
        return __PRIVATE_EvaluateResult.newValue({
          booleanValue: isNaN(__PRIVATE_asDouble(n2.value))
        });
      case "NULL":
        return __PRIVATE_EvaluateResult.gr();
      default:
...
class __PRIVATE_CoreIsNotNan {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 1, 50406);
    return new __PRIVATE_CoreNot(new y("not", [new y("is_nan", this.expr.params)])).evaluate(e2, t2);
  }
}
...
class __PRIVATE_CoreIsNull {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 1, 23123);
    switch (__PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2).type) {
      case "NULL":
        return __PRIVATE_EvaluateResult.newValue(Je);
      case "UNSET":
      case "ERROR":
...
class __PRIVATE_CoreIsNotNull {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 1, 23167);
    return new __PRIVATE_CoreNot(new y("not", [new y("is_null", this.expr.params)])).evaluate(e2, t2);
  }
}
...
class __PRIVATE_CoreIsError {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 1, 5228);
    return __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2).type === "ERROR" ? __PRIVATE_EvaluateResult.newValue(Je) : __PRIVATE_EvaluateResult.newValue(Ye);
  }
}
...
class __PRIVATE_CoreExists {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 1, 6877);
    switch (__PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2).type) {
      case "ERROR":
        return __PRIVATE_EvaluateResult.mr();
      case "UNSET":
...
class __PRIVATE_CoreCond {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 3, 11706);
    const n2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (n2.type) {
      case "BOOLEAN":
        return n2.value?.booleanValue ? __PRIVATE_toEvaluable(this.expr.params[1]).evaluate(e2, t2) : __PRIVATE_toEvaluable(this.expr.params[2]).evaluate(e2, t2);
      case "NULL":
        return __PRIVATE_toEvaluable(this.expr.params[2]).evaluate(e2, t2);
      default:
        return __PRIVATE_EvaluateResult.mr();
    }
  }
...
class __PRIVATE_CoreLogicalMaximum {
  ...
  evaluate(e2, t2) {
    const n2 = this.expr.params.map((n3) => __PRIVATE_toEvaluable(n3).evaluate(e2, t2));
    let r2;
    for (const e3 of n2)
      switch (e3.type) {
        case "ERROR":
        ...
        case "NULL":
          continue;
        default:
          r2 = r2 === undefined || __PRIVATE_valueCompare(e3.value, r2.value) > 0 ? e3 : r2;
      }
    return r2 === undefined ? __PRIVATE_EvaluateResult.gr() : r2;
  }
}
...
class __PRIVATE_ComparisonBase {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 2, 31033, `${this.expr.name}() function should have exactly 2 params`);
    const n2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (n2.type) {
      case "ERROR":
      case "UNSET":
    ...
    const r2 = __PRIVATE_toEvaluable(this.expr.params[1]).evaluate(e2, t2);
    switch (r2.type) {
      case "ERROR":
      case "UNSET":
        return __PRIVATE_EvaluateResult.mr();
    }
    return this.Fr(n2, r2);
  }
}
...
class __PRIVATE_CoreEq extends __PRIVATE_ComparisonBase {
  ...
  Fr(e2, t2) {
    ...
    if (e2.wr() || t2.wr())
      return __PRIVATE_EvaluateResult.newValue(Ye);
    if (__PRIVATE_isNanValue(e2.value) || __PRIVATE_isNanValue(t2.value))
      return __PRIVATE_EvaluateResult.newValue(Ye);
    if (__PRIVATE_typeOrder(e2.value) !== __PRIVATE_typeOrder(t2.value))
      return __PRIVATE_EvaluateResult.newValue(Ye);
    switch (__PRIVATE_strictValueEquals(e2.value, t2.value)) {
      case "EQ":
        return __PRIVATE_EvaluateResult.newValue(Je);
      case "NOT_EQ":
        return __PRIVATE_EvaluateResult.newValue(Ye);
...
class __PRIVATE_CoreLt extends __PRIVATE_ComparisonBase {
  ...
  Fr(e2, t2) {
    return __PRIVATE_typeOrder(e2.value) !== __PRIVATE_typeOrder(t2.value) || __PRIVATE_isNanValue(e2.value) || __PRIVATE_isNanValue(t2.value) ? __PRIVATE_EvaluateResult.newValue(Ye) : __PRIVATE_EvaluateResult.newValue({
      booleanValue: __PRIVATE_valueCompare(e2.value, t2.value) < 0
    });
  }
}
...
class __PRIVATE_CoreLte extends __PRIVATE_ComparisonBase {
  ...
  Fr(e2, t2) {
    return __PRIVATE_typeOrder(e2.value) !== __PRIVATE_typeOrder(t2.value) || __PRIVATE_isNanValue(e2.value) || __PRIVATE_isNanValue(t2.value) ? __PRIVATE_EvaluateResult.newValue(Ye) : __PRIVATE_strictValueEquals(e2.value, t2.value) === "EQ" ? __PRIVATE_EvaluateResult.newValue(Je) : __PRIVATE_EvaluateResult.newValue({
      booleanValue: __PRIVATE_valueCompare(e2.value, t2.value) < 0
    });
  }
}
...
class __PRIVATE_CoreArrayReverse {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 1, 216);
    const n2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (n2.type) {
      case "NULL":
        return __PRIVATE_EvaluateResult.gr();
      case "ARRAY": {
        const e3 = n2.value.arrayValue?.values ?? [];
        return __PRIVATE_EvaluateResult.newValue({
          arrayValue: {
            values: [...e3].reverse()
          }
...
class __PRIVATE_CoreArrayContains {
  ...
  evaluate(e2, t2) {
    return __PRIVATE_hardAssert(this.expr.params.length === 2, 52884), new __PRIVATE_CoreEqAny(new y("eq_any", [this.expr.params[1], this.expr.params[0]])).evaluate(e2, t2);
  }
}
...
class __PRIVATE_CoreArrayContainsAll {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 2, 1392);
    let n2 = false;
    const r2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (r2.type) {
      case "ARRAY":
    ...
    if (n2)
      return __PRIVATE_EvaluateResult.gr();
    const s2 = i2.value?.arrayValue?.values ?? [], _ = r2.value?.arrayValue?.values ?? [];
    for (const e3 of s2) {
      let t3 = false;
      n2 = false;
      for (const r3 of _) {
        switch (__PRIVATE_isNullValue(e3) && __PRIVATE_isNullValue(r3) ? "EQ" : __PRIVATE_strictValueEquals(e3, r3)) {
          case "EQ":
            t3 = true;
            break;
          case "NOT_EQ":
...
class __PRIVATE_CoreArrayContainsAny {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 2, 2680);
    let n2 = false;
    const r2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (r2.type) {
      case "ARRAY":
    ...
    for (const e3 of _)
      for (const t3 of s2) {
        switch (__PRIVATE_isNullValue(e3) && __PRIVATE_isNullValue(t3) ? "EQ" : __PRIVATE_strictValueEquals(e3, t3)) {
          case "EQ":
            return __PRIVATE_EvaluateResult.newValue(Je);
          case "NOT_EQ":
          case "TYPE_MISMATCH":
...
class __PRIVATE_CoreArrayLength {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 1, 38605);
    const n2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (n2.type) {
      case "NULL":
        return __PRIVATE_EvaluateResult.gr();
      case "ARRAY":
        return __PRIVATE_EvaluateResult.newValue({
          integerValue: `${n2.value?.arrayValue?.values?.length ?? 0}`
        });
      default:
        return __PRIVATE_EvaluateResult.mr();
    }
...
class __PRIVATE_CoreReverse {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 1, 1508);
    const n2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (n2.type) {
      case "NULL":
        return __PRIVATE_EvaluateResult.gr();
      case "BYTES": {
        const e3 = n2.value?.bytesValue;
        if (typeof e3 == "string") {
          const t3 = ByteString.fromBase64String(e3).toUint8Array();
          return t3.reverse(), __PRIVATE_EvaluateResult.newValue({
            bytesValue: ByteString.fromUint8Array(t3).toBase64()
          });
        }
        return __PRIVATE_EvaluateResult.newValue({
          bytesValue: new Uint8Array(e3).reverse()
        });
      }
      case "STRING": {
        const e3 = n2.value?.stringValue, t3 = new Intl.__PRIVATE_Segmenter(undefined, {
          granularity: "grapheme"
        }).segment(e3), r2 = Array.from(t3, (e4) => e4.segment).reverse();
        return __PRIVATE_EvaluateResult.newValue({
          stringValue: r2.join("")
        });
      }
...
class __PRIVATE_CoreCharLength {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 1, 19400);
    const n2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (n2.type) {
      case "NULL":
        return __PRIVATE_EvaluateResult.gr();
      case "STRING": {
        const e3 = function __PRIVATE_getUnicodePointCount(e4) {
          let t3 = 0;
          for (let n3 = 0;n3 < e4.length; n3++) {
            const r2 = e4.codePointAt(n3);
            if (r2 === undefined)
              return;
            if (r2 <= 65535)
              if (r2 >= 55296 && r2 <= 57343)
                if (r2 <= 56319) {
                  const r3 = e4.codePointAt(n3 + 1);
                  r3 !== undefined && r3 >= 56320 && r3 <= 57343 ? (t3 += 1, n3++) : t3 += 1;
                } else
                  t3 += 1;
              else
                t3 += 1;
          ...
          return t3;
        }(n2.value.stringValue);
        return e3 === undefined ? __PRIVATE_EvaluateResult.mr() : __PRIVATE_EvaluateResult.newValue({
          integerValue: e3
        });
      }
      default:
...
class __PRIVATE_CoreByteLength {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 1, 8486);
    const n2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (n2.type) {
      case "BYTES": {
        const e3 = n2.value?.bytesValue;
        return typeof e3 == "string" ? __PRIVATE_EvaluateResult.newValue({
          integerValue: ByteString.fromBase64String(e3).toUint8Array().length
        }) : __PRIVATE_EvaluateResult.newValue({
          integerValue: new Uint8Array(e3).length
        });
      }
      case "STRING": {
        const e3 = function __PRIVATE_getUtf8ByteLength(e4) {
          let t3 = 0;
          for (let n3 = 0;n3 < e4.length; n3++) {
            const r2 = e4.codePointAt(n3);
            if (r2 === undefined)
            ...
            if (r2 >= 55296 && r2 <= 57343) {
              ...
              {
                const r3 = e4.codePointAt(n3 + 1);
                if (r3 === undefined || !(r3 >= 56320 && r3 <= 57343))
                  return;
                t3 += 4, n3++;
              }
            } else if (r2 <= 127)
...
class __PRIVATE_StringSearchFunctionBase {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 2, 39773, `${this.expr.name}() function should have exactly two parameters`);
    let n2 = false;
    const r2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (r2.type) {
      case "STRING":
    ...
    return n2 ? __PRIVATE_EvaluateResult.gr() : this.Or(r2.value?.stringValue, i2.value?.stringValue);
  }
}
...
class __PRIVATE_CoreLike extends __PRIVATE_StringSearchFunctionBase {
  Or(e2, t2) {
    try {
      const n2 = function __PRIVATE_likeToRegex(e3) {
        let t3 = "";
        for (let n3 = 0;n3 < e3.length; n3++) {
          const r3 = e3.charAt(n3);
          switch (r3) {
            case "_":
              t3 += ".";
              break;
        ...
        return "^" + t3 + "$";
      }(t2), r2 = RE2JS.compile(n2);
      return __PRIVATE_EvaluateResult.newValue({
        booleanValue: r2.matches(e2)
      });
    } catch (e3) {
      return __PRIVATE_logWarn(`Invalid LIKE pattern converted to regex: ${t2}, returning error. Error: ${e3}`), __PRIVATE_EvaluateResult.mr();
    }
  }
}
...
class __PRIVATE_CoreRegexContains extends __PRIVATE_StringSearchFunctionBase {
  Or(e2, t2) {
    try {
      const n2 = RE2JS.compile(t2);
      return __PRIVATE_EvaluateResult.newValue({
        booleanValue: n2.test(e2)
      });
    } catch (e3) {
      return __PRIVATE_logWarn(`Invalid regex pattern found in regex_contains: ${t2}, returning error`), __PRIVATE_EvaluateResult.mr();
    }
  }
}
...
class __PRIVATE_CoreRegexMatch extends __PRIVATE_StringSearchFunctionBase {
  Or(e2, t2) {
    try {
      return __PRIVATE_EvaluateResult.newValue({
        booleanValue: RE2JS.compile(t2).matches(e2)
      });
    } catch (e3) {
      return __PRIVATE_logWarn(`Invalid regex pattern found in regex_match: ${t2}, returning error`), __PRIVATE_EvaluateResult.mr();
    }
  }
}
...
class __PRIVATE_CoreToLower {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 1, 29079);
    const n2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (n2.type) {
      case "STRING":
        return __PRIVATE_EvaluateResult.newValue({
          stringValue: n2.value?.stringValue?.toLowerCase()
        });
      case "NULL":
        return __PRIVATE_EvaluateResult.gr();
      default:
...
class __PRIVATE_CoreToUpper {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 1, 60487);
    const n2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (n2.type) {
      case "STRING":
        return __PRIVATE_EvaluateResult.newValue({
          stringValue: n2.value?.stringValue?.toUpperCase()
        });
      case "NULL":
        return __PRIVATE_EvaluateResult.gr();
      default:
...
class __PRIVATE_CoreTrim {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 1, 28544);
    const n2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (n2.type) {
      case "STRING":
        return __PRIVATE_EvaluateResult.newValue({
          stringValue: n2.value?.stringValue?.trim()
        });
      case "NULL":
        return __PRIVATE_EvaluateResult.gr();
      default:
...
class __PRIVATE_CoreStrConcat {
  ...
  evaluate(e2, t2) {
    ...
    return i2 ? __PRIVATE_EvaluateResult.gr() : __PRIVATE_EvaluateResult.newValue({
      stringValue: r2
    });
  }
}
...
class __PRIVATE_CoreMapGet {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 2, 4483);
    const n2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (n2.type) {
      case "UNSET":
        return __PRIVATE_EvaluateResult.pr();
      case "MAP":
        break;
      default:
        return __PRIVATE_EvaluateResult.mr();
    ...
    if (r2.type !== "STRING")
      return __PRIVATE_EvaluateResult.mr();
    const i2 = n2.value?.mapValue?.fields?.[r2.value?.stringValue];
    return i2 === undefined ? __PRIVATE_EvaluateResult.pr() : __PRIVATE_EvaluateResult.newValue(i2);
  }
}
...
class __PRIVATE_DistanceBase {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 2, 25231, `${this.expr.name}() function should have exactly 2 params`);
    let n2 = false;
    const r2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (r2.type) {
      case "VECTOR":
    ...
    if (n2)
      return __PRIVATE_EvaluateResult.gr();
    const s2 = __PRIVATE_getVectorValue(r2.value), _ = __PRIVATE_getVectorValue(i2.value);
    if (s2 === undefined || _ === undefined || s2.values?.length !== _.values?.length)
      return __PRIVATE_EvaluateResult.mr();
    const o2 = this.Mr(s2, _);
    return o2 === undefined || isNaN(o2) ? __PRIVATE_EvaluateResult.mr() : __PRIVATE_EvaluateResult.newValue({
      doubleValue: o2
    });
  }
}
...
class __PRIVATE_CoreCosineDistance extends __PRIVATE_DistanceBase {
  Mr(e2, t2) {
    ...
    for (let e3 = 0;e3 < n2.length; e3++) {
      if (!R(n2[e3]) || !R(r2[e3]))
        return;
      const t3 = __PRIVATE_asDouble(n2[e3]), o3 = __PRIVATE_asDouble(r2[e3]);
      i2 += t3 * o3, s2 += t3 * t3, _ += o3 * o3;
    }
    const o2 = Math.sqrt(s2) * Math.sqrt(_);
    if (o2 === 0)
      return;
    return 1 - Math.max(-1, Math.min(1, i2 / o2));
  }
}
...
class __PRIVATE_CoreDotProduct extends __PRIVATE_DistanceBase {
  Mr(e2, t2) {
    ...
    for (let e3 = 0;e3 < n2.length; e3++) {
      if (!R(n2[e3]) || !R(r2[e3]))
        return;
      i2 += __PRIVATE_asDouble(n2[e3]) * __PRIVATE_asDouble(r2[e3]);
    }
    return i2;
  }
}
...
class __PRIVATE_CoreEuclideanDistance extends __PRIVATE_DistanceBase {
  Mr(e2, t2) {
    ...
    for (let e3 = 0;e3 < n2.length; e3++) {
      if (!R(n2[e3]) || !R(r2[e3]))
        return;
      const t3 = __PRIVATE_asDouble(n2[e3]), s2 = __PRIVATE_asDouble(r2[e3]);
      i2 += Math.pow(t3 - s2, 2);
    }
    return Math.sqrt(i2);
  }
...
class __PRIVATE_CoreVectorLength {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 1, 39044);
    const n2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (n2.type) {
      case "VECTOR": {
        const e3 = __PRIVATE_getVectorValue(n2.value);
        return __PRIVATE_EvaluateResult.newValue({
          integerValue: e3?.values?.length ?? 0
        });
      }
      case "NULL":
        return __PRIVATE_EvaluateResult.gr();
...
function __PRIVATE_isMicrosInBounds(e2) {
  return e2 >= qt && e2 <= $t;
}
function __PRIVATE_isSecondsInBounds(e2) {
  return e2 >= Mt && e2 <= Nt;
}
function __PRIVATE_isTimestampInBounds(e2, t2) {
  const n2 = BigInt(e2);
  return !(n2 < Mt || n2 > Nt) && (!(t2 < 0 || t2 >= 1e9) && ((n2 !== Mt || t2 === 0) && !(n2 === Nt && t2 > 999999999)));
}
function __PRIVATE_adjustTimestamp(e2, t2) {
  return t2 < 0 ? {
    seconds: e2 - 1,
    nanos: t2 + 1e9
  } : {
...
function __PRIVATE_timestampToMicros(e2) {
  return BigInt(e2.seconds) * Bt + BigInt(Math.trunc(e2.nanoseconds / 1000));
}
...
class __PRIVATE_UnixToTimestamp {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 1, 49262, `${this.expr.name}() function should have exactly one parameter`);
    const n2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (n2.type) {
      case "INT":
        return this.toTimestamp(BigInt(n2.value.integerValue));
      case "NULL":
        return __PRIVATE_EvaluateResult.gr();
      default:
        return __PRIVATE_EvaluateResult.mr();
...
class __PRIVATE_CoreUnixMicrosToTimestamp extends __PRIVATE_UnixToTimestamp {
  toTimestamp(e2) {
    if (!__PRIVATE_isMicrosInBounds(e2))
      return __PRIVATE_EvaluateResult.mr();
    let t2 = Number(e2 / Bt), n2 = Number(e2 % Bt * BigInt(1000));
    const r2 = __PRIVATE_adjustTimestamp(t2, n2);
    return t2 = r2.seconds, n2 = r2.nanos, __PRIVATE_isTimestampInBounds(t2, n2) ? __PRIVATE_EvaluateResult.newValue({
      timestampValue: {
        seconds: t2,
        nanos: n2
      }
...
class __PRIVATE_CoreUnixMillisToTimestamp extends __PRIVATE_UnixToTimestamp {
  toTimestamp(e2) {
    if (!function __PRIVATE_isMillisInBounds(e3) {
      return e3 >= Ut && e3 <= kt;
    }(e2))
      return __PRIVATE_EvaluateResult.mr();
    let t2 = Number(e2 / Lt), n2 = Number(e2 % Lt * BigInt(1e6));
    const r2 = __PRIVATE_adjustTimestamp(t2, n2);
    return t2 = r2.seconds, n2 = r2.nanos, __PRIVATE_isTimestampInBounds(t2, n2) ? __PRIVATE_EvaluateResult.newValue({
      timestampValue: {
        seconds: t2,
...
class __PRIVATE_CoreUnixSecondsToTimestamp extends __PRIVATE_UnixToTimestamp {
  toTimestamp(e2) {
    if (!__PRIVATE_isSecondsInBounds(e2))
      return __PRIVATE_EvaluateResult.mr();
    const t2 = Number(e2);
    return __PRIVATE_EvaluateResult.newValue({
      timestampValue: {
...
class __PRIVATE_TimestampToUnix {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 1, 1265, `${this.expr.name}() function should have exactly one parameter`);
    const n2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (n2.type) {
      case "TIMESTAMP":
        break;
    ...
    const r2 = fromTimestamp(n2.value.timestampValue);
    return __PRIVATE_isTimestampInBounds(r2.seconds, r2.nanoseconds) ? this.Nr(r2) : __PRIVATE_EvaluateResult.mr();
  }
}

class __PRIVATE_CoreTimestampToUnixMicros extends __PRIVATE_TimestampToUnix {
  Nr(e2) {
    const t2 = __PRIVATE_timestampToMicros(e2);
    return __PRIVATE_isMicrosInBounds(t2) ? __PRIVATE_EvaluateResult.newValue({
      integerValue: `${t2.toString()}`
    }) : __PRIVATE_EvaluateResult.mr();
  }
}
...
class __PRIVATE_CoreTimestampToUnixMillis extends __PRIVATE_TimestampToUnix {
  Nr(e2) {
    const t2 = __PRIVATE_timestampToMicros(e2), n2 = t2 / BigInt(1000), r2 = t2 % BigInt(1000);
    return n2 > BigInt(0) || r2 === BigInt(0) ? __PRIVATE_EvaluateResult.newValue({
      integerValue: n2.toString()
    }) : __PRIVATE_EvaluateResult.newValue({
      integerValue: (n2 - BigInt(1)).toString()
    });
  }
}
...
class __PRIVATE_CoreTimestampToUnixSeconds extends __PRIVATE_TimestampToUnix {
  Nr(e2) {
    const t2 = BigInt(e2.seconds);
    return __PRIVATE_isSecondsInBounds(t2) ? __PRIVATE_EvaluateResult.newValue({
      integerValue: t2.toString()
    }) : __PRIVATE_EvaluateResult.mr();
  }
}
...
class __PRIVATE_TimestampArithmetic {
  ...
  evaluate(e2, t2) {
    __PRIVATE_hardAssert(this.expr.params.length === 3, 2775, `${this.expr.name}() function should have exactly 3 parameters`);
    let n2 = false;
    const r2 = __PRIVATE_toEvaluable(this.expr.params[0]).evaluate(e2, t2);
    switch (r2.type) {
      case "TIMESTAMP":
    ...
    switch (i2.type) {
      case "STRING":
        if (s2 = function __PRIVATE_asTimeUnit(e3) {
          switch (e3) {
            case "microsecond":
              return "microsecond";
            case "millisecond":
        ...
        }(i2.value.stringValue), s2 === undefined)
          return __PRIVATE_EvaluateResult.mr();
        break;
      case "NULL":
        n2 = true;
    ...
    const _ = __PRIVATE_toEvaluable(this.expr.params[2]).evaluate(e2, t2);
    switch (_.type) {
      case "INT":
        break;
      case "NULL":
    ...
    if (n2)
      return __PRIVATE_EvaluateResult.gr();
    const o2 = BigInt(_.value.integerValue);
    let a;
    try {
      switch (s2) {
        case "microsecond":
        ...
        case "second":
          ...
          break;
        case "minute":
          a = o2 * BigInt(60000000);
          break;
        case "hour":
          a = o2 * BigInt(3600000000);
          break;
        case "day":
          a = o2 * BigInt(86400000000);
          break;
        default:
          return __PRIVATE_EvaluateResult.mr();
      }
      if (s2 !== "microsecond" && o2 !== BigInt(0) && a / o2 !== BigInt(this.Lr(s2)))
        return __PRIVATE_EvaluateResult.mr();
    } catch (e3) {
      return __PRIVATE_logWarn(`Error during timestamp arithmetic: ${e3}`), __PRIVATE_EvaluateResult.mr();
    }
    const u2 = fromTimestamp(r2.value.timestampValue);
    if (!__PRIVATE_isTimestampInBounds(u2.seconds, u2.nanoseconds))
      return __PRIVATE_EvaluateResult.mr();
    const c2 = __PRIVATE_timestampToMicros(u2), l2 = this.Br(c2, a);
    if (!__PRIVATE_isMicrosInBounds(l2))
      return __PRIVATE_EvaluateResult.mr();
    const E = Number(l2 / Bt), h = l2 % Bt, T = Number((h < 0 ? h + Bt : h) * BigInt(1000)), P = h < 0 ? E - 1 : E;
...
function __PRIVATE_canonifyExpr(e2) {
  if ((e2 = __PRIVATE_unwrapExpression(e2)) instanceof o)
    return `fld(${e2.fieldName})`;
  if (e2 instanceof Constant)
    return `cst(${function __PRIVATE_canonifyConstantValue(e3) {
      return e3 === null ? "null" : typeof e3 == "number" ? e3.toString() : typeof e3 == "string" ? `"${e3}"` : e3 instanceof aa ? `ref(${e3.path})` : e3 instanceof n ? `vec(${JSON.stringify(e3)})` : JSON.stringify(e3);
    }(e2.value)})`;
  if (e2 instanceof y)
    return `fn(${e2.name},[${e2.params.map(__PRIVATE_canonifyExpr).join(",")}])`;
  if (e2.expressionType === "ListOfExpressions")
    return `list([${e2.cr.map(__PRIVATE_canonifyExpr).join(",")}])`;
  throw new Error(`Unrecognized expr ${JSON.stringify(e2, null, 2)}`);
}
function __PRIVATE_canonifyStage(e2) {
  if (e2 instanceof w)
    return `${e2._name}(${__PRIVATE_canonifyExprMap(e2.fields)})`;
  if (e2 instanceof O) {
    let t2 = `${e2._name}(${__PRIVATE_canonifyExprMap(e2.accumulators)})`;
    return e2.groups.size > 0 && (t2 += `grouping(${__PRIVATE_canonifyExprMap(e2.groups)})`), t2;
  }
  if (e2 instanceof M)
    return `${e2._name}(${__PRIVATE_canonifyExprMap(e2.groups)})`;
  if (e2 instanceof X)
    return `${e2._name}(${e2.hr})`;
  if (e2 instanceof Y)
    return `${e2._name}(${e2.collectionId})`;
  if (e2 instanceof Z)
    return `${e2._name}()`;
  if (e2 instanceof sa)
    return `${e2._name}(${e2.Tr.sort()})`;
  if (e2 instanceof A)
    return `${e2._name}(${__PRIVATE_canonifyExpr(e2.condition)})`;
  if (e2 instanceof v)
    return `${e2._name}(${e2.limit})`;
  if (e2 instanceof L)
    return `${e2._name}(${function __PRIVATE_canonifySortOrderings(e3) {
      return e3.map((e4) => `${__PRIVATE_canonifyExpr(e4.expr)}${e4.direction}`).join(",");
    }(e2.orderings)})`;
  throw new Error(`Unrecognized stage ${e2._name}`);
}
function __PRIVATE_canonifyExprMap(e2) {
  return `${Array.from(e2.entries()).sort().map(([e3, t2]) => `${e3}=${__PRIVATE_canonifyExpr(t2)}`).join(",")}`;
}
function __PRIVATE_canonifyPipeline(e2) {
  return e2.stages.map((e3) => __PRIVATE_canonifyStage(e3)).join("|");
}
function __PRIVATE_pipelineEq(e2, t2) {
  return __PRIVATE_canonifyPipeline(e2) === __PRIVATE_canonifyPipeline(t2);
}
function __PRIVATE_isPipeline(e2) {
  return e2 instanceof CorePipeline;
}
function __PRIVATE_stringifyQueryOrPipeline(e2) {
  return __PRIVATE_isPipeline(e2) ? __PRIVATE_canonifyPipeline(e2) : __PRIVATE_stringifyQuery(e2);
}
function __PRIVATE_canonifyQueryOrPipeline(e2) {
  return __PRIVATE_isPipeline(e2) ? __PRIVATE_canonifyPipeline(e2) : function __PRIVATE_canonifyQuery(e3) {
    return `${__PRIVATE_canonifyTarget(__PRIVATE_queryToTarget(e3))}|lt:${e3.limitType}`;
  }(e2);
}
function __PRIVATE_queryOrPipelineEqual(e2, t2) {
  return e2 instanceof CorePipeline && t2 instanceof CorePipeline ? __PRIVATE_pipelineEq(e2, t2) : !(e2 instanceof CorePipeline && !(t2 instanceof CorePipeline) || !(e2 instanceof CorePipeline) && t2 instanceof CorePipeline) && __PRIVATE_queryEquals(e2, t2);
}
function __PRIVATE_canonifyTargetOrPipeline(e2) {
  return __PRIVATE_targetIsPipelineTarget(e2) ? __PRIVATE_canonifyPipeline(e2) : __PRIVATE_canonifyTarget(e2);
}
function __PRIVATE_targetOrPipelineEqual(e2, t2) {
  return e2 instanceof CorePipeline && t2 instanceof CorePipeline ? __PRIVATE_pipelineEq(e2, t2) : !(e2 instanceof CorePipeline && !(t2 instanceof CorePipeline) || !(e2 instanceof CorePipeline) && t2 instanceof CorePipeline) && __PRIVATE_targetEquals(e2, t2);
}
function __PRIVATE_toCorePipeline(e2, t2) {
  const n2 = function __PRIVATE_rewriteStages(e3) {
    let t3 = false;
    const n3 = [];
    for (const r2 of e3)
      if (r2 instanceof L)
        if (t3 = true, r2.orderings.some((e4) => e4.expr instanceof o && e4.expr.fieldName === Ce))
          n3.push(r2);
        else {
          const e4 = r2.orderings.map((e5) => e5);
          e4.push(s(Ce).ascending()), n3.push(new L(e4, {}));
        }
      else
        r2 instanceof v ? (t3 || (n3.push(new L([s(Ce).ascending()], {})), t3 = true), n3.push(r2)) : n3.push(r2);
    return t3 || n3.push(new L([s(Ce).ascending()], {})), n3;
  }(e2.stages);
  if (e2.userDataReader) {
    const t3 = e2.userDataReader.createContext(3, "toCorePipeline");
    n2.forEach((e3) => e3._readUserData(t3));
  }
  return new CorePipeline(e2.userDataReader.serializer, n2, t2);
}

class MutationBatch {
  constructor(e2, t2, n2, r2) {
    this.batchId = e2, this.localWriteTime = t2, this.baseMutations = n2, this.mutations = r2;
  }
  applyToRemoteDocument(e2, t2) {
    ...
    for (let t3 = 0;t3 < this.mutations.length; t3++) {
      ...
      if (r2.key.isEqual(e2.key)) {
        __PRIVATE_mutationApplyToRemoteDocument(r2, e2, n2[t3]);
      }
    }
  }
  applyToLocalView(e2, t2) {
    for (const n2 of this.baseMutations)
      n2.key.isEqual(e2.key) && (t2 = __PRIVATE_mutationApplyToLocalView(n2, e2, t2, this.localWriteTime));
  ...
  applyToLocalDocumentSet(e2, t2) {
    ...
    return this.mutations.forEach((r2) => {
      const i2 = e2.get(r2.key), s2 = i2.overlayedDocument;
      let _ = this.applyToLocalView(s2, i2.mutatedFields);
      _ = t2.has(r2.key) ? null : _;
      const o2 = __PRIVATE_calculateOverlayMutation(s2, _);
      o2 !== null && n2.set(r2.key, o2), s2.isValidDocument() || s2.convertToNoDocument(SnapshotVersion.min());
    }), n2;
  }
  keys() {
...
class MutationBatchResult {
  ...
  static from(e2, t2, n2) {
    ...
    for (let e3 = 0;e3 < i2.length; e3++)
      r2 = r2.insert(i2[e3].key, n2[e3].version);
    return new MutationBatchResult(e2, t2, n2, r2);
  }
}
var Kt = "\x01";
function __PRIVATE_encodeResourcePath(e2) {
  ...
  for (let n2 = 0;n2 < e2.length; n2++)
    t2.length > 0 && (t2 = __PRIVATE_encodeSeparator(t2)), t2 = __PRIVATE_encodeSegment(e2.get(n2), t2);
...
var Qt = "remoteDocuments";
var Wt = "owner";
var zt = "mutationQueues";
var Ht = "mutations";
var en = "documentMutations";
var tn = "remoteDocumentsV14";
var an = "remoteDocumentGlobal";
var cn = "targets";
var Tn = "targetDocuments";
var Vn = "targetGlobal";
var dn = "collectionParents";
var mn = "clientMetadata";
var gn = "bundles";
var wn = "namedQueries";
var Sn = "indexConfiguration";
var Cn = "indexState";
var Nn = "indexEntries";
var kn = "documentOverlays";
var Gn = "globals";
var jn = [...[...[...[...[zt, Ht, en, Qt, cn, Wt, Vn, Tn], mn], an], dn], gn, wn];
var Hn = [...jn, kn];
var Jn = [zt, Ht, en, tn, cn, Wt, Vn, Tn, mn, an, dn, gn, wn, kn];
var Yn = Jn;
var Zn = [...Yn, Sn, Cn, Nn];
var er = [...Zn, Gn];
class Overlay {
  ...
  isEqual(e2) {
    return e2 !== null && this.mutation === e2.mutation;
  }
  toString() {
    return `Overlay{
...
class TargetData {
  constructor(e2, t2, n2, r2, i2 = SnapshotVersion.min(), s2 = SnapshotVersion.min(), _ = ByteString.EMPTY_BYTE_STRING, o2 = null) {
    this.target = e2, this.targetId = t2, this.purpose = n2, this.sequenceNumber = r2, this.snapshotVersion = i2, this.lastLimboFreeSnapshotVersion = s2, this.resumeToken = _, this.expectedCount = o2;
  }
  ...
  withResumeToken(e2, t2) {
    return new TargetData(this.target, this.targetId, this.purpose, this.sequenceNumber, t2, this.lastLimboFreeSnapshotVersion, e2, null);
  }
...
function __PRIVATE_fromBundledQuery(e2) {
  ...
  return e2.limitType === "LAST" ? __PRIVATE_queryWithLimit(t2, t2.limit, "L") : t2;
}
class __PRIVATE_FirestoreIndexValueWriter {
  ...
  ti(e2, t2) {
    if ("nullValue" in e2)
      this.ri(t2, 5);
    else if ("booleanValue" in e2)
      this.ri(t2, 10), t2.ii(e2.booleanValue ? 1 : 0);
    else if ("integerValue" in e2)
      this.ri(t2, 15), t2.ii(__PRIVATE_normalizeNumber(e2.integerValue));
    else if ("doubleValue" in e2) {
      const n2 = __PRIVATE_normalizeNumber(e2.doubleValue);
      isNaN(n2) ? this.ri(t2, 13) : (this.ri(t2, 15), __PRIVATE_isNegativeZero(n2) ? t2.ii(0) : t2.ii(n2));
    } else if ("timestampValue" in e2) {
      let n2 = e2.timestampValue;
      this.ri(t2, 20), typeof n2 == "string" && (n2 = __PRIVATE_normalizeTimestamp(n2)), t2.si(`${n2.seconds || ""}`), t2.ii(n2.nanos || 0);
    } else if ("stringValue" in e2)
      this._i(e2.stringValue, t2), this.oi(t2);
    else if ("bytesValue" in e2)
      this.ri(t2, 30), t2.ai(__PRIVATE_normalizeByteString(e2.bytesValue)), this.oi(t2);
    else if ("referenceValue" in e2)
      this.ui(e2.referenceValue, t2);
    else if ("geoPointValue" in e2) {
      const n2 = e2.geoPointValue;
      this.ri(t2, 45), t2.ii(n2.latitude || 0), t2.ii(n2.longitude || 0);
    } else
      "mapValue" in e2 ? __PRIVATE_isMaxValue(e2) ? this.ri(t2, Number.MAX_SAFE_INTEGER) : __PRIVATE_isVectorValue(e2) ? this.ci(e2.mapValue, t2) : (this.li(e2.mapValue, t2), this.oi(t2)) : ("arrayValue" in e2) ? (this.Ei(e2.arrayValue, t2), this.oi(t2)) : l(19022, {
        hi: e2
      });
  }
  _i(e2, t2) {
    this.ri(t2, 25), this.Ti(e2, t2);
  }
  Ti(e2, t2) {
    t2.si(e2);
  }
  li(e2, t2) {
    ...
    for (const e3 of Object.keys(n2))
      this._i(e3, t2), this.ti(n2[e3], t2);
  }
  ci(e2, t2) {
    const n2 = e2.fields || {};
    this.ri(t2, 53);
    const r2 = je, i2 = n2[r2].arrayValue?.values?.length || 0;
    this.ri(t2, 15), t2.ii(__PRIVATE_normalizeNumber(i2)), this._i(r2, t2), this.ti(n2[r2], t2);
  }
  Ei(e2, t2) {
    const n2 = e2.values || [];
    this.ri(t2, 50);
...
class __PRIVATE_MemoryIndexManager {
  ...
  addToCollectionParentIndex(e2, t2) {
    return this.Zi.add(t2), PersistencePromise.resolve();
  }
  getCollectionParents(e2, t2) {
    return PersistencePromise.resolve(this.Zi.getEntries(t2));
  }
  addFieldIndex(e2, t2) {
    return PersistencePromise.resolve();
  }
  ...
  updateCollectionGroup(e2, t2, n2) {
    return PersistencePromise.resolve();
  }
...
class __PRIVATE_MemoryCollectionParentIndex {
  ...
  add(e2) {
    const t2 = e2.lastSegment(), n2 = e2.popLast(), r2 = this.index[t2] || new SortedSet(ResourcePath.comparator), i2 = !r2.has(n2);
    return this.index[t2] = r2.add(n2), i2;
  }
  has(e2) {
    const t2 = e2.lastSegment(), n2 = e2.popLast(), r2 = this.index[t2];
    return r2 && r2.has(n2);
  }
  getEntries(e2) {
...
function __PRIVATE_runPipeline(e2, t2) {
  let n2 = t2;
  for (const t3 of e2.stages)
    n2 = evaluate({
      serializer: e2.serializer,
      serverTimestampBehavior: e2.listenOptions?.serverTimestampBehavior
    }, t3, n2);
  return n2;
}
function __PRIVATE_pipelineMatches(e2, t2) {
  return __PRIVATE_runPipeline(e2, [t2]).length > 0;
}
function __PRIVATE_queryOrPipelineMatches(e2, t2) {
  return __PRIVATE_isPipeline(e2) ? __PRIVATE_pipelineMatches(e2, t2) : __PRIVATE_queryMatches(e2, t2);
}
function evaluate(e2, t2, n2) {
  if (t2 instanceof X)
    return function __PRIVATE_evaluateCollection(e3, t3, n3) {
      return n3.filter((e4) => e4.isFoundDocument() && `/${e4.key.getCollectionPath().canonicalString()}` === t3.hr);
    }(0, t2, n2);
  if (t2 instanceof A)
    return function __PRIVATE_evaluateWhere(e3, t3, n3) {
      return n3.filter((n4) => {
        const r2 = __PRIVATE_valueOrUndefined(__PRIVATE_toEvaluable(t3.condition).evaluate(e3, n4));
        return r2 !== undefined && __PRIVATE_valueEquals$1(r2, Je);
      });
    }(e2, t2, n2);
  if (t2 instanceof Y)
    return function __PRIVATE_evaluateCollectionGroup(e3, t3, n3) {
      return n3.filter((e4) => e4.isFoundDocument() && e4.key.getCollectionPath().lastSegment() === t3.collectionId);
    }(0, t2, n2);
  if (t2 instanceof Z)
    return function __PRIVATE_evaluateDatabase(e3, t3, n3) {
      return n3.filter((e4) => e4.isFoundDocument());
    }(0, 0, n2);
  if (t2 instanceof sa)
    return function __PRIVATE_evaluateDocuments(e3, t3, n3) {
      return n3.filter((e4) => e4.isFoundDocument() && t3.Pr.has(e4.key.path.toStringWithLeadingSlash()));
    }(0, t2, n2);
  if (t2 instanceof v)
    return function __PRIVATE_evaluateLimit(e3, t3, n3) {
      return n3.slice(0, t3.limit);
    }(0, t2, n2);
  if (t2 instanceof L)
    return function __PRIVATE_evaluateSort(e3, t3, n3) {
      const r2 = t3.orderings.map((e4) => ({
        Ms: __PRIVATE_toEvaluable(e4.expr),
        direction: e4.direction
      }));
      return [...n3].sort((t4, n4) => {
        for (const { Ms: i2, direction: s2 } of r2) {
          const r3 = __PRIVATE_valueOrUndefined(i2.evaluate(e3, t4)), _ = __PRIVATE_valueOrUndefined(i2.evaluate(e3, n4)), o2 = __PRIVATE_valueCompare(r3 ?? He, _ ?? He);
          if (o2 !== 0)
            return s2 === "ascending" ? o2 : -o2;
        }
        return 0;
      });
    }(e2, t2, n2);
  throw new Error(`Unknown stage: ${t2._name}`);
}
function __PRIVATE_newPipelineComparator(e2) {
  const t2 = function __PRIVATE_lastEffectiveSort(e3) {
    for (let t3 = e3.stages.length - 1;t3 >= 0; t3--) {
      const n2 = e3.stages[t3];
      if (n2 instanceof L)
        return n2.orderings;
    }
  ...
  return (n2, r2) => {
    for (const i2 of t2) {
      const t3 = __PRIVATE_valueOrUndefined(__PRIVATE_toEvaluable(i2.expr).evaluate({
        serializer: e2.serializer
      }, n2)), s2 = __PRIVATE_valueOrUndefined(__PRIVATE_toEvaluable(i2.expr).evaluate({
        serializer: e2.serializer
      }, r2)), _ = __PRIVATE_valueCompare(t3 || He, s2 || He);
      if (_ !== 0)
        return i2.direction === "ascending" ? _ : -_;
    }
    return 0;
  };
}
function __PRIVATE_getLastEffectiveLimit(e2) {
  for (let t2 = e2.stages.length - 1;t2 >= 0; t2--) {
    const n2 = e2.stages[t2];
    if (n2 instanceof v)
      return {
        limit: n2.limit
      };
...
class RemoteDocumentChangeBuffer {
  ...
  getEntry(e2, t2) {
    ...
    return n2 !== undefined ? PersistencePromise.resolve(n2) : this.getFromCache(e2, t2);
  }
  getEntries(e2, t2) {
...
class LocalDocumentsView {
  constructor(e2, t2, n2, r2) {
    this.remoteDocumentCache = e2, this.mutationQueue = t2, this.documentOverlayCache = n2, this.indexManager = r2;
  }
  getDocument(e2, t2) {
    let n2 = null;
    return this.documentOverlayCache.getOverlay(e2, t2).next((r2) => (n2 = r2, this.remoteDocumentCache.getEntry(e2, t2))).next((e3) => (n2 !== null && __PRIVATE_mutationApplyToLocalView(n2.mutation, e3, FieldMask.empty(), Timestamp.now()), e3));
  }
  getDocuments(e2, t2) {
  ...
  getLocalViewOfDocuments(e2, t2, n2 = __PRIVATE_documentKeySet()) {
    const r2 = __PRIVATE_newOverlayMap();
    return this.populateOverlays(e2, r2, t2).next(() => this.computeViews(e2, t2, r2, n2).next((e3) => {
      let t3 = documentMap();
      return e3.forEach((e4, n3) => {
        t3 = t3.insert(e4, n3.overlayedDocument);
      }), t3;
    }));
  }
  getOverlayedDocuments(e2, t2) {
    const n2 = __PRIVATE_newOverlayMap();
    return this.populateOverlays(e2, n2, t2).next(() => this.computeViews(e2, t2, n2, __PRIVATE_documentKeySet()));
  }
  populateOverlays(e2, t2, n2) {
    const r2 = [];
  ...
  computeViews(e2, t2, n2, r2) {
    let i2 = __PRIVATE_mutableDocumentMap();
    const s2 = __PRIVATE_newDocumentKeyMap(), _ = function __PRIVATE_newOverlayedDocumentMap() {
      return __PRIVATE_newDocumentKeyMap();
    }();
    return t2.forEach((e3, t3) => {
      const _2 = n2.get(t3.key);
      r2.has(t3.key) && (_2 === undefined || _2.mutation instanceof __PRIVATE_PatchMutation) ? i2 = i2.insert(t3.key, t3) : _2 !== undefined ? (s2.set(t3.key, _2.mutation.getFieldMask()), __PRIVATE_mutationApplyToLocalView(_2.mutation, t3, _2.mutation.getFieldMask(), Timestamp.now())) : s2.set(t3.key, FieldMask.empty());
    }), this.recalculateAndSaveOverlays(e2, i2).next((e3) => (e3.forEach((e4, t3) => s2.set(e4, t3)), t2.forEach((e4, t3) => _.set(e4, new OverlayedDocument(t3, s2.get(e4) ?? null))), _));
  }
  recalculateAndSaveOverlays(e2, t2) {
    const n2 = __PRIVATE_newDocumentKeyMap();
    let r2 = new SortedMap((e3, t3) => e3 - t3), i2 = __PRIVATE_documentKeySet();
    return this.mutationQueue.getAllMutationBatchesAffectingDocumentKeys(e2, t2).next((e3) => {
      for (const i3 of e3)
        i3.keys().forEach((e4) => {
          ...
          if (s2 === null)
            return;
          let _ = n2.get(e4) || FieldMask.empty();
          _ = i3.applyToLocalView(s2, _), n2.set(e4, _);
          const o2 = (r2.get(i3.batchId) || __PRIVATE_documentKeySet()).add(e4);
          r2 = r2.insert(i3.batchId, o2);
        });
    }).next(() => {
      const s2 = [], _ = r2.getReverseIterator();
      for (;_.hasNext(); ) {
        const r3 = _.getNext(), o2 = r3.key, a = r3.value, u2 = __PRIVATE_newMutationMap();
        a.forEach((e3) => {
          if (!i2.has(e3)) {
            const r4 = __PRIVATE_calculateOverlayMutation(t2.get(e3), n2.get(e3));
            r4 !== null && u2.set(e3, r4), i2 = i2.add(e3);
          }
        }), s2.push(this.documentOverlayCache.saveOverlays(e2, o2, u2));
      }
  ...
  getDocumentsMatchingQuery(e2, t2, n2, r2) {
    return __PRIVATE_isPipeline(t2) ? this.getDocumentsMatchingPipeline(e2, t2, n2, r2) : __PRIVATE_isDocumentQuery$1(t2) ? this.getDocumentsMatchingDocumentQuery(e2, t2.path) : __PRIVATE_isCollectionGroupQuery(t2) ? this.getDocumentsMatchingCollectionGroupQuery(e2, t2, n2, r2) : this.getDocumentsMatchingCollectionQuery(e2, t2, n2, r2);
  }
  getNextDocuments(e2, t2, n2, r2) {
    return this.remoteDocumentCache.getAllFromCollectionGroup(e2, t2, n2, r2).next((i2) => {
      const s2 = r2 - i2.size > 0 ? this.documentOverlayCache.getOverlaysForCollectionGroup(e2, t2, n2.largestBatchId, r2 - i2.size) : PersistencePromise.resolve(__PRIVATE_newOverlayMap());
      let _ = Xe, o2 = i2;
      return s2.next((t3) => PersistencePromise.forEach(t3, (t4, n3) => (_ < n3.largestBatchId && (_ = n3.largestBatchId), i2.get(t4) ? PersistencePromise.resolve() : this.remoteDocumentCache.getEntry(e2, t4).next((e3) => {
        o2 = o2.insert(t4, e3);
      }))).next(() => this.populateOverlays(e2, t3, i2)).next(() => this.computeViews(e2, o2, t3, __PRIVATE_documentKeySet())).next((e3) => ({
        batchId: _,
  ...
  getDocumentsMatchingCollectionGroupQuery(e2, t2, n2, r2) {
    const i2 = t2.collectionGroup;
    let s2 = documentMap();
    return this.indexManager.getCollectionParents(e2, i2).next((_) => PersistencePromise.forEach(_, (_2) => {
      const o2 = function __PRIVATE_asCollectionQueryAtPath(e3, t3) {
        return new __PRIVATE_QueryImpl(t3, null, e3.explicitOrderBy.slice(), e3.filters.slice(), e3.limit, e3.limitType, e3.startAt, e3.endAt);
      }(t2, _2.child(i2));
      return this.getDocumentsMatchingCollectionQuery(e2, o2, n2, r2).next((e3) => {
        e3.forEach((e4, t3) => {
          s2 = s2.insert(e4, t3);
        });
      });
  ...
  getDocumentsMatchingCollectionQuery(e2, t2, n2, r2) {
    let i2;
    return this.documentOverlayCache.getOverlaysForCollection(e2, t2.path, n2.largestBatchId).next((s2) => (i2 = s2, this.remoteDocumentCache.getDocumentsMatchingQuery(e2, t2, n2, i2, r2))).next((e3) => this.retrieveMatchingLocalDocuments(i2, e3, (e4) => __PRIVATE_queryMatches(t2, e4)));
  }
  getDocumentsMatchingPipeline(t2, n2, r2, i2) {
    if (getPipelineSourceType(n2) === "collection_group") {
      const e2 = getPipelineCollectionGroup(n2);
      let s2 = documentMap();
      return this.indexManager.getCollectionParents(t2, e2).next((_) => PersistencePromise.forEach(_, (_2) => {
        const o2 = function __PRIVATE_asCollectionPipelineAtPath(e3, t3) {
          const n3 = e3.stages.map((e4) => e4 instanceof Y ? new X(t3.canonicalString(), {}) : e4);
          return new CorePipeline(e3.serializer, n3);
        }(n2, _2.child(e2));
        return this.getDocumentsMatchingPipeline(t2, o2, r2, i2).next((e3) => {
          e3.forEach((e4, t3) => {
            s2 = s2.insert(e4, t3);
          });
        });
    ...
    {
      let s2;
      return this.getOverlaysForPipeline(t2, n2, r2.largestBatchId).next((_) => {
        switch (s2 = _, getPipelineSourceType(n2)) {
          case "collection":
            return this.remoteDocumentCache.getDocumentsMatchingQuery(t2, n2, r2, s2, i2);
          case "documents":
            let _2 = __PRIVATE_documentKeySet();
            for (const e2 of getPipelineDocuments(n2))
              _2 = _2.add(DocumentKey.fromPath(e2));
            return this.remoteDocumentCache.getEntries(t2, _2);
          case "database":
            return this.remoteDocumentCache.getAllEntries(t2);
          default:
            throw new e("invalid-argument", `Invalid pipeline source to execute offline: ${__PRIVATE_canonifyPipeline(n2)}`);
        }
      }).next((e2) => this.retrieveMatchingLocalDocuments(s2, e2, (e3) => __PRIVATE_pipelineMatches(n2, e3)));
    }
  }
  retrieveMatchingLocalDocuments(e2, t2, n2) {
    e2.forEach((e3, n3) => {
      const r3 = n3.getKey();
      t2.get(r3) === null && (t2 = t2.insert(r3, MutableDocument.newInvalidDocument(r3)));
    });
    let r2 = documentMap();
    return t2.forEach((t3, i2) => {
      const s2 = e2.get(t3);
      s2 !== undefined && __PRIVATE_mutationApplyToLocalView(s2.mutation, i2, FieldMask.empty(), Timestamp.now()), n2(i2) && (r2 = r2.insert(t3, i2));
    }), r2;
  }
  getOverlaysForPipeline(t2, n2, r2) {
    switch (getPipelineSourceType(n2)) {
      case "collection":
        return this.documentOverlayCache.getOverlaysForCollection(t2, ResourcePath.fromString(getPipelineCollection(n2)), r2);
      case "collection_group":
        throw new e("invalid-argument", `Unexpected collection group pipeline: ${__PRIVATE_canonifyPipeline(n2)}`);
      case "documents":
        return this.documentOverlayCache.getOverlays(t2, getPipelineDocuments(n2).map((e2) => DocumentKey.fromPath(e2)));
      case "database":
        return this.documentOverlayCache.getAllOverlays(t2, r2);
      default:
        throw new e("invalid-argument", `Failed to get overlays for pipeline: ${__PRIVATE_canonifyPipeline(n2)}`);
    }
  }
}
...
class __PRIVATE_MemoryBundleCache {
  constructor(e2) {
    this.serializer = e2, this.Qs = new Map, this.Ws = new Map;
  }
  getBundleMetadata(e2, t2) {
    return PersistencePromise.resolve(this.Qs.get(t2));
  }
  saveBundleMetadata(e2, t2) {
    return this.Qs.set(t2.id, function __PRIVATE_fromBundleMetadata(e3) {
      return {
        id: e3.id,
        version: e3.version,
  ...
  getNamedQuery(e2, t2) {
    return PersistencePromise.resolve(this.Ws.get(t2));
  }
  saveNamedQuery(e2, t2) {
    return this.Ws.set(t2.name, function __PRIVATE_fromProtoNamedQuery(e3) {
      return {
        name: e3.name,
...
class __PRIVATE_MemoryDocumentOverlayCache {
  constructor() {
    this.overlays = new SortedMap(DocumentKey.comparator), this.Gs = new Map;
  }
  getOverlay(e2, t2) {
  ...
  getOverlays(e2, t2) {
    ...
    return PersistencePromise.forEach(t2, (t3) => this.getOverlay(e2, t3).next((e3) => {
      e3 !== null && n2.set(t3, e3);
    })).next(() => n2);
  }
  getAllOverlays(e2, t2) {
    const n2 = __PRIVATE_newOverlayMap();
    return this.overlays.forEach((e3, r2) => {
      r2.largestBatchId > t2 && n2.set(e3, r2);
    }), PersistencePromise.resolve(n2);
  }
  saveOverlays(e2, t2, n2) {
    return n2.forEach((n3, r2) => {
      this.Zr(e2, t2, r2);
    }), PersistencePromise.resolve();
  }
  removeOverlaysForBatchId(e2, t2, n2) {
    const r2 = this.Gs.get(n2);
    return r2 !== undefined && (r2.forEach((e3) => this.overlays = this.overlays.remove(e3)), this.Gs.delete(n2)), PersistencePromise.resolve();
  }
  getOverlaysForCollection(e2, t2, n2) {
    const r2 = __PRIVATE_newOverlayMap(), i2 = t2.length + 1, s2 = new DocumentKey(t2.child("")), _ = this.overlays.getIteratorFrom(s2);
    for (;_.hasNext(); ) {
      const e3 = _.getNext().value, s3 = e3.getKey();
      if (!t2.isPrefixOf(s3.path))
        break;
      s3.path.length === i2 && (e3.largestBatchId > n2 && r2.set(e3.getKey(), e3));
    }
    return PersistencePromise.resolve(r2);
  }
  getOverlaysForCollectionGroup(e2, t2, n2, r2) {
    let i2 = new SortedMap((e3, t3) => e3 - t3);
    const s2 = this.overlays.getIterator();
    for (;s2.hasNext(); ) {
      const e3 = s2.getNext().value;
      if (e3.getKey().getCollectionGroup() === t2 && e3.largestBatchId > n2) {
        let t3 = i2.get(e3.largestBatchId);
        t3 === null && (t3 = __PRIVATE_newOverlayMap(), i2 = i2.insert(e3.largestBatchId, t3)), t3.set(e3.getKey(), e3);
      }
    }
    const _ = __PRIVATE_newOverlayMap(), o2 = i2.getIterator();
    for (;o2.hasNext(); ) {
      if (o2.getNext().value.forEach((e3, t3) => _.set(e3, t3)), _.size() >= r2)
        break;
    }
  ...
  Zr(e2, t2, n2) {
    ...
    if (r2 !== null) {
      const e3 = this.Gs.get(r2.largestBatchId).delete(n2.key);
      this.Gs.set(r2.largestBatchId, e3);
    }
    this.overlays = this.overlays.insert(n2.key, new Overlay(t2, n2));
    let i2 = this.Gs.get(t2);
    i2 === undefined && (i2 = __PRIVATE_documentKeySet(), this.Gs.set(t2, i2)), this.Gs.set(t2, i2.add(n2.key));
  }
}
...
class __PRIVATE_ReferenceSet {
  ...
  addReference(e2, t2) {
    const n2 = new __PRIVATE_DocReference(e2, t2);
    this.zs = this.zs.add(n2), this.Hs = this.Hs.add(n2);
  }
  Ys(e2, t2) {
    e2.forEach((e3) => this.addReference(e3, t2));
  }
  removeReference(e2, t2) {
    this.Zs(new __PRIVATE_DocReference(e2, t2));
  }
  Xs(e2, t2) {
    e2.forEach((e3) => this.removeReference(e3, t2));
  }
  e_(e2) {
    const t2 = new DocumentKey(new ResourcePath([])), n2 = new __PRIVATE_DocReference(t2, e2), r2 = new __PRIVATE_DocReference(t2, e2 + 1), i2 = [];
    return this.Hs.forEachInRange([n2, r2], (e3) => {
      this.Zs(e3), i2.push(e3.key);
    }), i2;
  }
  t_() {
    this.zs.forEach((e2) => this.Zs(e2));
  }
  Zs(e2) {
    this.zs = this.zs.delete(e2), this.Hs = this.Hs.delete(e2);
  }
  n_(e2) {
    const t2 = new DocumentKey(new ResourcePath([])), n2 = new __PRIVATE_DocReference(t2, e2), r2 = new __PRIVATE_DocReference(t2, e2 + 1);
  ...
  containsKey(e2) {
    const t2 = new __PRIVATE_DocReference(e2, 0), n2 = this.zs.firstAfterOrEqual(t2);
    return n2 !== null && e2.isEqual(n2.key);
  }
}

class __PRIVATE_DocReference {
  constructor(e2, t2) {
    this.key = e2, this.r_ = t2;
  }
  static js(e2, t2) {
    return DocumentKey.comparator(e2.key, t2.key) || __PRIVATE_primitiveComparator(e2.r_, t2.r_);
  }
  static Js(e2, t2) {
    return __PRIVATE_primitiveComparator(e2.r_, t2.r_) || DocumentKey.comparator(e2.key, t2.key);
  }
}

class __PRIVATE_MemoryMutationQueue {
  constructor(e2, t2) {
    this.indexManager = e2, this.referenceDelegate = t2, this.mutationQueue = [], this.Gr = 1, this.i_ = new SortedSet(__PRIVATE_DocReference.js);
  }
  checkEmpty(e2) {
    return PersistencePromise.resolve(this.mutationQueue.length === 0);
  }
  addMutationBatch(e2, t2, n2, r2) {
    const i2 = this.Gr;
    this.Gr++, this.mutationQueue.length > 0 && this.mutationQueue[this.mutationQueue.length - 1];
    const s2 = new MutationBatch(i2, t2, n2, r2);
    this.mutationQueue.push(s2);
    for (const t3 of r2)
      this.i_ = this.i_.add(new __PRIVATE_DocReference(t3.key, i2)), this.indexManager.addToCollectionParentIndex(e2, t3.key.path.popLast());
    return PersistencePromise.resolve(s2);
  }
  lookupMutationBatch(e2, t2) {
    return PersistencePromise.resolve(this.s_(t2));
  }
  getNextMutationBatchAfterBatchId(e2, t2) {
    const n2 = t2 + 1, r2 = this.__(n2), i2 = r2 < 0 ? 0 : r2;
    return PersistencePromise.resolve(this.mutationQueue.length > i2 ? this.mutationQueue[i2] : null);
  }
  getHighestUnacknowledgedBatchId() {
    return PersistencePromise.resolve(this.mutationQueue.length === 0 ? Ke : this.Gr - 1);
  }
  getAllMutationBatches(e2) {
    return PersistencePromise.resolve(this.mutationQueue.slice());
  }
  getAllMutationBatchesAffectingDocumentKey(e2, t2) {
    const n2 = new __PRIVATE_DocReference(t2, 0), r2 = new __PRIVATE_DocReference(t2, Number.POSITIVE_INFINITY), i2 = [];
    return this.i_.forEachInRange([n2, r2], (e3) => {
      const t3 = this.s_(e3.r_);
      i2.push(t3);
  ...
  getAllMutationBatchesAffectingDocumentKeys(e2, t2) {
    ...
    return t2.forEach((e3) => {
      ...
      this.i_.forEachInRange([t3, r2], (e4) => {
        n2 = n2.add(e4.r_);
      });
    }), PersistencePromise.resolve(this.o_(n2));
  }
  getAllMutationBatchesAffectingQuery(e2, t2) {
    const n2 = t2.path, r2 = n2.length + 1;
    let i2 = n2;
    DocumentKey.isDocumentKey(i2) || (i2 = i2.child(""));
    const s2 = new __PRIVATE_DocReference(new DocumentKey(i2), 0);
    let _ = new SortedSet(__PRIVATE_primitiveComparator);
    return this.i_.forEachWhile((e3) => {
      const t3 = e3.key.path;
      return !!n2.isPrefixOf(t3) && (t3.length === r2 && (_ = _.add(e3.r_)), true);
    }, s2), PersistencePromise.resolve(this.o_(_));
  }
  o_(e2) {
    const t2 = [];
  ...
  removeMutationBatch(e2, t2) {
    __PRIVATE_hardAssert(this.a_(t2.batchId, "removed") === 0, 55003), this.mutationQueue.shift();
    let n2 = this.i_;
    return PersistencePromise.forEach(t2.mutations, (r2) => {
      const i2 = new __PRIVATE_DocReference(r2.key, t2.batchId);
      return n2 = n2.delete(i2), this.referenceDelegate.markPotentiallyOrphaned(e2, r2.key);
    }).next(() => {
      this.i_ = n2;
    });
  }
  Hr(e2) {}
  containsKey(e2, t2) {
    const n2 = new __PRIVATE_DocReference(t2, 0), r2 = this.i_.firstAfterOrEqual(n2);
...
class __PRIVATE_MemoryRemoteDocumentCacheImpl {
  constructor(e2) {
    this.u_ = e2, this.docs = function __PRIVATE_documentEntryMap() {
      return new SortedMap(DocumentKey.comparator);
    }(), this.size = 0;
  }
  setIndexManager(e2) {
  ...
  addEntry(e2, t2) {
    const n2 = t2.key, r2 = this.docs.get(n2), i2 = r2 ? r2.size : 0, s2 = this.u_(t2);
    return this.docs = this.docs.insert(n2, {
      document: t2.mutableCopy(),
      size: s2
    }), this.size += s2 - i2, this.indexManager.addToCollectionParentIndex(e2, n2.path.popLast());
  }
  removeEntry(e2) {
  ...
  getEntries(e2, t2) {
    ...
    return t2.forEach((e3) => {
      ...
      n2 = n2.insert(e3, t3 ? t3.document.mutableCopy() : MutableDocument.newInvalidDocument(e3));
    }), PersistencePromise.resolve(n2);
  }
  getAllEntries(e2) {
    let t2 = __PRIVATE_mutableDocumentMap();
    return this.docs.forEach((e3, n2) => {
      t2 = t2.insert(e3, n2.document);
    }), PersistencePromise.resolve(t2);
  }
  getDocumentsMatchingQuery(e2, t2, n2, r2) {
    let i2, s2;
    __PRIVATE_isPipeline(t2) ? (i2 = ResourcePath.fromString(getPipelineCollection(t2)), s2 = (e3) => __PRIVATE_pipelineMatches(t2, e3)) : (i2 = t2.path, s2 = (e3) => __PRIVATE_queryMatches(t2, e3));
    let _ = __PRIVATE_mutableDocumentMap();
    const o2 = new DocumentKey(i2.child("__id-9223372036854775808__")), a = this.docs.getIteratorFrom(o2);
    for (;a.hasNext(); ) {
      const { key: e3, value: { document: t3 } } = a.getNext();
      if (!i2.isPrefixOf(e3.path))
        break;
      e3.path.length > i2.length + 1 || (__PRIVATE_indexOffsetComparator(__PRIVATE_newIndexOffsetFromDocument(t3), n2) <= 0 || (r2.has(t3.key) || s2(t3)) && (_ = _.insert(t3.key, t3.mutableCopy())));
    }
    return PersistencePromise.resolve(_);
  }
  getAllFromCollectionGroup(e2, t2, n2, r2) {
    l(9500);
  }
  c_(e2, t2) {
...
class __PRIVATE_MemoryRemoteDocumentChangeBuffer extends RemoteDocumentChangeBuffer {
  ...
  applyChanges(e2) {
    ...
    return this.changes.forEach((n2, r2) => {
      r2.isValidDocument() ? t2.push(this.$s.addEntry(e2, r2)) : this.$s.removeEntry(n2);
    }), PersistencePromise.waitFor(t2);
  }
  getFromCache(e2, t2) {
    return this.$s.getEntry(e2, t2);
  }
  getAllFromCache(e2, t2) {
    return this.$s.getEntries(e2, t2);
  }
}

class __PRIVATE_MemoryTargetCache {
  constructor(e2) {
    this.persistence = e2, this.l_ = new ObjectMap((e3) => __PRIVATE_canonifyTargetOrPipeline(e3), __PRIVATE_targetOrPipelineEqual), this.lastRemoteSnapshotVersion = SnapshotVersion.min(), this.highestTargetId = 0, this.E_ = 0, this.h_ = new __PRIVATE_ReferenceSet, this.targetCount = 0, this.T_ = __PRIVATE_TargetIdGenerator.ws();
  }
  forEachTarget(e2, t2) {
    return this.l_.forEach((e3, n2) => t2(n2)), PersistencePromise.resolve();
  }
  ...
  setTargetsMetadata(e2, t2, n2) {
    return n2 && (this.lastRemoteSnapshotVersion = n2), t2 > this.E_ && (this.E_ = t2), PersistencePromise.resolve();
  }
  Ds(e2) {
    this.l_.set(e2.target, e2);
    const t2 = e2.targetId;
    t2 > this.highestTargetId && (this.T_ = new __PRIVATE_TargetIdGenerator(t2), this.highestTargetId = t2), e2.sequenceNumber > this.E_ && (this.E_ = e2.sequenceNumber);
  }
  addTargetData(e2, t2) {
    return this.Ds(t2), this.targetCount += 1, PersistencePromise.resolve();
  }
  updateTargetData(e2, t2) {
    return this.Ds(t2), PersistencePromise.resolve();
  }
  removeTargetData(e2, t2) {
    return this.l_.delete(t2.target), this.h_.e_(t2.targetId), this.targetCount -= 1, PersistencePromise.resolve();
  }
  removeTargets(e2, t2, n2) {
    let r2 = 0;
    const i2 = [];
    return this.l_.forEach((s2, _) => {
      _.sequenceNumber <= t2 && n2.get(_.targetId) === null && (this.l_.delete(s2), i2.push(this.removeMatchingKeysForTargetId(e2, _.targetId)), r2++);
    }), PersistencePromise.waitFor(i2).next(() => r2);
  }
  getTargetCount(e2) {
    return PersistencePromise.resolve(this.targetCount);
  }
  getTargetData(e2, t2) {
    const n2 = this.l_.get(t2) || null;
    return PersistencePromise.resolve(n2);
  }
  addMatchingKeys(e2, t2, n2) {
    return this.h_.Ys(t2, n2), PersistencePromise.resolve();
  }
  removeMatchingKeys(e2, t2, n2) {
    this.h_.Xs(t2, n2);
    const r2 = this.persistence.referenceDelegate, i2 = [];
    return r2 && t2.forEach((t3) => {
      i2.push(r2.markPotentiallyOrphaned(e2, t3));
    }), PersistencePromise.waitFor(i2);
  }
  removeMatchingKeysForTargetId(e2, t2) {
    return this.h_.e_(t2), PersistencePromise.resolve();
  }
  getMatchingKeysForTargetId(e2, t2) {
    const n2 = this.h_.n_(t2);
    return PersistencePromise.resolve(n2);
  }
  containsKey(e2, t2) {
    return PersistencePromise.resolve(this.h_.containsKey(t2));
  }
}

class __PRIVATE_MemoryPersistence {
  constructor(e2, t2) {
    this.P_ = {}, this.overlays = {}, this.I_ = new __PRIVATE_ListenSequence(0), this.R_ = false, this.R_ = true, this.A_ = new __PRIVATE_MemoryGlobalsCache, this.referenceDelegate = e2(this), this.V_ = new __PRIVATE_MemoryTargetCache(this);
    ...
    }((e3) => this.referenceDelegate.d_(e3)), this.serializer = new __PRIVATE_LocalSerializer(t2), this.f_ = new __PRIVATE_MemoryBundleCache(this.serializer);
  }
  start() {
    return Promise.resolve();
  }
  ...
  getMutationQueue(e2, t2) {
    let n2 = this.P_[e2.toKey()];
    return n2 || (n2 = new __PRIVATE_MemoryMutationQueue(t2, this.referenceDelegate), this.P_[e2.toKey()] = n2), n2;
  }
  getGlobalsCache() {
    return this.A_;
  }
  ...
  runTransaction(e2, t2, n2) {
    __PRIVATE_logDebug("MemoryPersistence", "Starting transaction:", e2);
    const r2 = new __PRIVATE_MemoryTransaction(this.I_.next());
    return this.referenceDelegate.m_(), n2(r2).next((e3) => this.referenceDelegate.p_(r2).next(() => e3)).toPromise().then((e3) => (r2.raiseOnCommittedEvent(), e3));
  }
  g_(e2, t2) {
    return PersistencePromise.or(Object.values(this.P_).map((n2) => () => n2.containsKey(e2, t2)));
  }
}
...
class __PRIVATE_MemoryEagerDelegate {
  constructor(e2) {
    this.persistence = e2, this.y_ = new __PRIVATE_ReferenceSet, this.w_ = null;
  }
  static b_(e2) {
  ...
  addReference(e2, t2, n2) {
    return this.y_.addReference(n2, t2), this.S_.delete(n2.toString()), PersistencePromise.resolve();
  }
  removeReference(e2, t2, n2) {
    return this.y_.removeReference(n2, t2), this.S_.add(n2.toString()), PersistencePromise.resolve();
  }
  markPotentiallyOrphaned(e2, t2) {
    return this.S_.add(t2.toString()), PersistencePromise.resolve();
  }
  removeTarget(e2, t2) {
    this.y_.e_(t2.targetId).forEach((e3) => this.S_.add(e3.toString()));
    const n2 = this.persistence.getTargetCache();
    return n2.getMatchingKeysForTargetId(e2, t2.targetId).next((e3) => {
      e3.forEach((e4) => this.S_.add(e4.toString()));
    }).next(() => n2.removeTargetData(e2, t2));
  }
  m_() {
    this.w_ = new Set;
  }
  p_(e2) {
    ...
    return PersistencePromise.forEach(this.S_, (n2) => {
      const r2 = DocumentKey.fromPath(n2);
      return this.v_(e2, r2).next((e3) => {
        e3 || t2.removeEntry(r2, SnapshotVersion.min());
      });
    }).next(() => (this.w_ = null, t2.apply(e2)));
  }
  updateLimboDocument(e2, t2) {
    return this.v_(e2, t2).next((e3) => {
      e3 ? this.S_.delete(t2.toString()) : this.S_.add(t2.toString());
    });
  }
  d_(e2) {
    return 0;
  ...
  v_(e2, t2) {
    return PersistencePromise.or([() => PersistencePromise.resolve(this.y_.containsKey(t2)), () => this.persistence.getTargetCache().containsKey(e2, t2), () => this.persistence.g_(e2, t2)]);
  }
}

class __PRIVATE_MemoryLruDelegate {
  constructor(e2, t2) {
    this.persistence = e2, this.D_ = new ObjectMap((e3) => __PRIVATE_encodeResourcePath(e3.path), (e3, t3) => e3.isEqual(t3)), this.garbageCollector = __PRIVATE_newLruGarbageCollector(this, t2);
  }
  static b_(e2, t2) {
  ...
  Cs(e2) {
    let t2 = 0;
    return this.sr(e2, (e3) => {
      t2++;
    }).next(() => t2);
  }
  sr(e2, t2) {
    return PersistencePromise.forEach(this.D_, (n2, r2) => this.Os(e2, n2, r2).next((e3) => e3 ? PersistencePromise.resolve() : t2(r2)));
  }
  removeTargets(e2, t2, n2) {
    return this.persistence.getTargetCache().removeTargets(e2, t2, n2);
  }
  removeOrphanedDocuments(e2, t2) {
    let n2 = 0;
    const r2 = this.persistence.getRemoteDocumentCache(), i2 = r2.newChangeBuffer();
    return r2.c_(e2, (r3) => this.Os(e2, r3, t2).next((e3) => {
      e3 || (n2++, i2.removeEntry(r3, SnapshotVersion.min()));
    })).next(() => i2.apply(e2)).next(() => n2);
  }
  markPotentiallyOrphaned(e2, t2) {
    return this.D_.set(t2, e2.currentSequenceNumber), PersistencePromise.resolve();
  }
  removeTarget(e2, t2) {
    const n2 = t2.withSequenceNumber(e2.currentSequenceNumber);
    return this.persistence.getTargetCache().updateTargetData(e2, n2);
  }
  addReference(e2, t2, n2) {
    return this.D_.set(n2, e2.currentSequenceNumber), PersistencePromise.resolve();
  }
  removeReference(e2, t2, n2) {
    return this.D_.set(n2, e2.currentSequenceNumber), PersistencePromise.resolve();
  }
  ...
  Os(e2, t2, n2) {
    return PersistencePromise.or([() => this.persistence.g_(e2, t2), () => this.persistence.getTargetCache().containsKey(e2, t2), () => {
      const e3 = this.D_.get(t2);
      return PersistencePromise.resolve(e3 !== undefined && e3 > n2);
    }]);
  }
  getCacheSize(e2) {
...
class __PRIVATE_LocalViewChanges {
  constructor(e2, t2, n2, r2) {
    this.targetId = e2, this.fromCache = t2, this.Vo = n2, this.fo = r2;
  }
  static mo(e2, t2) {
    let n2 = __PRIVATE_documentKeySet(), r2 = __PRIVATE_documentKeySet();
    for (const e3 of t2.docChanges)
      switch (e3.type) {
        case 0:
          n2 = n2.add(e3.doc.key);
          break;
        case 1:
          r2 = r2.add(e3.doc.key);
      }
    return new __PRIVATE_LocalViewChanges(e2, t2.fromCache, n2, r2);
  }
}
function __PRIVATE_compareByKey(e2, t2) {
  return DocumentKey.comparator(e2.key, t2.key);
}
...
class __PRIVATE_QueryEngine {
  ...
  initialize(e2, t2) {
    this.So = e2, this.indexManager = t2, this.po = true;
  }
  getDocumentsMatchingQuery(e2, t2, n2, r2) {
    const i2 = {
      result: null
    };
    return this.vo(e2, t2).next((e3) => {
      i2.result = e3;
    }).next(() => {
      if (!i2.result)
        return this.Do(e2, t2, r2, n2).next((e3) => {
          i2.result = e3;
        });
    }).next(() => {
      if (i2.result)
      ...
      return this.xo(e2, t2, n3).next((r3) => {
        if (i2.result = r3, this.yo)
          return this.Co(e2, t2, n3, r3.size);
      });
    }).next(() => i2.result);
  }
  Co(e2, t2, n2, r2) {
    return __PRIVATE_isPipeline(t2) ? PersistencePromise.resolve() : n2.documentReadCount < this.wo ? (__PRIVATE_getLogLevel() <= LogLevel.DEBUG && __PRIVATE_logDebug("QueryEngine", "SDK will not create cache indexes for query:", __PRIVATE_stringifyQuery(t2), "since it only creates cache indexes for collection contains", "more than or equal to", this.wo, "documents"), PersistencePromise.resolve()) : (__PRIVATE_getLogLevel() <= LogLevel.DEBUG && __PRIVATE_logDebug("QueryEngine", "Query:", __PRIVATE_stringifyQuery(t2), "scans", n2.documentReadCount, "local documents and returns", r2, "documents as results."), n2.documentReadCount > this.bo * r2 ? (__PRIVATE_getLogLevel() <= LogLevel.DEBUG && __PRIVATE_logDebug("QueryEngine", "The SDK decides to create cache indexes for query:", __PRIVATE_stringifyQuery(t2), "as using cache indexes may help improve performance."), this.indexManager.createTargetIndexes(e2, __PRIVATE_queryToTarget(t2))) : PersistencePromise.resolve());
  }
  vo(e2, t2) {
    if (__PRIVATE_isPipeline(t2))
      return PersistencePromise.resolve(null);
    let n2 = t2;
    if (__PRIVATE_queryMatchesAllDocuments(n2))
      return PersistencePromise.resolve(null);
    let r2 = __PRIVATE_queryToTarget(n2);
    return this.indexManager.getIndexType(e2, r2).next((t3) => t3 === 0 ? null : (n2.limit !== null && t3 === 1 && (n2 = __PRIVATE_queryWithLimit(n2, null, "F"), r2 = __PRIVATE_queryToTarget(n2)), this.indexManager.getDocumentsMatchingTarget(e2, r2).next((t4) => {
      const i2 = __PRIVATE_documentKeySet(...t4);
      return this.So.getDocuments(e2, i2).next((t5) => this.indexManager.getMinOffset(e2, r2).next((r3) => {
        const s2 = this.Fo(n2, t5);
        return this.Oo(n2, s2, i2, r3.readTime) ? this.vo(e2, __PRIVATE_queryWithLimit(n2, null, "F")) : this.Mo(e2, s2, n2, r3);
      }));
    })));
  }
  Do(e2, t2, n2, r2) {
    return (__PRIVATE_isPipeline(t2) ? function __PRIVATE_pipelineMatchesAllDocuments(e3) {
      for (const t3 of e3.stages) {
        if (t3 instanceof v || t3 instanceof I)
          return false;
        if (t3 instanceof A) {
          if (t3.condition instanceof __PRIVATE_BooleanFunctionExpression && t3.condition._expr.name === "exists" && t3.condition._expr.params[0] instanceof o && t3.condition._expr.params[0].fieldName === Ce)
            continue;
          return false;
        }
      }
      return true;
    }(t2) : __PRIVATE_queryMatchesAllDocuments(t2)) || r2.isEqual(SnapshotVersion.min()) ? PersistencePromise.resolve(null) : this.So.getDocuments(e2, n2).next((i2) => {
      const s2 = this.Fo(t2, i2);
      return this.Oo(t2, s2, n2, r2) ? PersistencePromise.resolve(null) : (__PRIVATE_getLogLevel() <= LogLevel.DEBUG && __PRIVATE_logDebug("QueryEngine", "Re-using previous result from %s to execute query: %s", r2.toString(), __PRIVATE_stringifyQueryOrPipeline(t2)), this.Mo(e2, s2, t2, __PRIVATE_newIndexOffsetSuccessorFromReadTime(r2, Xe)).next((e3) => e3));
    });
  }
  Fo(e2, t2) {
    let n2, r2;
    return __PRIVATE_isPipeline(e2) ? (n2 = new SortedSet(__PRIVATE_compareByKey), r2 = (t3) => __PRIVATE_pipelineMatches(e2, t3)) : (n2 = new SortedSet(__PRIVATE_newQueryComparator(e2)), r2 = (t3) => __PRIVATE_queryMatches(e2, t3)), t2.forEach((e3, t3) => {
      r2(t3) && (n2 = n2.add(t3));
    }), n2;
  }
  Oo(e2, t2, n2, r2) {
    if (__PRIVATE_isPipeline(e2))
      return function __PRIVATE_pipelineHasRanges(e3) {
        return e3.stages.some((e4) => e4 instanceof v || e4 instanceof I);
      }(e2);
    if (e2.limit === null)
      return false;
    if (n2.size !== t2.size)
      return true;
    const i2 = e2.limitType === "F" ? t2.last() : t2.first();
    return !!i2 && (i2.hasPendingWrites || i2.version.compareTo(r2) > 0);
  }
  xo(e2, t2, n2) {
    return __PRIVATE_getLogLevel() <= LogLevel.DEBUG && __PRIVATE_logDebug("QueryEngine", "Using full collection scan to execute query:", __PRIVATE_stringifyQueryOrPipeline(t2)), this.So.getDocumentsMatchingQuery(e2, t2, IndexOffset.min(), n2);
  }
  Mo(e2, t2, n2, r2) {
    return this.So.getDocumentsMatchingQuery(e2, n2, r2).next((e3) => (t2.forEach((t3) => {
...
class __PRIVATE_LocalStoreImpl {
  constructor(e2, t2, n2, r2) {
    this.persistence = e2, this.No = t2, this.serializer = r2, this.Lo = new SortedMap(__PRIVATE_primitiveComparator), this.Bo = new ObjectMap((e3) => __PRIVATE_canonifyTargetOrPipeline(e3), __PRIVATE_targetOrPipelineEqual), this.Uo = new Map, this.ko = e2.getRemoteDocumentCache(), this.V_ = e2.getTargetCache(), this.f_ = e2.getBundleCache(), this.qo(n2);
  }
  qo(e2) {
    this.documentOverlayCache = this.persistence.getDocumentOverlayCache(e2), this.indexManager = this.persistence.getIndexManager(e2), this.mutationQueue = this.persistence.getMutationQueue(e2, this.indexManager), this.localDocuments = new LocalDocumentsView(this.ko, this.mutationQueue, this.documentOverlayCache, this.indexManager), this.ko.setIndexManager(this.indexManager), this.No.initialize(this.localDocuments, this.indexManager);
  }
  collectGarbage(e2) {
    return this.persistence.runTransaction("Collect garbage", "readwrite-primary", (t2) => e2.collect(t2, this.Lo));
  }
}
function __PRIVATE_newLocalStore(e2, t2, n2, r2) {
  return new __PRIVATE_LocalStoreImpl(e2, t2, n2, r2);
}
async function __PRIVATE_localStoreHandleUserChange(e2, t2) {
  const n2 = __PRIVATE_debugCast(e2);
  return await n2.persistence.runTransaction("Handle user change", "readonly", (e3) => {
    let r2;
    return n2.mutationQueue.getAllMutationBatches(e3).next((i2) => (r2 = i2, n2.qo(t2), n2.mutationQueue.getAllMutationBatches(e3))).next((t3) => {
      const i2 = [], s2 = [];
      let _ = __PRIVATE_documentKeySet();
      for (const e4 of r2) {
        i2.push(e4.batchId);
        for (const t4 of e4.mutations)
          _ = _.add(t4.key);
      }
      for (const e4 of t3) {
        s2.push(e4.batchId);
      ...
      return n2.localDocuments.getDocuments(e3, _).next((e4) => ({
        $o: e4,
        removedBatchIds: i2,
        addedBatchIds: s2
      }));
...
function __PRIVATE_localStoreAcknowledgeBatch(e2, t2) {
  ...
  return n2.persistence.runTransaction("Acknowledge batch", "readwrite-primary", (e3) => {
    const r2 = t2.batch.keys(), i2 = n2.ko.newChangeBuffer({
      trackRemovals: true
    });
    return function __PRIVATE_applyWriteToRemoteDocuments(e4, t3, n3, r3) {
      ...
      return s2.forEach((e5) => {
        _ = _.next(() => r3.getEntry(t3, e5)).next((t4) => {
          ...
          __PRIVATE_hardAssert(s3 !== null, 48541), t4.version.compareTo(s3) < 0 && (i3.applyToRemoteDocument(t4, n3), t4.isValidDocument() && (t4.setReadTime(n3.commitVersion), r3.addEntry(t4)));
        });
...
function __PRIVATE_localStoreGetLastRemoteSnapshotVersion(e2) {
  ...
  return t2.persistence.runTransaction("Get last remote snapshot version", "readonly", (e3) => t2.V_.getLastRemoteSnapshotVersion(e3));
}
function __PRIVATE_localStoreApplyRemoteEventToLocalCache(e2, t2) {
  const n2 = __PRIVATE_debugCast(e2), r2 = t2.snapshotVersion;
  let i2 = n2.Lo;
  return n2.persistence.runTransaction("Apply remote event", "readwrite-primary", (e3) => {
    const s2 = n2.ko.newChangeBuffer({
      trackRemovals: true
    });
    i2 = n2.Lo;
    const _ = [];
    t2.targetChanges.forEach((s3, o3) => {
      const a2 = i2.get(o3);
      if (!a2)
        return;
      _.push(n2.V_.removeMatchingKeys(e3, s3.removedDocuments, o3).next(() => n2.V_.addMatchingKeys(e3, s3.addedDocuments, o3)));
      let u2 = a2.withSequenceNumber(e3.currentSequenceNumber);
      t2.targetMismatches.get(o3) !== null ? u2 = u2.withResumeToken(ByteString.EMPTY_BYTE_STRING, SnapshotVersion.min()).withLastLimboFreeSnapshotVersion(SnapshotVersion.min()) : s3.resumeToken.approximateByteSize() > 0 && (u2 = u2.withResumeToken(s3.resumeToken, r2)), i2 = i2.insert(o3, u2), function __PRIVATE_shouldPersistTargetData(e4, t3, n3) {
        if (e4.resumeToken.approximateByteSize() === 0)
          return true;
        ...
        return i3 > 0;
      }(a2, u2, s3) && _.push(n2.V_.updateTargetData(e3, u2));
    });
    let o2 = __PRIVATE_mutableDocumentMap(), a = __PRIVATE_documentKeySet();
    if (t2.documentUpdates.forEach((r3) => {
      t2.resolvedLimboDocuments.has(r3) && _.push(n2.persistence.referenceDelegate.updateLimboDocument(e3, r3));
    }), _.push(__PRIVATE_populateDocumentChangeBuffer(e3, s2, t2.documentUpdates).next((e4) => {
      o2 = e4.Ko, a = e4.Qo;
    })), !r2.isEqual(SnapshotVersion.min())) {
      const t3 = n2.V_.getLastRemoteSnapshotVersion(e3).next((t4) => n2.V_.setTargetsMetadata(e3, e3.currentSequenceNumber, r2));
      _.push(t3);
    }
    return PersistencePromise.waitFor(_).next(() => s2.apply(e3)).next(() => n2.localDocuments.getLocalViewOfDocuments(e3, o2, a)).next(() => o2);
  }).then((e3) => (n2.Lo = i2, e3));
}
function __PRIVATE_populateDocumentChangeBuffer(e2, t2, n2) {
  let r2 = __PRIVATE_documentKeySet(), i2 = __PRIVATE_documentKeySet();
  return n2.forEach((e3) => r2 = r2.add(e3)), t2.getEntries(e2, r2).next((e3) => {
    let r3 = __PRIVATE_mutableDocumentMap();
    return n2.forEach((n3, s2) => {
      const _ = e3.get(n3);
      s2.isFoundDocument() !== _.isFoundDocument() && (i2 = i2.add(n3)), s2.isNoDocument() && s2.version.isEqual(SnapshotVersion.min()) ? (t2.removeEntry(n3, s2.readTime), r3 = r3.insert(n3, s2)) : !_.isValidDocument() || s2.version.compareTo(_.version) > 0 || s2.version.compareTo(_.version) === 0 && _.hasPendingWrites ? (t2.addEntry(s2), r3 = r3.insert(n3, s2)) : __PRIVATE_logDebug(lr, "Ignoring outdated watch update for ", n3, ". Current version:", _.version, " Watch version:", s2.version);
    }), {
      Ko: r3,
      Qo: i2
    };
...
function __PRIVATE_localStoreGetNextMutationBatch(e2, t2) {
  const n2 = __PRIVATE_debugCast(e2);
  return n2.persistence.runTransaction("Get next mutation batch", "readonly", (e3) => (t2 === undefined && (t2 = Ke), n2.mutationQueue.getNextMutationBatchAfterBatchId(e3, t2)));
}
function __PRIVATE_localStoreAllocateTarget(e2, t2) {
  const n2 = __PRIVATE_debugCast(e2);
  return n2.persistence.runTransaction("Allocate target", "readwrite", (e3) => {
    let r2;
    return n2.V_.getTargetData(e3, t2).next((i2) => i2 ? (r2 = i2, PersistencePromise.resolve(r2)) : n2.V_.allocateTargetId(e3).next((i3) => (r2 = new TargetData(t2, i3, "TargetPurposeListen", e3.currentSequenceNumber), n2.V_.addTargetData(e3, r2).next(() => r2))));
  }).then((e3) => {
    const r2 = n2.Lo.get(e3.targetId);
    return (r2 === null || e3.snapshotVersion.compareTo(r2.snapshotVersion) > 0) && (n2.Lo = n2.Lo.insert(e3.targetId, e3), n2.Bo.set(t2, e3.targetId)), e3;
  });
}
async function __PRIVATE_localStoreReleaseTarget(e2, t2, n2) {
  const r2 = __PRIVATE_debugCast(e2), i2 = r2.Lo.get(t2), s2 = n2 ? "readwrite" : "readwrite-primary";
  try {
    n2 || await r2.persistence.runTransaction("Release target", s2, (e3) => r2.persistence.referenceDelegate.removeTarget(e3, i2));
  } catch (e3) {
    if (!__PRIVATE_isIndexedDbTransactionError(e3))
      throw e3;
    __PRIVATE_logDebug(lr, `Failed to update sequence numbers for target ${t2}: ${e3}`);
  }
  r2.Lo = r2.Lo.remove(t2), r2.Bo.delete(i2.target);
}
function __PRIVATE_localStoreExecuteQuery(e2, t2, n2) {
  const r2 = __PRIVATE_debugCast(e2);
  let i2 = SnapshotVersion.min(), s2 = __PRIVATE_documentKeySet();
  return r2.persistence.runTransaction("Execute query", "readwrite", (e3) => function __PRIVATE_localStoreGetTargetData(e4, t3, n3) {
    const r3 = __PRIVATE_debugCast(e4), i3 = r3.Bo.get(n3);
    return i3 !== undefined ? PersistencePromise.resolve(r3.Lo.get(i3)) : r3.V_.getTargetData(t3, n3);
  }(r2, e3, __PRIVATE_isPipeline(t2) ? t2 : __PRIVATE_queryToTarget(t2)).next((t3) => {
    if (t3)
      return i2 = t3.lastLimboFreeSnapshotVersion, r2.V_.getMatchingKeysForTargetId(e3, t3.targetId).next((e4) => {
        s2 = e4;
      });
  }).next(() => r2.No.getDocumentsMatchingQuery(e3, t2, n2 ? i2 : SnapshotVersion.min(), n2 ? s2 : __PRIVATE_documentKeySet())).next((e4) => (__PRIVATE_setMaxReadTime(r2, e4), {
    documents: e4,
    Wo: s2
  })));
}
function __PRIVATE_setMaxReadTime(e2, t2) {
  t2.forEach((t3, n2) => {
    const r2 = n2.key.getCollectionGroup(), i2 = e2.Uo.get(r2) || SnapshotVersion.min();
    n2.readTime.compareTo(i2) > 0 && e2.Uo.set(r2, n2.readTime);
  });
}
class __PRIVATE_OnlineStateTracker {
  constructor(e2, t2) {
    this.asyncQueue = e2, this.onlineStateHandler = t2, this.state = "Unknown", this.Yo = 0, this.Zo = null, this.Xo = true;
  }
  ea() {
  ...
  ra(e2) {
    this.state === "Online" ? this.ta("Unknown") : (this.Yo++, this.Yo >= 1 && (this.ia(), this.na(`Connection failed 1 times. Most recent error: ${e2.toString()}`), this.ta("Offline")));
  }
  set(e2) {
    this.ia(), this.Yo = 0, e2 === "Online" && (this.Xo = false), this.ta(e2);
  }
  ta(e2) {
...
This typically indicates that your device does not have a healthy Internet connection at the moment. The client will operate in offline mode until it is able to successfully connect to the backend.`;
    this.Xo ? (__PRIVATE_logError(t2), this.Xo = false) : __PRIVATE_logDebug("OnlineStateTracker", t2);
  }
  ia() {
...
class __PRIVATE_RemoteStoreImpl {
  constructor(e2, t2, n2, r2, i2) {
    this.localStore = e2, this.datastore = t2, this.asyncQueue = n2, this.remoteSyncer = {}, this.sa = [], this._a = new Map, this.oa = new Map, this.aa = new Map, this.ua = new __PRIVATE_TargetIdGenerator(1000), this.ca = new __PRIVATE_TargetIdGenerator(1001), this.la = new Set, this.Ea = [], this.ha = i2, this.ha.Qe((e3) => {
      n2.enqueueAndForget(async () => {
        __PRIVATE_canUseNetwork(this) && (__PRIVATE_logDebug(hr, "Restarting streams for network reachability change."), await async function __PRIVATE_restartNetwork(e4) {
          const t3 = __PRIVATE_debugCast(e4);
          t3.la.add(4), await __PRIVATE_disableNetworkInternal(t3), t3.Ta.set("Unknown"), t3.la.delete(4), await __PRIVATE_enableNetworkInternal(t3);
        }(this));
      });
    }), this.Ta = new __PRIVATE_OnlineStateTracker(n2, r2);
  }
}
async function __PRIVATE_enableNetworkInternal(e2) {
  if (__PRIVATE_canUseNetwork(e2))
    for (const t2 of e2.Ea)
      await t2(true);
}
...
function __PRIVATE_getRemoteTargetId(e2, t2) {
  return e2.oa.get(t2) || undefined;
}
function __PRIVATE_remoteStoreListen(e2, t2) {
  const n2 = __PRIVATE_debugCast(e2), r2 = __PRIVATE_getRemoteTargetId(n2, t2.targetId);
  if (r2 !== undefined && n2._a.has(r2))
    return;
  const i2 = function __PRIVATE_allocateRemoteTargetId(e3, t3) {
    const n3 = __PRIVATE_getRemoteTargetId(e3, t3);
    n3 !== undefined && e3.aa.delete(n3);
    const r3 = function __PRIVATE_generateRemoteTargetId(e4, t4) {
      return t4 % 2 != 0 ? e4.ca.next() : e4.ua.next();
    }(e3, t3);
    return e3.oa.set(t3, r3), e3.aa.set(r3, t3), r3;
  }(n2, t2.targetId);
  __PRIVATE_logDebug(hr, "remoteStoreListen mapping SDK target ID to remote", t2.targetId, i2);
  const s2 = new TargetData(t2.target, i2, t2.purpose, t2.sequenceNumber, t2.snapshotVersion, t2.lastLimboFreeSnapshotVersion, t2.resumeToken);
  n2._a.set(i2, s2), __PRIVATE_shouldStartWatchStream(n2) ? __PRIVATE_startWatchStream(n2) : __PRIVATE_ensureWatchStream(n2).Yt() && __PRIVATE_sendWatchRequest(n2, s2);
}
function __PRIVATE_remoteStoreUnlisten(e2, t2) {
  const n2 = __PRIVATE_debugCast(e2), r2 = __PRIVATE_ensureWatchStream(n2), i2 = __PRIVATE_getRemoteTargetId(n2, t2);
  __PRIVATE_logDebug(hr, "remoteStoreUnlisten removing mapping of SDK target ID to remote", t2, i2), n2._a.delete(i2), n2.oa.delete(t2), n2.aa.delete(i2), r2.Yt() && __PRIVATE_sendUnwatchRequest(n2, i2), n2._a.size === 0 && (r2.Yt() ? r2.en() : __PRIVATE_canUseNetwork(n2) && n2.Ta.set("Unknown"));
}
function __PRIVATE_sendWatchRequest(e2, t2) {
  if (e2.Pa.J(t2.targetId), t2.resumeToken.approximateByteSize() > 0 || t2.snapshotVersion.compareTo(SnapshotVersion.min()) > 0) {
    const n2 = e2.aa.get(t2.targetId);
    if (n2 === undefined)
      return void __PRIVATE_logDebug(hr, "SDK target ID not found for remote ID: " + t2.targetId);
    const r2 = e2.remoteSyncer.getRemoteKeysForTarget(n2).size;
    t2 = t2.withExpectedCount(r2);
  }
  __PRIVATE_ensureWatchStream(e2).Pn(t2);
}
function __PRIVATE_sendUnwatchRequest(e2, t2) {
  e2.Pa.J(t2), __PRIVATE_ensureWatchStream(e2).In(t2);
}
function __PRIVATE_startWatchStream(e2) {
  e2.Pa = new __PRIVATE_WatchChangeAggregator({
    getRemoteKeysForTarget: (t2) => {
      const n2 = e2.aa.get(t2);
      return n2 !== undefined ? e2.remoteSyncer.getRemoteKeysForTarget(n2) : __PRIVATE_documentKeySet();
    },
    ye: (t2) => e2._a.get(t2) || null,
    Ve: () => e2.datastore.serializer.databaseId
  }), __PRIVATE_ensureWatchStream(e2).start(), e2.Ta.ea();
}
function __PRIVATE_shouldStartWatchStream(e2) {
  return __PRIVATE_canUseNetwork(e2) && !__PRIVATE_ensureWatchStream(e2).Jt() && e2._a.size > 0;
}
function __PRIVATE_canUseNetwork(e2) {
  return __PRIVATE_debugCast(e2).la.size === 0;
}
...
async function __PRIVATE_onWatchStreamClose(e2, t2) {
  __PRIVATE_cleanUpWatchStreamState(e2), __PRIVATE_shouldStartWatchStream(e2) ? (e2.Ta.ra(t2), __PRIVATE_startWatchStream(e2)) : e2.Ta.set("Unknown");
}
async function __PRIVATE_onWatchStreamChange(e2, t2, n2) {
  if (e2.Ta.set("Online"), t2 instanceof __PRIVATE_WatchTargetChange && t2.state === 2 && t2.cause)
    try {
      await async function __PRIVATE_handleTargetError(e3, t3) {
        const n3 = t3.cause;
        for (const r2 of t3.targetIds) {
          if (e3._a.has(r2)) {
            const t4 = e3.aa.get(r2);
            t4 !== undefined && (await e3.remoteSyncer.rejectListen(t4, n3), e3.oa.delete(t4), e3.aa.delete(r2)), e3._a.delete(r2);
          }
          e3.Pa.removeTarget(r2);
        }
      }(e2, t2);
    } catch (n3) {
      __PRIVATE_logDebug(hr, "Failed to remove targets %s: %s ", t2.targetIds.join(","), n3), await __PRIVATE_disableNetworkUntilRecovery(e2, n3);
    }
  else if (t2 instanceof __PRIVATE_DocumentWatchChange ? e2.Pa._e(t2) : t2 instanceof __PRIVATE_ExistenceFilterChange ? e2.Pa.he(t2) : e2.Pa.ue(t2), !n2.isEqual(SnapshotVersion.min()))
    try {
      const t3 = await __PRIVATE_localStoreGetLastRemoteSnapshotVersion(e2.localStore);
      n2.compareTo(t3) >= 0 && await function __PRIVATE_raiseWatchSnapshot(e3, t4) {
        const n3 = e3.Pa.fe(t4);
        n3.targetChanges.forEach((n4, r3) => {
          if (n4.resumeToken.approximateByteSize() > 0) {
            const i2 = e3._a.get(r3);
            i2 && e3._a.set(r3, i2.withResumeToken(n4.resumeToken, t4));
          }
        }), n3.targetMismatches.forEach((t5, n4) => {
          const r3 = e3._a.get(t5);
          if (!r3)
            return;
          e3._a.set(t5, r3.withResumeToken(ByteString.EMPTY_BYTE_STRING, r3.snapshotVersion)), __PRIVATE_sendUnwatchRequest(e3, t5);
          const i2 = new TargetData(r3.target, t5, n4, r3.sequenceNumber);
          __PRIVATE_sendWatchRequest(e3, i2);
        });
        const r2 = function __PRIVATE_toSdkRemoteEvent(e4, t5) {
          const n4 = new Map;
          t5.targetChanges.forEach((t6, r4) => {
            const i2 = e4.aa.get(r4);
            i2 !== undefined && n4.set(i2, t6);
          });
          let r3 = new SortedMap(__PRIVATE_primitiveComparator);
          return t5.targetMismatches.forEach((t6, n5) => {
            const i2 = e4.aa.get(t6);
            i2 !== undefined && (r3 = r3.insert(i2, n5));
          }), new RemoteEvent(t5.snapshotVersion, n4, r3, t5.documentUpdates, t5.augmentedDocumentUpdates, t5.resolvedLimboDocuments);
        }(e3, n3);
        return e3.remoteSyncer.applyRemoteEvent(r2);
      }(e2, n2);
    } catch (t3) {
      __PRIVATE_logDebug(hr, "Failed to raise snapshot:", t3), await __PRIVATE_disableNetworkUntilRecovery(e2, t3);
    }
}
async function __PRIVATE_disableNetworkUntilRecovery(e2, t2, n2) {
  if (!__PRIVATE_isIndexedDbTransactionError(t2))
    throw t2;
  e2.la.add(1), await __PRIVATE_disableNetworkInternal(e2), e2.Ta.set("Offline"), n2 || (n2 = () => __PRIVATE_localStoreGetLastRemoteSnapshotVersion(e2.localStore)), e2.asyncQueue.enqueueRetryable(async () => {
    __PRIVATE_logDebug(hr, "Retrying IndexedDB access"), await n2(), e2.la.delete(1), await __PRIVATE_enableNetworkInternal(e2);
  });
}
...
function __PRIVATE_canAddToWritePipeline(e2) {
  return __PRIVATE_canUseNetwork(e2) && e2.sa.length < 10;
}
function __PRIVATE_addToWritePipeline(e2, t2) {
  e2.sa.push(t2);
  const n2 = __PRIVATE_ensureWriteStream(e2);
  n2.Yt() && n2.Rn && n2.An(t2.mutations);
}
function __PRIVATE_shouldStartWriteStream(e2) {
  return __PRIVATE_canUseNetwork(e2) && !__PRIVATE_ensureWriteStream(e2).Jt() && e2.sa.length > 0;
}
function __PRIVATE_startWriteStream(e2) {
  __PRIVATE_ensureWriteStream(e2).start();
}
async function __PRIVATE_onWriteStreamOpen(e2) {
  __PRIVATE_ensureWriteStream(e2).fn();
}
async function __PRIVATE_onWriteHandshakeComplete(e2) {
  const t2 = __PRIVATE_ensureWriteStream(e2);
  for (const n2 of e2.sa)
    t2.An(n2.mutations);
}
async function __PRIVATE_onMutationResult(e2, t2, n2) {
  const r2 = e2.sa.shift(), i2 = MutationBatchResult.from(r2, t2, n2);
  await __PRIVATE_executeWithRecovery(e2, () => e2.remoteSyncer.applySuccessfulWrite(i2)), await __PRIVATE_fillWritePipeline(e2);
}
async function __PRIVATE_onWriteStreamClose(e2, t2) {
  t2 && __PRIVATE_ensureWriteStream(e2).Rn && await async function __PRIVATE_handleWriteError(e3, t3) {
    if (function __PRIVATE_isPermanentWriteError(e4) {
      return __PRIVATE_isPermanentError(e4) && e4 !== ta.ABORTED;
    }(t3.code)) {
      const n2 = e3.sa.shift();
      __PRIVATE_ensureWriteStream(e3).Xt(), await __PRIVATE_executeWithRecovery(e3, () => e3.remoteSyncer.rejectFailedWrite(n2.batchId, t3)), await __PRIVATE_fillWritePipeline(e3);
    }
  }(e2, t2), __PRIVATE_shouldStartWriteStream(e2) && __PRIVATE_startWriteStream(e2);
}
async function __PRIVATE_remoteStoreHandleCredentialChange(e2, t2) {
  ...
  n2.la.add(3), await __PRIVATE_disableNetworkInternal(n2), r2 && n2.Ta.set("Unknown"), await n2.remoteSyncer.handleCredentialChange(t2), n2.la.delete(3), await __PRIVATE_enableNetworkInternal(n2);
}
async function __PRIVATE_remoteStoreApplyPrimaryState(e2, t2) {
  const n2 = __PRIVATE_debugCast(e2);
  t2 ? (n2.la.delete(2), await __PRIVATE_enableNetworkInternal(n2)) : t2 || (n2.la.add(2), await __PRIVATE_disableNetworkInternal(n2), n2.Ta.set("Unknown"));
}
function __PRIVATE_ensureWatchStream(e2) {
  return e2.Ia || (e2.Ia = function __PRIVATE_newPersistentWatchStream(e3, t2, n2) {
    const r2 = __PRIVATE_debugCast(e3);
    return r2.pn(), new __PRIVATE_PersistentListenStream(t2, r2.connection, r2.authCredentials, r2.appCheckCredentials, r2.serializer, n2);
  }(e2.datastore, e2.asyncQueue, {
    ct: __PRIVATE_onWatchStreamConnected.bind(null, e2),
    Et: __PRIVATE_onWatchStreamOpen.bind(null, e2),
    Tt: __PRIVATE_onWatchStreamClose.bind(null, e2),
    Tn: __PRIVATE_onWatchStreamChange.bind(null, e2)
  }), e2.Ea.push(async (t2) => {
    t2 ? (e2.Ia.Xt(), __PRIVATE_shouldStartWatchStream(e2) ? __PRIVATE_startWatchStream(e2) : e2.Ta.set("Unknown")) : (await e2.Ia.stop(), __PRIVATE_cleanUpWatchStreamState(e2));
  })), e2.Ia;
}
function __PRIVATE_ensureWriteStream(e2) {
  return e2.Ra || (e2.Ra = function __PRIVATE_newPersistentWriteStream(e3, t2, n2) {
    const r2 = __PRIVATE_debugCast(e3);
    return r2.pn(), new __PRIVATE_PersistentWriteStream(t2, r2.connection, r2.authCredentials, r2.appCheckCredentials, r2.serializer, n2);
  }(e2.datastore, e2.asyncQueue, {
    ct: () => Promise.resolve(),
    Et: __PRIVATE_onWriteStreamOpen.bind(null, e2),
    Tt: __PRIVATE_onWriteStreamClose.bind(null, e2),
    Vn: __PRIVATE_onWriteHandshakeComplete.bind(null, e2),
    dn: __PRIVATE_onMutationResult.bind(null, e2)
  }), e2.Ea.push(async (t2) => {
    t2 ? (e2.Ra.Xt(), await __PRIVATE_fillWritePipeline(e2)) : (await e2.Ra.stop(), e2.sa.length > 0 && (__PRIVATE_logDebug(hr, `Stopping write stream with ${e2.sa.length} pending writes`), e2.sa = []));
  })), e2.Ra;
}

class __PRIVATE_AsyncObserver {
  constructor(e2) {
    this.observer = e2, this.muted = false;
  }
  next(e2) {
    this.muted || this.observer.next && this.Aa(this.observer.next, e2);
  }
  error(e2) {
    this.muted || (this.observer.error ? this.Aa(this.observer.error, e2) : __PRIVATE_logError("Uncaught Error in snapshot listener:", e2.toString()));
  }
  Va() {
    this.muted = true;
  }
...
class DelayedOperation {
  constructor(e2, t2, n2, r2, i2) {
    this.asyncQueue = e2, this.timerId = t2, this.targetTimeMs = n2, this.op = r2, this.removalCallback = i2, this.deferred = new __PRIVATE_Deferred, this.then = this.deferred.promise.then.bind(this.deferred.promise), this.deferred.promise.catch((e3) => {});
  }
  get promise() {
    return this.deferred.promise;
  }
  static createAndSchedule(e2, t2, n2, r2, i2) {
    const s2 = Date.now() + n2, _ = new DelayedOperation(e2, t2, s2, r2, i2);
    return _.start(n2), _;
  }
  start(e2) {
  ...
  cancel(t2) {
    this.timerHandle !== null && (this.clearTimeout(), this.deferred.reject(new e(ta.CANCELLED, "Operation cancelled" + (t2 ? ": " + t2 : ""))));
  }
  handleDelayElapsed() {
    this.asyncQueue.enqueueAndForget(() => this.timerHandle !== null ? (this.clearTimeout(), this.op().then((e2) => this.deferred.resolve(e2))) : Promise.resolve());
  }
  clearTimeout() {
    this.timerHandle !== null && (this.removalCallback(this), clearTimeout(this.timerHandle), this.timerHandle = null);
  }
}
function __PRIVATE_wrapInUserErrorIfRecoverable(t2, n2) {
  if (__PRIVATE_logError("AsyncQueue", `${n2}: ${t2}`), __PRIVATE_isIndexedDbTransactionError(t2))
    return new e(ta.UNAVAILABLE, `${n2}: ${t2}`);
  throw t2;
}
...
class __PRIVATE_MemorySharedClientState {
  ...
  updateMutationState(e2, t2, n2) {}
  addLocalQueryTarget(e2, t2 = true) {
    return t2 && this.fu.Ba(e2), this.mu[e2] || "not-current";
  }
  updateQueryState(e2, t2, n2) {
    this.mu[e2] = t2;
  }
  ...
  isLocalQueryTarget(e2) {
    return this.fu.activeTargetIds.has(e2);
  }
  clearQueryState(e2) {
    delete this.mu[e2];
  }
...
class DocumentSet {
  ...
  constructor(e2) {
    this.comparator = e2 ? (t2, n2) => e2(t2, n2) || DocumentKey.comparator(t2.key, n2.key) : (e3, t2) => DocumentKey.comparator(e3.key, t2.key), this.keyedMap = documentMap(), this.sortedSet = new SortedMap(this.comparator);
  }
  has(e2) {
    return this.keyedMap.get(e2) != null;
  }
  get(e2) {
  ...
  forEach(e2) {
    this.sortedSet.inorderTraversal((t2, n2) => (e2(t2), false));
  }
  add(e2) {
  ...
  isEqual(e2) {
    if (!(e2 instanceof DocumentSet))
      return false;
    if (this.size !== e2.size)
      return false;
    const t2 = this.sortedSet.getIterator(), n2 = e2.sortedSet.getIterator();
    for (;t2.hasNext(); ) {
      const e3 = t2.getNext().key, r2 = n2.getNext().key;
      if (!e3.isEqual(r2))
        return false;
    }
    return true;
  ...
  toString() {
    ...
    return this.forEach((t2) => {
      e2.push(t2.toString());
    }), e2.length === 0 ? "DocumentSet ()" : `DocumentSet (
  ` + e2.join(`  
`) + `
)`;
  }
  copy(e2, t2) {
    ...
    return n2.comparator = this.comparator, n2.keyedMap = e2, n2.sortedSet = t2, n2;
  }
}
...
class __PRIVATE_DocumentChangeSet {
  ...
  track(e2) {
    const t2 = e2.doc.key, n2 = this.pu.get(t2);
    n2 ? e2.type !== 0 && n2.type === 3 ? this.pu = this.pu.insert(t2, e2) : e2.type === 3 && n2.type !== 1 ? this.pu = this.pu.insert(t2, {
      type: n2.type,
      doc: e2.doc
    }) : e2.type === 2 && n2.type === 2 ? this.pu = this.pu.insert(t2, {
      type: 2,
      doc: e2.doc
    }) : e2.type === 2 && n2.type === 0 ? this.pu = this.pu.insert(t2, {
      type: 0,
      doc: e2.doc
    }) : e2.type === 1 && n2.type === 0 ? this.pu = this.pu.remove(t2) : e2.type === 1 && n2.type === 2 ? this.pu = this.pu.insert(t2, {
      type: 1,
      doc: n2.doc
    }) : e2.type === 0 && n2.type === 1 ? this.pu = this.pu.insert(t2, {
      type: 2,
      doc: e2.doc
    }) : l(63341, {
      we: e2,
      gu: n2
    }) : this.pu = this.pu.insert(t2, e2);
  }
  yu() {
    const e2 = [];
    return this.pu.inorderTraversal((t2, n2) => {
      e2.push(n2);
    }), e2;
  }
}
...
class ViewSnapshot {
  constructor(e2, t2, n2, r2, i2, s2, _, o2, a) {
    this.query = e2, this.docs = t2, this.oldDocs = n2, this.docChanges = r2, this.mutatedKeys = i2, this.fromCache = s2, this.syncStateChanged = _, this.excludesMetadataChanges = o2, this.hasCachedResults = a;
  }
  static fromInitialDocuments(e2, t2, n2, r2, i2) {
    const s2 = [];
    return t2.forEach((e3) => {
      s2.push({
        type: 0,
    ...
    }), new ViewSnapshot(e2, t2, DocumentSet.emptySet(t2), s2, n2, r2, true, false, i2);
  }
  get hasPendingWrites() {
    return !this.mutatedKeys.isEmpty();
  }
  isEqual(e2) {
    if (!(this.fromCache === e2.fromCache && this.hasCachedResults === e2.hasCachedResults && this.syncStateChanged === e2.syncStateChanged && this.mutatedKeys.isEqual(e2.mutatedKeys) && __PRIVATE_queryOrPipelineEqual(this.query, e2.query) && this.docs.isEqual(e2.docs) && this.oldDocs.isEqual(e2.oldDocs)))
      return false;
    const t2 = this.docChanges, n2 = e2.docChanges;
    if (t2.length !== n2.length)
      return false;
    for (let e3 = 0;e3 < t2.length; e3++)
      if (t2[e3].type !== n2[e3].type || !t2[e3].doc.isEqual(n2[e3].doc))
        return false;
    return true;
  }
}
...
class __PRIVATE_QueryListenersInfo {
  constructor() {
    this.wu = undefined, this.bu = [];
  }
  Su() {
    return this.bu.some((e2) => e2.vu());
  }
}

class __PRIVATE_EventManagerImpl {
  constructor() {
    this.queries = __PRIVATE_newQueriesObjectMap(), this.onlineState = "Unknown", this.Du = new Set;
  }
  terminate() {
    (function __PRIVATE_errorAllTargets(e2, t2) {
      const n2 = __PRIVATE_debugCast(e2), r2 = n2.queries;
      n2.queries = __PRIVATE_newQueriesObjectMap(), r2.forEach((e3, n3) => {
        for (const e4 of n3.bu)
          e4.onError(t2);
      });
...
function __PRIVATE_newQueriesObjectMap() {
  return new ObjectMap((e2) => __PRIVATE_canonifyQueryOrPipeline(e2), __PRIVATE_queryOrPipelineEqual);
}
async function __PRIVATE_eventManagerListen(e2, t2) {
  const n2 = __PRIVATE_debugCast(e2);
  let r2 = 3;
  const i2 = t2.query;
  let s2 = n2.queries.get(i2);
  s2 ? !s2.Su() && t2.vu() && (r2 = 2) : (s2 = new __PRIVATE_QueryListenersInfo, r2 = t2.vu() ? 0 : 1);
  try {
    switch (r2) {
      case 0:
        s2.wu = await n2.onListen(i2, true);
        break;
      case 1:
        s2.wu = await n2.onListen(i2, false);
        break;
      case 2:
        await n2.onFirstRemoteStoreListen(i2);
    }
  } catch (e3) {
    const n3 = __PRIVATE_wrapInUserErrorIfRecoverable(e3, `Initialization of query '${__PRIVATE_isPipeline(t2.query) ? __PRIVATE_canonifyPipeline(t2.query) : __PRIVATE_stringifyQuery(t2.query)}' failed`);
    return void t2.onError(n3);
  }
  if (n2.queries.set(i2, s2), s2.bu.push(t2), t2.xu(n2.onlineState), s2.wu) {
    t2.Cu(s2.wu) && __PRIVATE_raiseSnapshotsInSyncEvent(n2);
  }
}
async function __PRIVATE_eventManagerUnlisten(e2, t2) {
  const n2 = __PRIVATE_debugCast(e2), r2 = t2.query;
  let i2 = 3;
  const s2 = n2.queries.get(r2);
  if (s2) {
    const e3 = s2.bu.indexOf(t2);
    e3 >= 0 && (s2.bu.splice(e3, 1), s2.bu.length === 0 ? i2 = t2.vu() ? 0 : 1 : !s2.Su() && t2.vu() && (i2 = 2));
  }
  switch (i2) {
    case 0:
      return n2.queries.delete(r2), n2.onUnlisten(r2, true);
    case 1:
      return n2.queries.delete(r2), n2.onUnlisten(r2, false);
    case 2:
...
function __PRIVATE_eventManagerOnWatchChange(e2, t2) {
  ...
  let r2 = false;
  for (const e3 of t2) {
    const t3 = e3.query, i2 = n2.queries.get(t3);
    if (i2) {
      for (const t4 of i2.bu)
        t4.Cu(e3) && (r2 = true);
      i2.wu = e3;
    }
  }
...
function __PRIVATE_eventManagerOnWatchError(e2, t2, n2) {
  const r2 = __PRIVATE_debugCast(e2), i2 = r2.queries.get(t2);
  if (i2)
    for (const e3 of i2.bu)
      e3.onError(n2);
...
var Vr;
(function(e2) {
  e2.Default = "default", e2.Cache = "cache";
})(Vr || (Vr = {}));

class __PRIVATE_QueryListener {
  constructor(e2, t2, n2) {
    this.query = e2, this.Fu = t2, this.Ou = false, this.Mu = null, this.onlineState = "Unknown", this.options = n2 || {};
  }
  Cu(e2) {
    if (!this.options.includeMetadataChanges) {
      const t3 = [];
      for (const n2 of e2.docChanges)
        n2.type !== 3 && t3.push(n2);
      e2 = new ViewSnapshot(e2.query, e2.docs, e2.oldDocs, t3, e2.mutatedKeys, e2.fromCache, e2.syncStateChanged, true, e2.hasCachedResults);
    }
    let t2 = false;
    return this.Ou ? this.Nu(e2) && (this.Fu.next(e2), t2 = true) : this.Lu(e2, this.onlineState) && (this.Bu(e2), t2 = true), this.Mu = e2, t2;
  }
  onError(e2) {
    this.Fu.error(e2);
  }
  xu(e2) {
    this.onlineState = e2;
    let t2 = false;
    return this.Mu && !this.Ou && this.Lu(this.Mu, e2) && (this.Bu(this.Mu), t2 = true), t2;
  }
  Lu(e2, t2) {
    if (!e2.fromCache)
      return true;
    if (!this.vu())
      return true;
    const n2 = t2 !== "Offline";
    return (!this.options.waitForSyncWhenOnline || !n2) && (!e2.docs.isEmpty() || e2.hasCachedResults || t2 === "Offline");
  }
  Nu(e2) {
    if (e2.docChanges.length > 0)
      return true;
    const t2 = this.Mu && this.Mu.hasPendingWrites !== e2.hasPendingWrites;
    return !(!e2.syncStateChanged && !t2) && this.options.includeMetadataChanges === true;
  }
  Bu(e2) {
    e2 = ViewSnapshot.fromInitialDocuments(e2.query, e2.docs, e2.mutatedKeys, e2.fromCache, e2.hasCachedResults), this.Ou = true, this.Fu.next(e2);
  }
  vu() {
...
class __PRIVATE_View {
  constructor(e2, t2) {
    this.query = e2, this.zu = t2, this.ju = null, this.hasCachedResults = false, this.current = false, this.Hu = __PRIVATE_documentKeySet(), this.mutatedKeys = __PRIVATE_documentKeySet(), this.Ju = __PRIVATE_isPipeline(e2) ? __PRIVATE_newPipelineComparator(e2) : __PRIVATE_newQueryComparator(e2), this.Yu = new DocumentSet(this.Ju);
  }
  get Zu() {
    return this.zu;
  }
  Xu(e2, t2) {
    const n2 = t2 ? t2.ec : new __PRIVATE_DocumentChangeSet, r2 = t2 ? t2.Yu : this.Yu;
    let i2 = t2 ? t2.mutatedKeys : this.mutatedKeys, s2 = r2, _ = false;
    const [o2, a] = this.tc(this.query, r2);
    e2.inorderTraversal((e3, t3) => {
      const u3 = r2.get(e3), c2 = __PRIVATE_queryOrPipelineMatches(this.query, t3) ? t3 : null, l2 = !!u3 && this.mutatedKeys.has(u3.key), E = !!c2 && (c2.hasLocalMutations || this.mutatedKeys.has(c2.key) && c2.hasCommittedMutations);
      let h = false;
      if (u3 && c2) {
        u3.data.isEqual(c2.data) ? l2 !== E && (n2.track({
          type: 3,
          doc: c2
        }), h = true) : this.nc(u3, c2) || (n2.track({
          type: 2,
          doc: c2
        }), h = true, (o2 && this.Ju(c2, o2) > 0 || a && this.Ju(c2, a) < 0) && (_ = true));
      } else
        !u3 && c2 ? (n2.track({
          type: 0,
          doc: c2
        }), h = true) : u3 && !c2 && (n2.track({
          type: 1,
          doc: u3
        }), h = true, (o2 || a) && (_ = true));
      h && (c2 ? (s2 = s2.add(c2), i2 = E ? i2.add(e3) : i2.delete(e3)) : (s2 = s2.delete(e3), i2 = i2.delete(e3)));
    });
    const u2 = this.rc(this.query);
    if (u2)
      if (__PRIVATE_isPipeline(this.query)) {
        const e3 = [];
        s2.forEach((t4) => e3.push(t4));
        const t3 = __PRIVATE_runPipeline(this.query, e3);
        let r3 = new DocumentSet(__PRIVATE_newPipelineComparator(this.query));
        for (const e4 of t3)
          r3 = r3.add(e4);
        s2.forEach((e4) => {
          r3.has(e4.key) || (i2 = i2.delete(e4.key), n2.track({
            type: 1,
            doc: e4
          }));
        }), s2 = r3;
      } else {
        const e3 = this.sc(this.query);
        for (;s2.size > u2; ) {
          const t3 = e3 === "F" ? s2.last() : s2.first();
          s2 = s2.delete(t3.key), i2 = i2.delete(t3.key), n2.track({
            type: 1,
            doc: t3
          });
        }
  ...
  rc(e2) {
    return __PRIVATE_isPipeline(e2) ? __PRIVATE_getLastEffectiveLimit(e2)?.limit : e2.limit || undefined;
  }
  sc(e2) {
    if (__PRIVATE_isPipeline(e2)) {
      const t2 = __PRIVATE_getLastEffectiveLimit(e2);
      return t2 && t2.limit < 0 ? "L" : "F";
    }
    return e2.limitType;
  }
  tc(e2, t2) {
    if (__PRIVATE_isPipeline(e2)) {
      const n2 = __PRIVATE_getLastEffectiveLimit(e2)?.limit;
      return [t2.size === n2 ? t2.last() : null, null];
    }
    return [e2.limitType === "F" && t2.size === this.rc(this.query) ? t2.last() : null, e2.limitType === "L" && t2.size === this.rc(this.query) ? t2.first() : null];
  }
  nc(e2, t2) {
    return e2.hasLocalMutations && t2.hasCommittedMutations && !t2.hasLocalMutations;
  }
  applyChanges(e2, t2, n2, r2) {
    const i2 = this.Yu;
    this.Yu = e2.Yu, this.mutatedKeys = e2.mutatedKeys;
    const s2 = e2.ec.yu();
    ...
    }(e3.type, t3.type) || this.Ju(e3.doc, t3.doc)), this._c(n2), r2 = r2 ?? false;
    const _ = t2 && !r2 ? this.oc() : [], o2 = this.Hu.size === 0 && this.current && !r2 ? 1 : 0, a = o2 !== this.ju;
    if (this.ju = o2, s2.length !== 0 || a) {
      return {
        snapshot: new ViewSnapshot(this.query, e2.Yu, i2, s2, e2.mutatedKeys, o2 === 0, a, false, !!n2 && n2.resumeToken.approximateByteSize() > 0),
        ac: _
      };
    }
    return {
  ...
  xu(e2) {
    return this.current && e2 === "Offline" ? (this.current = false, this.applyChanges({
      Yu: this.Yu,
  ...
  uc(e2) {
    return !this.zu.has(e2) && (!!this.Yu.has(e2) && !this.Yu.get(e2).hasLocalMutations);
  }
  _c(e2) {
    e2 && (e2.addedDocuments.forEach((e3) => this.zu = this.zu.add(e3)), e2.modifiedDocuments.forEach((e3) => {}), e2.removedDocuments.forEach((e3) => this.zu = this.zu.delete(e3)), this.current = e2.current);
  }
  oc() {
    if (!this.current)
      return [];
    const e2 = this.Hu;
    this.Hu = __PRIVATE_documentKeySet(), this.Yu.forEach((e3) => {
      this.uc(e3.key) && (this.Hu = this.Hu.add(e3.key));
    });
    const t2 = [];
    return e2.forEach((e3) => {
      this.Hu.has(e3) || t2.push(new __PRIVATE_RemovedLimboDocument(e3));
    }), this.Hu.forEach((n2) => {
      e2.has(n2) || t2.push(new __PRIVATE_AddedLimboDocument(n2));
    }), t2;
  }
  cc(e2) {
    this.zu = e2.Wo, this.Hu = __PRIVATE_documentKeySet();
    const t2 = this.Xu(e2.documents);
    return this.applyChanges(t2, true);
  }
  lc() {
    return ViewSnapshot.fromInitialDocuments(this.query, this.Yu, this.mutatedKeys, this.ju === 0, this.hasCachedResults);
  }
}
var dr = "SyncEngine";
...
class __PRIVATE_QueryView {
  constructor(e2, t2, n2) {
    this.query = e2, this.targetId = t2, this.view = n2;
  }
}

class LimboResolution {
  constructor(e2) {
    this.key = e2, this.Ec = false;
  }
}

class __PRIVATE_SyncEngineImpl {
  constructor(e2, t2, n2, r2, i2, s2) {
    this.localStore = e2, this.remoteStore = t2, this.eventManager = n2, this.sharedClientState = r2, this.currentUser = i2, this.maxConcurrentLimboResolutions = s2, this.hc = {}, this.Tc = new ObjectMap((e3) => __PRIVATE_canonifyQueryOrPipeline(e3), __PRIVATE_queryOrPipelineEqual), this.Pc = new Map, this.Ic = new Set, this.Rc = new SortedMap(DocumentKey.comparator), this.Ac = new Map, this.Vc = new __PRIVATE_ReferenceSet, this.dc = {}, this.fc = new Map, this.mc = __PRIVATE_TargetIdGenerator.bs(), this.onlineState = "Unknown", this.gc = undefined;
  }
  get isPrimaryClient() {
    return this.gc === true;
  }
}
async function __PRIVATE_syncEngineListen(e2, t2, n2 = true) {
  const r2 = __PRIVATE_ensureWatchCallbacks(e2);
  let i2;
  const s2 = r2.Tc.get(t2);
  return s2 ? (r2.sharedClientState.addLocalQueryTarget(s2.targetId), i2 = s2.view.lc()) : i2 = await __PRIVATE_allocateTargetAndMaybeListen(r2, t2, n2, true), i2;
}
async function __PRIVATE_triggerRemoteStoreListen(e2, t2) {
  const n2 = __PRIVATE_ensureWatchCallbacks(e2);
  await __PRIVATE_allocateTargetAndMaybeListen(n2, t2, true, false);
}
async function __PRIVATE_allocateTargetAndMaybeListen(e2, t2, n2, r2) {
  const i2 = await __PRIVATE_localStoreAllocateTarget(e2.localStore, __PRIVATE_isPipeline(t2) ? t2 : __PRIVATE_queryToTarget(t2)), s2 = i2.targetId, _ = e2.sharedClientState.addLocalQueryTarget(s2, n2);
  let o2;
  return r2 && (o2 = await __PRIVATE_initializeViewAndComputeSnapshot(e2, t2, s2, _ === "current", i2.resumeToken)), e2.isPrimaryClient && n2 && __PRIVATE_remoteStoreListen(e2.remoteStore, i2), o2;
}
async function __PRIVATE_initializeViewAndComputeSnapshot(e2, t2, n2, r2, i2) {
  e2.yc = (t3, n3, r3) => async function __PRIVATE_applyDocChanges(e3, t4, n4, r4) {
    let i3 = t4.view.Xu(n4);
    i3.Oo && (i3 = await __PRIVATE_localStoreExecuteQuery(e3.localStore, t4.query, false).then(({ documents: e4 }) => t4.view.Xu(e4, i3)));
    const s3 = r4 && r4.targetChanges.get(t4.targetId), _2 = r4 && r4.targetMismatches.get(t4.targetId) != null, o3 = t4.view.applyChanges(i3, e3.isPrimaryClient, s3, _2);
    return __PRIVATE_updateTrackedLimbos(e3, t4.targetId, o3.ac), o3.snapshot;
  }(e2, t3, n3, r3);
  const s2 = await __PRIVATE_localStoreExecuteQuery(e2.localStore, t2, true), _ = new __PRIVATE_View(t2, s2.Wo), o2 = _.Xu(s2.documents), a = TargetChange.createSynthesizedTargetChangeForCurrentChange(n2, r2 && e2.onlineState !== "Offline", i2), u2 = _.applyChanges(o2, e2.isPrimaryClient, a);
  __PRIVATE_updateTrackedLimbos(e2, n2, u2.ac);
  const c2 = new __PRIVATE_QueryView(t2, n2, _);
  return e2.Tc.set(t2, c2), e2.Pc.has(n2) ? e2.Pc.get(n2).push(t2) : e2.Pc.set(n2, [t2]), u2.snapshot;
}
async function __PRIVATE_syncEngineUnlisten(e2, t2, n2) {
  const r2 = __PRIVATE_debugCast(e2), i2 = r2.Tc.get(t2), s2 = r2.Pc.get(i2.targetId);
  if (s2.length > 1)
    return r2.Pc.set(i2.targetId, s2.filter((e3) => !__PRIVATE_queryOrPipelineEqual(e3, t2))), void r2.Tc.delete(t2);
  if (r2.isPrimaryClient) {
    r2.sharedClientState.removeLocalQueryTarget(i2.targetId);
    r2.sharedClientState.isActiveQueryTarget(i2.targetId) || await __PRIVATE_localStoreReleaseTarget(r2.localStore, i2.targetId, false).then(() => {
      r2.sharedClientState.clearQueryState(i2.targetId), n2 && __PRIVATE_remoteStoreUnlisten(r2.remoteStore, i2.targetId), __PRIVATE_removeAndCleanupTarget(r2, i2.targetId);
    }).catch(__PRIVATE_ignoreIfPrimaryLeaseLoss);
  } else
    __PRIVATE_removeAndCleanupTarget(r2, i2.targetId), await __PRIVATE_localStoreReleaseTarget(r2.localStore, i2.targetId, true);
}
async function __PRIVATE_triggerRemoteStoreUnlisten(e2, t2) {
  const n2 = __PRIVATE_debugCast(e2), r2 = n2.Tc.get(t2), i2 = n2.Pc.get(r2.targetId);
  n2.isPrimaryClient && i2.length === 1 && (n2.sharedClientState.removeLocalQueryTarget(r2.targetId), __PRIVATE_remoteStoreUnlisten(n2.remoteStore, r2.targetId));
}
  ...
  try {
    const e3 = await function __PRIVATE_localStoreWriteLocally(e4, t3) {
      ...
      return n3.persistence.runTransaction("Locally write mutations", "readwrite", (e5) => {
        let o2 = __PRIVATE_mutableDocumentMap(), a = __PRIVATE_documentKeySet();
        return n3.ko.getEntries(e5, i2).next((e6) => {
          ...
          for (const e6 of t3) {
            ...
            t4 != null && _2.push(new __PRIVATE_PatchMutation(e6.key, t4, __PRIVATE_extractFieldMask(t4.value.mapValue), Precondition.exists(true)));
          }
    ...
    r2.sharedClientState.addPendingMutation(e3.batchId), function __PRIVATE_addMutationCallback(e4, t3, n3) {
      let r3 = e4.dc[e4.currentUser.toKey()];
      r3 || (r3 = new SortedMap(__PRIVATE_primitiveComparator));
      r3 = r3.insert(t3, n3), e4.dc[e4.currentUser.toKey()] = r3;
...
async function __PRIVATE_syncEngineApplyRemoteEvent(e2, t2) {
  const n2 = __PRIVATE_debugCast(e2);
  try {
    const e3 = await __PRIVATE_localStoreApplyRemoteEventToLocalCache(n2.localStore, t2);
    t2.targetChanges.forEach((e4, t3) => {
      const r2 = n2.Ac.get(t3);
      r2 && (__PRIVATE_hardAssert(e4.addedDocuments.size + e4.modifiedDocuments.size + e4.removedDocuments.size <= 1, 22616), e4.addedDocuments.size > 0 ? r2.Ec = true : e4.modifiedDocuments.size > 0 ? __PRIVATE_hardAssert(r2.Ec, 14607) : e4.removedDocuments.size > 0 && (__PRIVATE_hardAssert(r2.Ec, 42227), r2.Ec = false));
    }), await __PRIVATE_syncEngineEmitNewSnapsAndNotifyLocalStore(n2, e3, t2);
  } catch (e3) {
    await __PRIVATE_ignoreIfPrimaryLeaseLoss(e3);
  }
}
function __PRIVATE_syncEngineApplyOnlineStateChange(e2, t2, n2) {
  const r2 = __PRIVATE_debugCast(e2);
  if (r2.isPrimaryClient && n2 === 0 || !r2.isPrimaryClient && n2 === 1) {
    const e3 = [];
    r2.Tc.forEach((n3, r3) => {
      const i2 = r3.view.xu(t2);
      i2.snapshot && e3.push(i2.snapshot);
    }), function __PRIVATE_eventManagerOnOnlineStateChange(e4, t3) {
      const n3 = __PRIVATE_debugCast(e4);
      n3.onlineState = t3;
      let r3 = false;
      n3.queries.forEach((e5, n4) => {
        for (const e6 of n4.bu)
          e6.xu(t3) && (r3 = true);
      }), r3 && __PRIVATE_raiseSnapshotsInSyncEvent(n3);
    }(r2.eventManager, t2), e3.length && r2.hc.Tn(e3), r2.onlineState = t2, r2.isPrimaryClient && r2.sharedClientState.setOnlineState(t2);
  }
}
async function __PRIVATE_syncEngineRejectListen(e2, t2, n2) {
  const r2 = __PRIVATE_debugCast(e2);
  r2.sharedClientState.updateQueryState(t2, "rejected", n2);
  const i2 = r2.Ac.get(t2), s2 = i2 && i2.key;
  if (s2) {
    let e3 = new SortedMap(DocumentKey.comparator);
    e3 = e3.insert(s2, MutableDocument.newNoDocument(s2, SnapshotVersion.min()));
    const n3 = __PRIVATE_documentKeySet().add(s2), i3 = new RemoteEvent(SnapshotVersion.min(), new Map, new SortedMap(__PRIVATE_primitiveComparator), e3, __PRIVATE_mutableDocumentMap(), n3);
    await __PRIVATE_syncEngineApplyRemoteEvent(r2, i3), r2.Rc = r2.Rc.remove(s2), r2.Ac.delete(t2), __PRIVATE_pumpEnqueuedLimboResolutions(r2);
  } else
    await __PRIVATE_localStoreReleaseTarget(r2.localStore, t2, false).then(() => __PRIVATE_removeAndCleanupTarget(r2, t2, n2)).catch(__PRIVATE_ignoreIfPrimaryLeaseLoss);
}
async function __PRIVATE_syncEngineApplySuccessfulWrite(e2, t2) {
  ...
  try {
    ...
    __PRIVATE_processUserCallback(n2, r2, null), __PRIVATE_triggerPendingWritesCallbacks(n2, r2), n2.sharedClientState.updateMutationState(r2, "acknowledged"), await __PRIVATE_syncEngineEmitNewSnapsAndNotifyLocalStore(n2, e3);
  } catch (e3) {
    await __PRIVATE_ignoreIfPrimaryLeaseLoss(e3);
  }
}
  ...
  try {
    const e3 = await function __PRIVATE_localStoreRejectBatch(e4, t3) {
      ...
      return n3.persistence.runTransaction("Reject batch", "readwrite-primary", (e5) => {
        let r3;
        return n3.mutationQueue.lookupMutationBatch(e5, t3).next((t4) => (__PRIVATE_hardAssert(t4 !== null, 37113), r3 = t4.keys(), n3.mutationQueue.removeMutationBatch(e5, t4))).next(() => n3.mutationQueue.performConsistencyCheck(e5)).next(() => n3.documentOverlayCache.removeOverlaysForBatchId(e5, r3, t3)).next(() => n3.localDocuments.recalculateAndSaveOverlaysForDocumentKeys(e5, r3)).next(() => n3.localDocuments.getDocuments(e5, r3));
      });
    }(r2.localStore, t2);
...
function __PRIVATE_triggerPendingWritesCallbacks(e2, t2) {
  (e2.fc.get(t2) || []).forEach((e3) => {
    e3.resolve();
  }), e2.fc.delete(t2);
}
function __PRIVATE_processUserCallback(e2, t2, n2) {
  const r2 = __PRIVATE_debugCast(e2);
  let i2 = r2.dc[r2.currentUser.toKey()];
  if (i2) {
    const e3 = i2.get(t2);
    e3 && (n2 ? e3.reject(n2) : e3.resolve(), i2 = i2.remove(t2)), r2.dc[r2.currentUser.toKey()] = i2;
  }
}
function __PRIVATE_removeAndCleanupTarget(e2, t2, n2 = null) {
  e2.sharedClientState.removeLocalQueryTarget(t2);
  for (const r2 of e2.Pc.get(t2))
    e2.Tc.delete(r2), n2 && e2.hc.wc(r2, n2);
  if (e2.Pc.delete(t2), e2.isPrimaryClient) {
    e2.Vc.e_(t2).forEach((t3) => {
      e2.Vc.containsKey(t3) || __PRIVATE_removeLimboTarget(e2, t3);
    });
  }
}
function __PRIVATE_removeLimboTarget(e2, t2) {
  e2.Ic.delete(t2.path.canonicalString());
  const n2 = e2.Rc.get(t2);
  n2 !== null && (__PRIVATE_remoteStoreUnlisten(e2.remoteStore, n2), e2.Rc = e2.Rc.remove(t2), e2.Ac.delete(n2), __PRIVATE_pumpEnqueuedLimboResolutions(e2));
}
function __PRIVATE_updateTrackedLimbos(e2, t2, n2) {
  for (const r2 of n2)
    if (r2 instanceof __PRIVATE_AddedLimboDocument)
      e2.Vc.addReference(r2.key, t2), __PRIVATE_trackLimboChange(e2, r2);
    else if (r2 instanceof __PRIVATE_RemovedLimboDocument) {
      __PRIVATE_logDebug(dr, "Document no longer in limbo: " + r2.key), e2.Vc.removeReference(r2.key, t2);
      e2.Vc.containsKey(r2.key) || __PRIVATE_removeLimboTarget(e2, r2.key);
    } else
      l(19791, {
        bc: r2
      });
...
function __PRIVATE_trackLimboChange(e2, t2) {
  ...
  e2.Rc.get(n2) || e2.Ic.has(r2) || (__PRIVATE_logDebug(dr, "New document in limbo: " + n2), e2.Ic.add(r2), __PRIVATE_pumpEnqueuedLimboResolutions(e2));
}
function __PRIVATE_pumpEnqueuedLimboResolutions(e2) {
  for (;e2.Ic.size > 0 && e2.Rc.size < e2.maxConcurrentLimboResolutions; ) {
    const t2 = e2.Ic.values().next().value;
    e2.Ic.delete(t2);
    const n2 = new DocumentKey(ResourcePath.fromString(t2)), r2 = e2.mc.next();
    e2.Ac.set(r2, new LimboResolution(n2)), e2.Rc = e2.Rc.insert(n2, r2), __PRIVATE_remoteStoreListen(e2.remoteStore, new TargetData(__PRIVATE_queryToTarget(__PRIVATE_newQueryForPath(n2.path)), r2, "TargetPurposeLimboResolution", __PRIVATE_ListenSequence.wn));
  }
}
async function __PRIVATE_syncEngineEmitNewSnapsAndNotifyLocalStore(e2, t2, n2) {
  const r2 = __PRIVATE_debugCast(e2), i2 = [], s2 = [], _ = [];
  r2.Tc.isEmpty() || (r2.Tc.forEach((e3, o2) => {
    _.push(r2.yc(o2, t2, n2).then((e4) => {
      if ((e4 || n2) && r2.isPrimaryClient) {
        const t3 = e4 ? !e4.fromCache : n2?.targetChanges.get(o2.targetId)?.current;
        r2.sharedClientState.updateQueryState(o2.targetId, t3 ? "current" : "not-current");
      }
      if (e4) {
        i2.push(e4);
        const t3 = __PRIVATE_LocalViewChanges.mo(o2.targetId, e4);
        s2.push(t3);
      }
    }));
  }), await Promise.all(_), r2.hc.Tn(i2), await async function __PRIVATE_localStoreNotifyLocalViewChanges(e3, t3) {
    const n3 = __PRIVATE_debugCast(e3);
    try {
      await n3.persistence.runTransaction("notifyLocalViewChanges", "readwrite", (e4) => PersistencePromise.forEach(t3, (t4) => PersistencePromise.forEach(t4.Vo, (r3) => n3.persistence.referenceDelegate.addReference(e4, t4.targetId, r3)).next(() => PersistencePromise.forEach(t4.fo, (r3) => n3.persistence.referenceDelegate.removeReference(e4, t4.targetId, r3)))));
    } catch (e4) {
      if (!__PRIVATE_isIndexedDbTransactionError(e4))
        throw e4;
      __PRIVATE_logDebug(lr, "Failed to update sequence numbers: " + e4);
    }
    for (const e4 of t3) {
      const t4 = e4.targetId;
      if (!e4.fromCache) {
        const e5 = n3.Lo.get(t4), r3 = e5.snapshotVersion, i3 = e5.withLastLimboFreeSnapshotVersion(r3);
        n3.Lo = n3.Lo.insert(t4, i3);
      }
    }
  }(r2.localStore, s2));
}
async function __PRIVATE_syncEngineHandleCredentialChange(t2, n2) {
  const r2 = __PRIVATE_debugCast(t2);
  if (!r2.currentUser.isEqual(n2)) {
    __PRIVATE_logDebug(dr, "User change. New user:", n2.toKey());
    const t3 = await __PRIVATE_localStoreHandleUserChange(r2.localStore, n2);
    r2.currentUser = n2, function __PRIVATE_rejectOutstandingPendingWritesCallbacks(t4, n3) {
      t4.fc.forEach((t5) => {
        t5.forEach((t6) => {
          t6.reject(new e(ta.CANCELLED, n3));
        });
      }), t4.fc.clear();
    }(r2, "'waitForPendingWrites' promise is rejected due to a user change."), r2.sharedClientState.handleUserChange(n2, t3.removedBatchIds, t3.addedBatchIds), await __PRIVATE_syncEngineEmitNewSnapsAndNotifyLocalStore(r2, t3.$o);
  }
}
function __PRIVATE_syncEngineGetRemoteKeysForTarget(e2, t2) {
  const n2 = __PRIVATE_debugCast(e2), r2 = n2.Ac.get(t2);
  if (r2 && r2.Ec)
    return __PRIVATE_documentKeySet().add(r2.key);
  {
    let e3 = __PRIVATE_documentKeySet();
    const r3 = n2.Pc.get(t2);
    if (!r3)
      return e3;
    for (const t3 of r3 ?? []) {
      const r4 = n2.Tc.get(t3);
      e3 = e3.unionWith(r4.view.Zu);
    }
    return e3;
  }
}
...
class __PRIVATE_MemoryOfflineComponentProvider {
  ...
  async initialize(e2) {
    this.serializer = __PRIVATE_newSerializer(e2.databaseInfo.databaseId), this.sharedClientState = this.vc(e2), this.persistence = this.Dc(e2), await this.persistence.start(), this.localStore = this.xc(e2), this.gcScheduler = this.Cc(e2, this.localStore), this.indexBackfillerScheduler = this.Fc(e2, this.localStore);
  }
  Cc(e2, t2) {
    return null;
  }
...
class __PRIVATE_LruGcMemoryOfflineComponentProvider extends __PRIVATE_MemoryOfflineComponentProvider {
  ...
  Cc(e2, t2) {
    ...
    return new __PRIVATE_LruScheduler(n2, e2.asyncQueue, t2);
  }
  Dc(e2) {
    const t2 = this.cacheSizeBytes !== undefined ? LruParams.withCacheSize(this.cacheSizeBytes) : LruParams.DEFAULT;
    return new __PRIVATE_MemoryPersistence((e3) => __PRIVATE_MemoryLruDelegate.b_(e3, t2), this.serializer);
  }
}
class OnlineComponentProvider {
  async initialize(e2, t2) {
    this.localStore || (this.localStore = e2.localStore, this.sharedClientState = e2.sharedClientState, this.datastore = this.createDatastore(t2), this.remoteStore = this.createRemoteStore(t2), this.eventManager = this.createEventManager(t2), this.syncEngine = this.createSyncEngine(t2, !e2.synchronizeTabs), this.sharedClientState.onlineStateHandler = (e3) => __PRIVATE_syncEngineApplyOnlineStateChange(this.syncEngine, e3, 1), this.remoteStore.remoteSyncer.handleCredentialChange = __PRIVATE_syncEngineHandleCredentialChange.bind(null, this.syncEngine), await __PRIVATE_remoteStoreApplyPrimaryState(this.remoteStore, this.syncEngine.isPrimaryClient));
  }
  ...
  createDatastore(e2) {
    const t2 = __PRIVATE_newSerializer(e2.databaseInfo.databaseId), n2 = __PRIVATE_newConnection(e2.databaseInfo);
    return __PRIVATE_newDatastore(e2.authCredentials, e2.appCheckCredentials, n2, t2);
  }
  createRemoteStore(e2) {
    return function __PRIVATE_newRemoteStore(e3, t2, n2, r2, i2) {
      return new __PRIVATE_RemoteStoreImpl(e3, t2, n2, r2, i2);
    }(this.localStore, this.datastore, e2.asyncQueue, (e3) => __PRIVATE_syncEngineApplyOnlineStateChange(this.syncEngine, e3, 0), function __PRIVATE_newConnectivityMonitor() {
  ...
  createSyncEngine(e2, t2) {
    return function __PRIVATE_newSyncEngine(e3, t3, n2, r2, i2, s2, _) {
      const o2 = new __PRIVATE_SyncEngineImpl(e3, t3, n2, r2, i2, s2);
      return _ && (o2.gc = true), o2;
  ...
  async terminate() {
    await async function __PRIVATE_remoteStoreShutdown(e2) {
      const t2 = __PRIVATE_debugCast(e2);
      __PRIVATE_logDebug(hr, "RemoteStore shutting down."), t2.la.add(5), await __PRIVATE_disableNetworkInternal(t2), t2.ha.shutdown(), t2.Ta.set("Unknown");
...
class FirestoreClient {
  constructor(e2, t2, n2, r2, i2) {
    this.authCredentials = e2, this.appCheckCredentials = t2, this.asyncQueue = n2, this._databaseInfo = r2, this.user = User.UNAUTHENTICATED, this.clientId = __PRIVATE_AutoId.newId(), this.authCredentialListener = () => Promise.resolve(), this.appCheckCredentialListener = () => Promise.resolve(), this._uninitializedComponentsProvider = i2, this.authCredentials.start(n2, async (e3) => {
      __PRIVATE_logDebug(fr, "Received user=", e3.uid), await this.authCredentialListener(e3), this.user = e3;
    }), this.appCheckCredentials.start(n2, (e3) => (__PRIVATE_logDebug(fr, "Received new app check token=", e3), this.appCheckCredentialListener(e3, this.user)));
  }
  get configuration() {
    return {
  ...
  terminate() {
    ...
    return this.asyncQueue.enqueueAndForgetEvenWhileRestricted(async () => {
      try {
        this._onlineComponents && await this._onlineComponents.terminate(), this._offlineComponents && await this._offlineComponents.terminate(), this.authCredentials.shutdown(), this.appCheckCredentials.shutdown(), e2.resolve();
      } catch (t2) {
...
async function __PRIVATE_setOfflineComponentProvider(e2, t2) {
  e2.asyncQueue.verifyOperationInProgress(), __PRIVATE_logDebug(fr, "Initializing OfflineComponentProvider");
  const n2 = e2.configuration;
  ...
  }), t2.persistence.setDatabaseDeletedListener(() => e2.terminate()), e2._offlineComponents = t2;
}
async function __PRIVATE_setOnlineComponentProvider(e2, t2) {
  e2.asyncQueue.verifyOperationInProgress();
  const n2 = await __PRIVATE_ensureOfflineComponents(e2);
  __PRIVATE_logDebug(fr, "Initializing OnlineComponentProvider"), await t2.initialize(n2, e2.configuration), e2.setCredentialChangeListener((e3) => __PRIVATE_remoteStoreHandleCredentialChange(t2.remoteStore, e3)), e2.setAppCheckTokenChangeListener((e3, n3) => __PRIVATE_remoteStoreHandleCredentialChange(t2.remoteStore, n3)), e2._onlineComponents = t2;
}
async function __PRIVATE_ensureOfflineComponents(e2) {
  if (!e2._offlineComponents)
    if (e2._uninitializedComponentsProvider) {
      ...
      try {
        ...
        if (!function __PRIVATE_canFallbackFromIndexedDbError(e3) {
          return e3.name === "FirebaseError" ? e3.code === ta.FAILED_PRECONDITION || e3.code === ta.UNIMPLEMENTED : !(typeof DOMException != "undefined" && e3 instanceof DOMException) || e3.code === 22 || e3.code === 20 || e3.code === 11;
        }(n2))
          throw n2;
        __PRIVATE_logWarn("Error using user provided cache. Falling back to memory cache: " + n2), await __PRIVATE_setOfflineComponentProvider(e2, new __PRIVATE_MemoryOfflineComponentProvider);
      }
    } else
      __PRIVATE_logDebug(fr, "Using default OfflineComponentProvider"), await __PRIVATE_setOfflineComponentProvider(e2, new __PRIVATE_LruGcMemoryOfflineComponentProvider(undefined));
...
function __PRIVATE_firestoreClientGetDocumentViaSnapshotListener(t2, n2, r2 = {}) {
  ...
  return t2.asyncQueue.enqueueAndForget(async () => function __PRIVATE_readDocumentViaSnapshotListener(t3, n3, r3, i3, s2) {
    const _ = new __PRIVATE_AsyncObserver({
      next: (a) => {
        _.Va(), n3.enqueueAndForget(() => __PRIVATE_eventManagerUnlisten(t3, o2));
        const u2 = a.docs.has(r3);
        !u2 && a.fromCache ? s2.reject(new e(ta.UNAVAILABLE, "Failed to get document because the client is offline.")) : u2 && a.fromCache && i3 && i3.source === "server" ? s2.reject(new e(ta.UNAVAILABLE, 'Failed to get document from server. (However, this document does exist in the local cache. Run again without setting source to "server" to retrieve the cached document.)')) : s2.resolve(a);
      },
      error: (e2) => s2.reject(e2)
    }), o2 = new __PRIVATE_QueryListener(__PRIVATE_newQueryForPath(r3.path), _, {
      includeMetadataChanges: true,
      waitForSyncWhenOnline: true
    });
    return __PRIVATE_eventManagerListen(t3, o2);
  }(await __PRIVATE_getEventManager(t2), t2.asyncQueue, n2, r2, i2)), i2.promise;
}
function __PRIVATE_firestoreClientGetDocumentsViaSnapshotListener(t2, n2, r2 = {}) {
  const i2 = new __PRIVATE_Deferred;
  return t2.asyncQueue.enqueueAndForget(async () => function __PRIVATE_executeQueryViaSnapshotListener(t3, n3, r3, i3, s2) {
    const _ = new __PRIVATE_AsyncObserver({
      next: (r4) => {
        _.Va(), n3.enqueueAndForget(() => __PRIVATE_eventManagerUnlisten(t3, o2)), r4.fromCache && i3.source === "server" ? s2.reject(new e(ta.UNAVAILABLE, 'Failed to get documents from server. (However, these documents may exist in the local cache. Run again without setting source to "server" to retrieve the cached documents.)')) : s2.resolve(r4);
      },
      error: (e2) => s2.reject(e2)
    }), o2 = new __PRIVATE_QueryListener(r3 instanceof __PRIVATE_RealtimePipeline ? __PRIVATE_toCorePipeline(r3) : r3, _, {
      includeMetadataChanges: true,
      waitForSyncWhenOnline: true
    });
    return __PRIVATE_eventManagerListen(t3, o2);
...
var mr = class DocumentSnapshot {
  constructor(e2, t2, n2, r2, i2) {
    this._firestore = e2, this._userDataWriter = t2, this._key = n2, this._document = r2, this._converter = i2;
  }
  get id() {
    return this._key.path.lastSegment();
  }
  ...
  data() {
    if (this._document) {
      if (this._converter) {
        const e2 = new pr(this._firestore, this._userDataWriter, this._key, this._document, null);
  ...
  get(e2) {
    if (this._document) {
      const t2 = this._document.data.field(K("DocumentSnapshot.get", e2));
      if (t2 !== null)
...
class AbstractUserDataWriter {
  ...
  convertObjectMap(e2, t2 = "none") {
    ...
    return forEach(e2, (e3, r2) => {
      n2[e3] = this.convertValue(r2, t2);
    }), n2;
  }
  convertVectorValue(e2) {
    const t2 = e2.fields?.[je].arrayValue?.values?.map((e3) => __PRIVATE_normalizeNumber(e3.doubleValue));
    return new n(t2);
  }
  convertGeoPoint(e2) {
  ...
  convertServerTimestamp(e2, t2) {
    switch (t2) {
      case "previous":
        const n2 = __PRIVATE_getPreviousValue(e2);
        return n2 == null ? null : this.convertValue(n2, t2);
      case "estimate":
  ...
  convertDocumentKey(e2, t2) {
    const n2 = ResourcePath.fromString(e2);
    __PRIVATE_hardAssert(__PRIVATE_isValidResourceName(n2), 9688, {
      name: e2
    });
    const r2 = new DatabaseId(n2.get(1), n2.get(3)), i2 = new DocumentKey(n2.popFirst(5));
    return r2.isEqual(t2) || __PRIVATE_logError(`A document reference to ${i2} refers to a different database (${r2.projectId}/${r2.database}), which is not supported. It will be treated as a reference in the current database (${t2.projectId}/${t2.database}) instead.`), i2;
  }
}
function __PRIVATE_applyFirestoreDataConverter(e2, t2, n2) {
  let r2;
...
class __PRIVATE_AsyncQueueImpl {
  constructor(e2 = Promise.resolve()) {
    this.$c = [], this.Kc = false, this.Qc = [], this.Wc = null, this.Gc = false, this.zc = false, this.jc = [], this.Ht = new __PRIVATE_ExponentialBackoff(this, "async_queue_retry"), this.Hc = () => {
      ...
      e3 && __PRIVATE_logDebug(gr, "Visibility state changed to " + e3.visibilityState), this.Ht.$t();
    }, this.Jc = e2;
    const t2 = getDocument();
    t2 && typeof t2.addEventListener == "function" && t2.addEventListener("visibilitychange", this.Hc);
  }
  get isShuttingDown() {
    return this.Kc;
  }
  ...
  enterRestrictedMode(e2) {
    if (!this.Kc) {
      this.Kc = true, this.zc = e2 || false;
      const t2 = getDocument();
      t2 && typeof t2.removeEventListener == "function" && t2.removeEventListener("visibilitychange", this.Hc);
    }
  }
  enqueue(e2) {
    if (this.Yc(), this.Kc)
    ...
    return this.Zc(() => this.Kc && this.zc ? Promise.resolve() : (e2().then(t2.resolve, t2.reject), t2.promise)).then(() => t2.promise);
  }
  enqueueRetryable(e2) {
    this.enqueueAndForget(() => (this.$c.push(e2), this.Xc()));
  }
  async Xc() {
    if (this.$c.length !== 0) {
      try {
        ...
        if (!__PRIVATE_isIndexedDbTransactionError(e2))
          throw e2;
        __PRIVATE_logDebug(gr, "Operation failed with retryable error: " + e2);
      }
      this.$c.length > 0 && this.Ht.kt(() => this.Xc());
    }
  }
  Zc(e2) {
    const t2 = this.Jc.then(() => (this.Gc = true, e2().catch((e3) => {
      this.Wc = e3, this.Gc = false;
      throw __PRIVATE_logError("INTERNAL UNHANDLED ERROR: ", __PRIVATE_getMessageOrStack(e3)), e3;
    }).then((e3) => (this.Gc = false, e3))));
    return this.Jc = t2, t2;
  }
  enqueueAfterDelay(e2, t2, n2) {
    this.Yc(), this.jc.indexOf(e2) > -1 && (t2 = 0);
    const r2 = DelayedOperation.createAndSchedule(this, e2, t2, n2, (e3) => this.el(e3));
    return this.Qc.push(r2), r2;
  }
  Yc() {
    this.Wc && l(47125, {
      tl: __PRIVATE_getMessageOrStack(this.Wc)
    });
  }
  ...
  il(e2) {
    return this.nl().then(() => {
      this.Qc.sort((e3, t2) => e3.targetTimeMs - t2.targetTimeMs);
      for (const t2 of this.Qc)
        if (t2.skipDelay(), e2 !== "all" && t2.timerId === e2)
          break;
      return this.nl();
    });
  }
...
class da extends Dt {
  constructor(e2, t2, n2, r2) {
    super(e2, t2, n2, r2), this.type = "firestore", this._queue = new __PRIVATE_AsyncQueueImpl, this._persistenceKey = r2?.name || "[DEFAULT]";
  }
  async _terminate() {
    if (this._firestoreClient) {
      const e2 = this._firestoreClient.terminate();
      this._queue = new __PRIVATE_AsyncQueueImpl(e2), this._firestoreClient = undefined, await e2;
    }
  }
}
function getFirestore(e2, t2) {
  const n2 = typeof e2 == "object" ? e2 : getApp(), r2 = typeof e2 == "string" ? e2 : t2 || $e, i2 = _getProvider(n2, "firestore").getImmediate({
    identifier: r2
  });
  if (!i2._initialized) {
    const e3 = getDefaultEmulatorHostnameAndPort("firestore");
    e3 && connectFirestoreEmulator(i2, ...e3);
  }
  return i2;
}
function oa(t2) {
  if (t2._terminated)
    throw new e(ta.FAILED_PRECONDITION, "The client has already been terminated.");
  return t2._firestoreClient || __PRIVATE_configureFirestore(t2), t2._firestoreClient;
}
function __PRIVATE_configureFirestore(e2) {
  const t2 = e2._freezeSettings(), n2 = __PRIVATE_makeDatabaseInfo(e2._databaseId, e2._app?.options.appId || "", e2._persistenceKey, e2._app?.options.apiKey, t2);
  e2._componentsProvider || t2.localCache?._offlineComponentProvider && t2.localCache?._onlineComponentProvider && (e2._componentsProvider = {
    _offline: t2.localCache._offlineComponentProvider,
    _online: t2.localCache._onlineComponentProvider
  }), e2._firestoreClient = new FirestoreClient(e2._authCredentials, e2._appCheckCredentials, e2._queue, n2, e2._componentsProvider && function __PRIVATE_buildComponentProvider(e3) {
    const t3 = e3?._online.build();
    return {
      _offline: e3?._offline.build(t3),
      _online: t3
    };
  }(e2._componentsProvider));
}
class ua extends AbstractUserDataWriter {
  constructor(e2) {
  ...
  convertReference(e2) {
    ...
    return new aa(this.firestore, null, t2);
  }
}
...
class DocumentSnapshot2 extends mr {
  constructor(e2, t2, n2, r2, i2, s2) {
    super(e2, t2, n2, r2, s2), this._firestore = e2, this._firestoreImpl = e2, this.metadata = i2;
  }
  exists() {
    return super.exists();
  }
  data(e2 = {}) {
    if (this._document) {
      if (this._converter) {
        const t2 = new QueryDocumentSnapshot2(this._firestore, this._userDataWriter, this._key, this._document, this.metadata, null);
  ...
  get(e2, t2 = {}) {
    if (this._document) {
      const n2 = this._document.data.field(K("DocumentSnapshot.get", e2));
      if (n2 !== null)
  ...
  toJSON() {
    if (this.metadata.hasPendingWrites)
      throw new e(ta.FAILED_PRECONDITION, "DocumentSnapshot.toJSON() attempted to serialize a document with pending writes. Await waitForPendingWrites() before invoking toJSON().");
    const t2 = this._document, n2 = {};
    if (n2.type = DocumentSnapshot2._jsonSchemaVersion, n2.bundle = "", n2.bundleSource = "DocumentSnapshot", n2.bundleName = this._key.toString(), !t2 || !t2.isValidDocument() || !t2.isFoundDocument())
      return n2;
    this._userDataWriter.convertObjectMap(t2.data.value.mapValue.fields, "previous");
    return n2.bundle = (this._firestore, this.ref.path, "NOT SUPPORTED"), n2;
  }
}
...
class QuerySnapshot {
  constructor(e2, t2, n2, r2) {
    this._firestore = e2, this._userDataWriter = t2, this._snapshot = r2, this.metadata = new SnapshotMetadata(r2.hasPendingWrites, r2.fromCache), this.query = n2;
  }
  get docs() {
    const e2 = [];
  ...
  forEach(e2, t2) {
    this._snapshot.docs.forEach((n2) => {
      e2.call(t2, new QueryDocumentSnapshot2(this._firestore, this._userDataWriter, n2.key, n2, new SnapshotMetadata(this._snapshot.mutatedKeys.has(n2.key), this._snapshot.fromCache), this.query.converter));
    });
  }
  docChanges(t2 = {}) {
    const n2 = !!t2.includeMetadataChanges;
    if (n2 && this._snapshot.excludesMetadataChanges)
      throw new e(ta.INVALID_ARGUMENT, "To include metadata changes with your document changes, you must also pass { includeMetadataChanges:true } to onSnapshot().");
    return this._cachedChanges && this._cachedChangesIncludeMetadataChanges === n2 || (this._cachedChanges = function __PRIVATE_changesFromSnapshot(e2, t3) {
      if (e2._snapshot.oldDocs.isEmpty()) {
        let t4 = 0;
        return e2._snapshot.docChanges.map((n3) => {
          __PRIVATE_isPipeline(e2._snapshot.query) ? __PRIVATE_newPipelineComparator(e2._snapshot.query) : __PRIVATE_newQueryComparator(e2.query._query);
          const r2 = new QueryDocumentSnapshot2(e2._firestore, e2._userDataWriter, n3.doc.key, n3.doc, new SnapshotMetadata(e2._snapshot.mutatedKeys.has(n3.doc.key), e2._snapshot.fromCache), e2.query.converter);
          return n3.doc, {
            type: "added",
            doc: r2,
            oldIndex: -1,
      ...
      {
        let n3 = e2._snapshot.oldDocs;
        return e2._snapshot.docChanges.filter((e3) => t3 || e3.type !== 3).map((t4) => {
          const r2 = new QueryDocumentSnapshot2(e2._firestore, e2._userDataWriter, t4.doc.key, t4.doc, new SnapshotMetadata(e2._snapshot.mutatedKeys.has(t4.doc.key), e2._snapshot.fromCache), e2.query.converter);
          let i2 = -1, s2 = -1;
          return t4.type !== 0 && (i2 = n3.indexOf(t4.doc.key), n3 = n3.delete(t4.doc.key)), t4.type !== 1 && (n3 = n3.add(t4.doc), s2 = n3.indexOf(t4.doc.key)), {
    ...
    }(this, n2), this._cachedChangesIncludeMetadataChanges = n2), this._cachedChanges;
  }
  toJSON() {
    if (this.metadata.hasPendingWrites)
      throw new e(ta.FAILED_PRECONDITION, "QuerySnapshot.toJSON() attempted to serialize a document with pending writes. Await waitForPendingWrites() before invoking toJSON().");
    const t2 = {};
    ...
    return this.docs.forEach((e2) => {
      e2._document !== null && (n2.push(e2._document), r2.push(this._userDataWriter.convertObjectMap(e2._document.data.value.mapValue.fields, "previous")), i2.push(e2.ref.path));
...
function __PRIVATE_validateHasExplicitOrderByForLimitToLast(e$1) {
  if (e$1.limitType === "L" && e$1.explicitOrderBy.length === 0)
    throw new e(ta.UNIMPLEMENTED, "limitToLast() queries require specifying at least one orderBy() clause");
}
function getDoc(e2) {
  e2 = ra(e2, aa);
...
function setDoc(e2, t2, n2) {
  ...
  return executeWrite(r2, [__PRIVATE_parseSetData(a, "setDoc", e2._key, s2, e2.converter !== null, n2).toMutation(e2._key, Precondition.none())]);
}
function executeWrite(e2, t2) {
  const n2 = oa(e2);
  return __PRIVATE_firestoreClientWrite(n2, t2);
}
function __PRIVATE_convertToDocSnapshot(e2, t2, n2) {
  const r2 = n2.docs.get(t2._key), s2 = new ua(e2);
  return new DocumentSnapshot2(e2, s2, t2._key, r2, new SnapshotMetadata(n2.hasPendingWrites, n2.fromCache), t2.converter);
}
var De2 = new WeakMap;
var Be2 = "@firebase/firestore";
var Me2 = "4.17.2";
(function __PRIVATE_registerFirestore(s2, a = true) {
  __PRIVATE_setSDKVersion(SDK_VERSION), _registerComponent(new Component("firestore", (e2, { instanceIdentifier: t2, options: n2 }) => {
    const r2 = e2.getProvider("app").getImmediate(), s3 = new da(new __PRIVATE_FirebaseAuthCredentialsProvider(e2.getProvider("auth-internal")), new __PRIVATE_FirebaseAppCheckTokenProvider(r2, e2.getProvider("app-check-internal")), __PRIVATE_databaseIdFromApp(r2, t2), r2);
    return n2 = {
      useFetchStreams: a,
      ...n2
    }, s3._setSettings(n2), s3;
  }, "PUBLIC").setMultipleInstances(true)), registerVersion(Be2, Me2, s2), registerVersion(Be2, Me2, "esm2020");
})();
// firebase-applet-config.json
var firebase_applet_config_default = {
  projectId: "gen-lang-client-0984152425",
  appId: "1:223425053807:web:cf2671825e9f2a6949799a",
  apiKey: "AIzaSyBORbfu9s8tQJy-Ju76wmsA854u_y5xH7E",
  authDomain: "gen-lang-client-0984152425.firebaseapp.com",
  firestoreDatabaseId: "ai-studio-baremos-4cd65427-519b-465d-92dc-822cb0099a24",
  storageBucket: "gen-lang-client-0984152425.firebasestorage.app",
  messagingSenderId: "223425053807",
  measurementId: "",
  oAuthClientId: "223425053807-nsh4lrfosaiadvgr2ks9osbunpa9h23n.apps.googleusercontent.com",
  recaptchaSiteKey: ""
};

// firebase-bridge.js
var app2 = initializeApp(firebase_applet_config_default);
var auth2 = getAuth(app2);
var db = getFirestore(app2, firebase_applet_config_default.firestoreDatabaseId);
var OperationType2 = {
  CREATE: "create",
  UPDATE: "update",
  DELETE: "delete",
...
function handleFirestoreError(error, operationType, path) {
  const errInfo = {
    ...
    authInfo: {
      userId: auth2.currentUser?.uid || null,
      email: auth2.currentUser?.email || null,
      emailVerified: auth2.currentUser?.emailVerified || false,
      isAnonymous: auth2.currentUser?.isAnonymous || false,
      tenantId: auth2.currentUser?.tenantId || null,
      providerInfo: auth2.currentUser?.providerData?.map((p2) => ({
...
async function loginWithGoogle() {
  try {
    const res = await signInWithPopup(auth2, googleProvider);
    return res.user;
  } catch (err) {
...
async function saveCloudUserProfile(profile) {
  ...
  const data = {
    userId: uid,
    nombre: String(profile.nombre || auth2.currentUser.displayName || "Contratista").slice(0, 100),
    legajo: String(profile.legajo || "").slice(0, 50),
    zona: String(profile.zona || "General").slice(0, 100),
    email: String(auth2.currentUser.email || profile.email || "").slice(0, 120),
  ...
  try {
    ...
    handleFirestoreError(err, OperationType2.WRITE, path);
  }
}
async function getCloudUserProfile() {
  ...
  try {
    ...
    handleFirestoreError(err, OperationType2.GET, path);
  }
}
async function syncJornadaToCloud(jornada) {
  if (!auth2.currentUser || !jornada)
    return;
  const uid = auth2.currentUser.uid;
  const jId = String(jornada.id || jornada.fecha + "_" + (jornada.legajo || "0")).replace(/[^a-zA-Z0-9_\-]/g, "_");
  const path = `users/${uid}/jornadas/${jId}`;
  const cleanItems = Array.isArray(jornada.items) ? jornada.items.slice(0, 150).map((it2) => ({
    baremo: String(it2.baremo || "").slice(0, 50),
    cant: Number(it2.cant) || 1,
    desc: String(it2.desc || "").slice(0, 200),
    monto: Number(it2.monto) || 0,
    subtotal: Number(it2.subtotal) || 0,
    unidad: String(it2.unidad || "").slice(0, 30)
  })) : [];
  const payload = {
    userId: uid,
    id: jId,
...
async function syncCombustibleToCloud(ticket) {
  if (!auth2.currentUser || !ticket)
    return;
  const uid = auth2.currentUser.uid;
...
async function syncQuincenaToCloud(quincena) {
  if (!auth2.currentUser || !quincena)
    return;
  const uid = auth2.currentUser.uid;
...
async function fetchCloudJornadas() {
  if (!auth2.currentUser)
    return [];
  const uid = auth2.currentUser.uid;
  ...
  try {
    ...
    handleFirestoreError(err, OperationType2.LIST, path);
  }
}
async function syncFullCloudDatabase() {
  if (!auth2.currentUser || !window.dbGetAll || !window.dbPut)
    return { synced: false, reason: "No auth or db" };
  const uid = auth2.currentUser.uid;
  try {
    const localJornadas = await window.dbGetAll("jornadas");
    for (const j of localJornadas) {
      if (j)
        await syncJornadaToCloud(j);
    }
    const localCombustible = await window.dbGetAll("combustible");
    for (const c2 of localCombustible) {
      if (c2)
        await syncCombustibleToCloud(c2);
    }
    const localQuincenas = await window.dbGetAll("quincenas");
    for (const q of localQuincenas) {
      if (q)
        await syncQuincenaToCloud(q);
    }
...
window.FirebaseSync = {
  ...
  onAuthStateChanged: (cb) => onAuthStateChanged(auth2, cb)
};
export {
